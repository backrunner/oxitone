...
export function projectBuildOptions(entry: string): BuildOptions {
  ...
  return { absWorkingDir: dirname(resolve(entry)), entryPoints: [resolve(entry)], bundle: true,
    splitting: false, write: false, platform: "node", format: "esm", target: "node22",
    sourcemap: "inline", metafile: true, logLevel: "silent", outfile: "project.mjs", plugins: [sources] };
}
...
export async function bundleProject(entry: string, output: string, watch = false): Promise<void> {
  entry = resolve(entry); output = resolve(output);
  ...
  if (!watch) { await publish(await build(options)); return; }
  options.plugins!.push({ name: "oxitone-build-output", setup(api) { api.onEnd(async result => {
    if (result.errors.length) console.error(`[PreviewBuildFailed] ${result.errors[0]!.text}`);
    else { try { await publish(result); } catch (error) { console.error(String(error)); } }
  }); } });
  const builder = await context(options);
  try {
    await builder.watch();
    await new Promise<void>(done => {
      const stop = () => { process.off("SIGINT", stop); process.off("SIGTERM", stop); done(); };
