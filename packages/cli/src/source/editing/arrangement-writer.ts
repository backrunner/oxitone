...
export function assertArrangement(before: ProjectSnapshot, expected: ProjectSnapshot, actual: ProjectSnapshot): void {
  ...
  for (const key of ["channels", "patternClips", "sampleClips", "automationClips"] as const) {
    ...
    a.forEach((item, i) => {
      ...
      if ("instrument" in item && "instrument" in b[i]!) {
        ...
        if (item.instrument.instanceId && other.instrument.instanceId)
          aliases.set(item.instrument.instanceId, other.instrument.instanceId);
      }
    });
  }
  const instances = (snapshot: ProjectSnapshot) => [
    ...snapshot.channels.flatMap((c) => [c.instrument, ...(c.effectChain ?? [])]),
    ...snapshot.mixerChannels.flatMap((b) => b.inserts ?? []),
  ];
  ...
  for (const key of ["channels", "mixerChannels"] as const) {
    for (const owner of expected[key]) {
      ...
      if (!other) fail();
      const refs = (item: typeof owner) =>
        "instrument" in item ? [item.instrument, ...(item.effectChain ?? [])] : (item.inserts ?? []);
      const a = refs(owner),
        b = refs(other);
      if (a.length !== b.length) fail();
      a.forEach((ref, i) => {
        const aId = ref.instanceId,
          bId = b[i]!.instanceId;
        if (aId && bId && !oldInstances.has(aId) && !oldInstances.has(bId)) aliases.set(aId, bId);
      });
    }
  }
  const translated = JSON.parse(JSON.stringify(expected), (_key, value: unknown) =>
...
function fail(): never {
  throw new OxitoneError(ErrorCode.EditScopeConflict, "Project transaction changed data outside the requested edit");
}
