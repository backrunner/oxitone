...
export function projectOrderEdit(
  before: ProjectEvaluation,
  owner: string,
  order: readonly string[],
): import("@oxitone/protocol").ProjectEdit | undefined {
  const late = before.configurationSites.some(
    (config) =>
      config.usages.some((u) => u.owner === owner) &&
      before.projectSites.some(
        (project) =>
          project.fileName === config.fileName &&
          project.anchor.start <= config.anchor.start &&
          project.anchor.end >= config.anchor.end,
      ),
  );
  ...
  if (order.length !== effects.length || new Set(order).size !== order.length || permutation.some((i) => i < 0))
    throw new OxitoneError(ErrorCode.EditScopeConflict, "order must contain every current instance once");
...
export function writeEffectOrder(
  text: string,
  site: EvaluatedEffectOwnerSite,
  snapshot: ProjectSnapshot,
  order: readonly string[],
) {
  const done = sourceSpan("effect-order-writing");
  try {
    return effectOrder(text, site, snapshot, order);
  } finally {
    done();
  }
}
...
function effectOrder(
  ...
  if (sourceHash(text) !== anchor.sourceHash || text.slice(anchor.start, anchor.end) !== anchor.expression)
    throw new OxitoneError(ErrorCode.SourceChanged, "effect owner source changed");
  const expected = structuredClone(snapshot);
  const owner =
    expected.channels.find((channel) => channel.id === site.owner) ??
    expected.mixerChannels.find((bus) => bus.id === site.owner);
  ...
  const permutation = order.map((id) => effects.findIndex((effect) => effect.instanceId === id));
  if (order.length !== effects.length || new Set(order).size !== order.length || permutation.some((index) => index < 0))
    throw new OxitoneError(ErrorCode.EditScopeConflict, "order must contain every current instance of this owner once");
  const reordered = permutation.map((index) => effects[index]!);
  if ("instrument" in owner) owner.effectChain = reordered;
  else owner.inserts = reordered;
  ...
  if (
    ...
    throw new OxitoneError(
      ...
      "effect reordering requires a local owner variable in a module or block",
    );
  }
  const block = declaration.parent.parent.parent;
  if (!ts.isSourceFile(block) && !ts.isBlock(block))
    throw new OxitoneError(ErrorCode.EditNotRepresentable, "owner scope cannot hold a local ordering statement");
  const last = block.statements.at(-1),
    terminal = last && ts.isReturnStatement(last) ? last : undefined;
  const previous = terminal ? block.statements.at(-2) : last;
  const at = terminal
    ? terminal.getFullStart()
    : ts.isSourceFile(block)
      ? file.endOfFileToken.getFullStart()
      : block.end - 1;
  ...
  if (
    ...
    ts.isCallExpression(previous.expression) &&
    !hasComments(previous.getText(file))
  ) {
    ...
    if (
      ...
      checker.getSymbolAtLocation(target) === checker.getSymbolAtLocation(declaration.name) &&
      print(call.expression) === print(imported.expression)
    ) {
      try {
        const prior = readLiteral(call.arguments[1]!);
        if (
          Array.isArray(prior) &&
          prior.length === indices.length &&
          new Set(prior).size === prior.length &&
          prior.every((index) => Number.isInteger(index) && index >= 0 && index < prior.length)
        ) {
          indices = permutation.map((index) => prior[index] as number);
          start = previous.getStart(file);
          end = previous.end;
        }
      } catch {
        /* Nonliteral calls retain their normal evaluation. */
      }
    }
  }
  ...
  const indent =
    text
      .slice(text.lastIndexOf("\n", declaration.getStart(file)) + 1, declaration.parent.parent.getStart(file))
      .match(/^\s*/)?.[0] ?? "";
