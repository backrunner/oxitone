...
export function noteEditExtent(source: PatternSourceDocument, operations: readonly NoteEdit[]): number | undefined {
  if (!operations.some(op => "insert" in op || ("set" in op && (op.set.start !== undefined || op.set.duration !== undefined)) ||
    ("shift" in op && (op.shift.start !== undefined || op.shift.duration !== undefined)))) return;
