...
export function writeConfigurationEdit(
  fileName: string,
  text: string,
  ...
  const config = (
    edit.kind === "parameters" ? original.withParameters(edit.values) : original.withHost(edit.values)
  ).toSpec();
  ...
  let base: ts.Expression = ts.factory.createCallExpression(imported.expression, undefined, [
    literal(site.kind),
    expression,
  ]);
  ...
  if (
    ...
    if (
      ...
      print(receiver.expression) === print(imported.expression) &&
      ts.isStringLiteral(receiver.arguments[0]!) &&
      receiver.arguments[0]!.text === site.kind
    ) {
      try {
        ...
        if (
          ...
          Object.entries(previous).every(
            ([key, value]) =>
              (edit.kind === "parameters"
                ? site.config.parameters[key]
                : (site.config as Record<string, unknown>)[key]) === value,
          )
        ) {
        ...
        /* Nonliteral arguments retain their evaluation and comments. */
      }
    }
  }
