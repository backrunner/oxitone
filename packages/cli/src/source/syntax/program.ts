...
export function sourceProgram(fileName: string, text: string): { file: ts.SourceFile; checker: ts.TypeChecker } {
  ...
  if (diagnostic)
    throw new OxitoneError(ErrorCode.DraftInvalid, ts.flattenDiagnosticMessageText(diagnostic.messageText, "\n"), {
...
export function expressionAt(file: ts.SourceFile, start: number, end: number): ts.Expression {
  ...
  visit(file);
  if (!expression || !isValueBoundary(expression))
    throw new OxitoneError(ErrorCode.EditNotRepresentable, "source anchor must cover one complete value expression", {
      details: { path: file.fileName, start, end },
    });
  return expression;
}
...
function isValueBoundary(node: ts.Expression): boolean {
  ...
  if (ts.isVariableDeclaration(parent) || ts.isPropertyAssignment(parent) || ts.isParameter(parent))
