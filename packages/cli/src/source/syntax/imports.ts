...
export interface ImportPatch { readonly at: number; readonly text: string }
export interface PatternImport { readonly expression: ts.Expression; readonly patch?: ImportPatch }
...
export function importPosition(file: ts.SourceFile): number {
  ...
  if (directive) {
    ...
    for (let token = scanner.scan(); ; token = scanner.scan()) {
      if (![ts.SyntaxKind.WhitespaceTrivia, ts.SyntaxKind.SingleLineCommentTrivia, ts.SyntaxKind.MultiLineCommentTrivia, ts.SyntaxKind.NewLineTrivia].includes(token)) break;
...
export function authoringImport(file: ts.SourceFile, checker: ts.TypeChecker, location: ts.Node, exportedName: "Pattern" | "pluginConfig" | "orderEffects"): PatternImport {
  const f = ts.factory;
  const imports = file.statements.filter(ts.isImportDeclaration);
  const sdkImports = imports.filter((statement) => ts.isStringLiteral(statement.moduleSpecifier) &&
  ...
  for (const statement of sdkImports) {
    ...
    if (ts.isNamespaceImport(bindings)) {
      ...
      if (symbol && visible.has(symbol)) return { expression: f.createPropertyAccessExpression(f.createIdentifier(bindings.name.text), exportedName) };
    } else for (const binding of bindings.elements) {
      const symbol = checker.getSymbolAtLocation(binding.name);
      if (!binding.isTypeOnly && (binding.propertyName ?? binding.name).text === exportedName && symbol && visible.has(symbol)) {
        return { expression: f.createIdentifier(binding.name.text) };
      }
    }
  }
  if (file.statements.some(ts.isImportEqualsDeclaration)) throw new OxitoneError(ErrorCode.EditNotRepresentable, "CommonJS import assignment requires a module adapter");
  ...
  for (let suffix = 1; names.has(name); suffix++) name = `Local${exportedName[0]!.toUpperCase()}${exportedName.slice(1)}${suffix}`;
  ...
  return { expression: f.createIdentifier(name), patch: { at,
...
function installedAuthoringModule(fileName: string): string {
  ...
  return ["oxitone", "@oxitone/core"].find(name => ts.resolveModuleName(name, fileName, options, ts.sys).resolvedModule) ?? "oxitone";
}
