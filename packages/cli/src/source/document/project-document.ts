...
import {
  ...
  type MaterializationReview,
  type AutomationRangeEdit,
} from "@oxitone/protocol";
import { projectSourceFiles } from "../files/project-files.js";
import { evaluateSourceProject, type ProjectEvaluation } from "../eval/project-evaluation.js";
...
import { writePatternEdit, type PatternWriteResult } from "../editing/pattern-writer.js";
import { noteEditExtent } from "../editing/pattern-extent.js";
import { materializePatternReference, writeLiteralPatternEdit } from "../editing/materialize.js";
import { checkSourceReads } from "../files/read-set.js";
import { sourceHash } from "../syntax/program.js";
import { SourceSaveStore } from "../save/source-save.js";
import { SourceOwnership } from "../files/ownership.js";
import {
  checkConflict,
  checkProjectText,
  projectModified,
  projectSaveFiles,
  readProjectDisk,
} from "../files/project-disk.js";
import { imageHash } from "../save/save-journal.js";
import { ProjectPluginCatalog } from "../plugins/project-plugins.js";
import { writeAutomationRange } from "../editing/automation-writer.js";
import { writeConfigurationEdit } from "../editing/configuration-writer.js";
...
const textBytes = (files: ReadonlyMap<string, string>) =>
...
export class ProjectDocument {
  ...
  private materialization:
    { review: MaterializationReview; files: Map<string, string>; evaluated: ProjectEvaluation } | undefined;
  private rackMaterialization:
    { review: RackMaterializationReview; files: Map<string, string>; evaluated: ProjectEvaluation } | undefined;
  ...
  private constructor(
    ...
    private readonly store: SourceSaveStore,
  ...
  static async open(options: ProjectDocumentOptions): Promise<ProjectDocument> {
    const entry = resolve(options.entry),
      root = resolve(options.projectRoot ?? dirname(entry));
  ...
  get view(): DocumentView {
    return structuredClone({
      ...
      status: this.status,
      saving: this.saving,
      modified: projectModified(this.files, this.disk),
      ...projectSites(this.accepted, (site) => this.handle(site)),
  ...
  get frame(): PreviewSnapshotFrame | undefined {
    ...
    frame.hash = sourceHash(canonicalEncode(JSON.parse(JSON.stringify(frame))));
    return frame;
  }
  get dependencyPaths(): readonly string[] {
    return [
      ...new Set([
        ...this.files.keys(),
        ...this.disk.keys(),
        ...this.plugins.dependencyPaths,
        ...(this.accepted?.reads ?? []).flatMap((read) => [read.path, read.realPath]),
      ]),
    ];
  }
  ...
  private notify(listener: (view: DocumentView) => void): void {
    try {
      listener(this.view);
    } catch {
      this.listeners.delete(listener);
  ...
  private handle(site: { handle: string }): string {
    return `${this.sessionId}:${this.acceptedRevision}:${site.handle}`;
  }
  private current(revision: number): void {
    if (this.status === "closed" || this.saving || this.revision !== revision) changed();
  }
  private ready(revision: number): ProjectEvaluation {
    this.current(revision);
    if (this.status !== "ready" || !this.accepted || this.acceptedRevision !== revision)
      throw new OxitoneError(ErrorCode.DraftInvalid, "editing requires a valid, conflict-free project");
  ...
  private check(generation: number): void {
    if (this.status === "closed" || generation !== this.generation) changed();
  }
  private record(stack: Map<string, string>[], files: Map<string, string>): void {
    stack.push(new Map(files));
    while (stack.length > 128 || stack.reduce((sum, files) => sum + textBytes(files), 0) > 32 * 1024 * 1024)
      stack.shift();
  }
  private async evaluate(files: ReadonlyMap<string, string>, generation: number): Promise<ProjectEvaluation> {
    ...
    const result = await evaluateSourceProject(
      this.options.entry,
      files,
      this.ownership,
    ...
    await checkSourceReads(result.reads);
    this.check(generation);
    return result;
  }
  private async rebuild(): Promise<DocumentView> {
    ...
    this.emit();
    try {
      this.accepted = await this.evaluate(this.files, generation);
  ...
  async changeCode(revision: number, fileName: string, text: string): Promise<DocumentView> {
    ...
    if (!this.files.has(fileName))
      throw new OxitoneError(ErrorCode.EditNotRepresentable, "file is not part of the source document");
    if (this.files.get(fileName) === text) return this.view;
    const files = new Map(this.files);
    files.set(fileName, text);
    checkProjectText(files);
    this.record(this.history, this.files);
  ...
  async createFile(revision: number, fileName: string, text: string): Promise<DocumentView> {
    ...
    if (this.files.has(fileName))
      throw new OxitoneError(ErrorCode.EditTargetMissing, "source file already exists in the document");
    const files = new Map(this.files);
    files.set(fileName, text);
    checkProjectText(files);
    if (this.disk.has(fileName)) {
      await this.ownership.assertWritable(fileName);
      this.check(generation);
      this.current(revision);
    } else
      await this.ownership.enroll(fileName, () => {
        this.check(generation);
        this.current(revision);
      });
    this.record(this.history, this.files);
  ...
  private async noteCandidate(
    revision: number,
    handle: string,
    operations: readonly NoteEdit[],
    mode: "edit" | "materialize",
    placement?: string,
  ): Promise<DocumentView> {
    ...
    if (placement && (site.scope !== "reference" || site.placements.length !== 1 || site.placements[0] !== placement)) {
      throw new OxitoneError(
        ...
        "this source boundary does not isolate the selected placement",
      );
    }
    ...
    const request = {
      ...
      text: this.files.get(site.fileName)!,
      anchor: site.anchor,
      source: site.source,
      operations,
    ...
    if (
      [...Pattern.fromSource(site.source).notes, ...Pattern.fromSource(candidate.source).notes].some(
        (note) => note.chance !== undefined && note.chance !== 1,
      )
    ) {
      throw new OxitoneError(
        ErrorCode.EditNotRepresentable,
        "probabilistic note editing requires the musical-origin runtime migration",
      );
    }
    if (
      before.frame.snapshot.patternClips.some(
        (clip) =>
          (clip.patternId === site.patternId ||
            before.frame.snapshot.patterns
              .find((p) => p.id === clip.patternId)
              ?.parts?.some((part) => part.patternId === site.patternId)) &&
          (!placement || clip.id === placement) &&
          clip.probability !== undefined &&
          clip.probability !== 1,
      )
    ) {
      throw new OxitoneError(
        ErrorCode.EditNotRepresentable,
        "probabilistic placement editing requires the musical-origin runtime migration",
      );
    }
    const files = new Map(this.files);
          ...
          review: {
            ...
            affectedClips: placement
              ...
              : before.frame.snapshot.patternClips
                  ...
                  .map((clip) => clip.id),
            beforeNotes: detached.summary.beforeNotes,
            afterNotes: detached.summary.afterNotes,
  ...
  private async transact(
    ...
    accept: (candidate: ProjectEvaluation) => void = (candidate) => this.adopt(files, candidate),
    expectedFrame = before.frame,
  ): Promise<DocumentView> {
    const generation = this.begin();
    this.status = "building";
    this.emit();
  ...
  async editAutomationRange(
    ...
    const before = this.ready(revision),
      site = before.automationSites.find((site) => this.handle(site) === handle);
    ...
    if (clipId !== undefined) {
      ...
      if (!clip || !site.lanes.includes(clip.laneId))
        throw new OxitoneError(ErrorCode.EditTargetMissing, "automation clip is not part of this source site");
      const siblings = (before.frame.snapshot.automationClips ?? []).filter(
        (item) => item.laneId === clip.laneId && item.enabled !== false,
      );
      if (siblings.length !== 1)
        throw new OxitoneError(
          ErrorCode.EditScopeConflict,
          "timeline automation editing requires a lane used by exactly one enabled clip",
        );
      const start = Number(clip.startBeat.numerator) / Number(clip.startBeat.denominator);
      const duration =
        clip.durationBeats === undefined
          ? 0
          : Number(clip.durationBeats.numerator) / Number(clip.durationBeats.denominator);
      if (edit.start < start || edit.end > start + duration)
        throw new OxitoneError(ErrorCode.EditScopeConflict, "automation edit falls outside the selected clip");
    ...
    const files = new Map(this.files);
    files.set(site.fileName, candidate.text);
    return this.transact(before, files, (evaluated) =>
      assertProjectAutomationEdit(
        before.frame.snapshot,
        evaluated.frame.snapshot,
        lane ? [lane] : site.lanes,
        candidate.source,
      ),
    );
  }
  ...
  async configure(revision: number, edit: import("@oxitone/protocol").ProjectEdit): Promise<DocumentView> {
    const before = this.ready(revision),
      candidate = writeProjectEdit(before, this.files, edit);
    if (!candidate) return this.view;
    return this.transact(before, candidate.files, (evaluated) =>
      assertArrangement(before.frame.snapshot, candidate.expected, evaluated.frame.snapshot),
    );
  ...
  async assignPlugin(revision: number, input: PluginAssignment): Promise<DocumentView> {
    ...
    await this.plugins.verify(input.plugin, before.frame, this.controller.signal, () => {
      this.check(generation);
      this.ready(revision);
    });
    ...
    return this.transact(
      candidate.before,
      candidate.files,
      (evaluated) => assertArrangement(before.frame.snapshot, candidate.expected, evaluated.frame.snapshot),
      undefined,
      candidate.expectedFrame,
    );
  }
  async confirmMaterialize(revision: number, planId: string): Promise<DocumentView> {
    const before = this.ready(revision),
      pending = this.materialization ?? this.rackMaterialization,
      generation = this.generation;
    if (!pending || pending.review.planId !== planId || pending.review.baseRevision !== revision) changed();
    await checkSourceReads(before.reads);
    await checkSourceReads(pending.evaluated.reads);
    this.check(generation);
    this.ready(revision);
    if (this.materialization !== pending && this.rackMaterialization !== pending) changed();
    this.begin();
    this.adopt(pending.files, pending.evaluated);
    this.emit();
    return this.view;
  }
  async editConfiguration(
    ...
    const before = this.ready(revision),
      site = before.configurationSites.find((site) => this.handle(site) === handle);
  ...
  async editEffectOrder(
    ...
    try {
      const before = this.ready(revision),
        site = before.effectOwnerSites.find((site) => this.handle(site) === handle && site.owner === owner);
      ...
      const files = new Map(this.files);
      files.set(site.fileName, candidate.text);
      return await this.transact(before, files, (evaluated) =>
        assertProjectEquivalent(candidate.expected, evaluated.frame.snapshot),
      );
    } finally {
      done();
    }
  ...
  cancelMaterialize(revision: number, planId: string): DocumentView {
    this.current(revision);
    if (this.materialization?.review.planId === planId) {
      this.materialization = undefined;
      this.emit();
    }
    if (this.rackMaterialization?.review.planId === planId) {
      this.rackMaterialization = undefined;
      this.emit();
    }
    return this.view;
  ...
  async planMaterializeRack(revision: number, handle: string, owner?: string): Promise<DocumentView> {
    const before = this.ready(revision),
      site = before.rackSites.find((site) => this.handle(site) === handle);
    ...
    const text = this.files.get(site.fileName)!,
      candidate = materializeRack(site.fileName, text, site);
    const files = new Map(this.files);
    files.set(site.fileName, candidate.text);
    return this.transact(
      before,
      files,
      (evaluated) => assertProjectEquivalent(before.frame.snapshot, evaluated.frame.snapshot),
      (evaluated) => {
        this.rackMaterialization = {
          files,
          evaluated,
          review: {
            planId: randomUUID(),
  ...
  async undo(revision: number): Promise<DocumentView> {
    this.current(revision);
    const files = this.history.pop();
    if (!files) return this.view;
    this.record(this.future, this.files);
    this.files = files;
    this.revision++;
    return this.rebuild();
  }
  ...
  async save(revision: number): Promise<DocumentView> {
    ...
    try {
      ...
      this.disk = new Map(this.files);
      this.savedRevision = revision;
      for (const file of files) {
        const owned = await this.ownership.assertWritable(file.path);
        accepted.reads = accepted.reads.map((read) =>
          read.realPath === owned.realPath ? { ...read, sha256: imageHash(file.text) } : read,
        );
      }
    } finally {
      this.saving = false;
  ...
  async synchronizeDisk(): Promise<DocumentView> {
    ...
    if (this.status === "building") return this.view;
    const generation = this.generation,
      revision = this.revision;
    const check = () => {
      this.check(generation);
    ...
    const external = await readProjectDisk(this.files, this.disk, check);
    check();
    if (external.conflicts.length) {
      if (JSON.stringify(this.conflicts) !== JSON.stringify(external.conflicts)) {
        this.begin();
        this.revision++;
      }
      this.conflicts = external.conflicts;
      this.status = "conflict";
    ...
    if (!external.changed && this.status !== "invalid" && this.status !== "conflict") {
      try {
        if (this.accepted) await checkSourceReads(this.accepted.reads);
        check();
        try {
          await this.plugins.checkReads();
          check();
        } catch {
          check();
          return this.refreshPlugins(revision);
        }
        return this.view;
      } catch {
        check();
    ...
    this.conflicts = [];
    this.files = external.files;
    this.disk = external.disk;
  ...
  async resolveDiskConflict(
    ...
    fileName: string,
    diskHash: string,
    resolution: "use-disk" | "keep-draft" | "merge",
  ): Promise<DocumentView> {
    this.current(revision);
    const generation = this.generation;
    const conflict = this.conflicts.find((item) => item.path === resolve(fileName));
    await checkConflict(conflict, diskHash);
    this.check(generation);
    this.current(revision);
    const draft = this.files.get(conflict!.path);
    const merged =
      resolution === "merge" && draft !== undefined
        ? mergeThreeWay(conflict!.baseline, draft, conflict!.disk)
        : undefined;
    if (resolution === "merge" && merged === undefined)
      throw new OxitoneError(
        ErrorCode.EditScopeConflict,
        "draft and disk edits overlap; choose one side or edit the conflict manually",
      );
  ...
  async refreshPlugins(revision: number): Promise<DocumentView> {
    ...
    const generation = this.generation;
    try {
      await this.plugins.refresh(this.accepted?.frame, () => {
        this.check(generation);
        this.current(revision);
      });
    } catch (error) {
  ...
  async verifyPlugin(revision: number, handle: string): Promise<DocumentView> {
    ...
    const generation = this.generation;
    try {
      await this.plugins.verify(handle, this.accepted?.frame, this.controller.signal, () => {
        this.check(generation);
        this.current(revision);
      });
    } finally {
  ...
  private async pluginTask(
    ...
    this.current(revision);
    if (!["ready", "invalid"].includes(this.status) || this.saving || this.conflicts.length)
      throw new OxitoneError(
        ErrorCode.PluginTaskConflict,
        "plugin tasks require an idle project without pending Save or conflicts",
      );
    ...
    this.emit();
    try {
      await this.pluginLifecycle.run(task, this.controller.signal);
      this.check(generation);
