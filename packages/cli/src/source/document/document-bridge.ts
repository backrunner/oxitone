...
export async function openDocumentBridge(
  path: string,
  ...
  const server = createServer((socket) => {
    ...
    const send = (message: unknown) => {
      ...
      if (socket.writableLength > 64 * 1024 * 1024) {
        socket.destroy(new Error("editor bridge backpressure limit exceeded"));
        return;
      }
    ...
    socket.on("error", () => {
      /* A failed editor does not terminate the shared document. */
    });
    socket.on("close", () => {
      unsubscribe();
