...
function musicalProject(
  ...
  replacement?: { id: string; source: PatternSourceDocument; placement?: string },
): unknown {
  const result = structuredClone(snapshot);
  result.revision = "0";
  const patterns = new Map(result.patterns.map((pattern) => [pattern.id, pattern]));
  if (replacement && !replacement.placement)
    patterns.set(replacement.id, { ...Pattern.fromSource(replacement.source).toSpec(), id: replacement.id });
  if (replacement && !replacement.placement)
    for (const pattern of patterns.values()) {
      if (pattern.parts?.some((part) => part.patternId === replacement.id)) {
        pattern.lengthBeats = beatToWire(
          Math.max(beatFromWire(pattern.lengthBeats), Pattern.fromSource(replacement.source).lengthBeats),
        );
      }
    }
    ...
    ...(pattern.parts
      ? {
          parts: pattern.parts.map((part) => ({
            ...
            pattern: expand(patterns.get(part.patternId)!),
          })),
        }
      : {}),
  });
  const placed = new Set([
    ...result.patternClips.map((clip) => clip.patternId),
    ...[...patterns.values()].flatMap((pattern) => pattern.parts?.map((part) => part.patternId) ?? []),
  ]);
  const unplacedPatterns = [...patterns]
    .filter(([id]) => !placed.has(id))
    .map(([, pattern]) => {
      const value = JSON.parse(JSON.stringify(expand(pattern)));
  ...
  return JSON.parse(
    JSON.stringify({
      ...
      unplacedPatterns,
      patterns: undefined,
      patternClips: result.patternClips.map((clip) => {
        const pattern =
          replacement?.placement === clip.id
            ? Pattern.fromSource(replacement.source).toSpec()
            : patterns.get(clip.patternId);
...
export function assertProjectNoteEdit(
  ...
  if (
    canonicalEncode(musicalProject(before, { id: patternId, source, ...(placement ? { placement } : {}) })) !==
    canonicalEncode(musicalProject(after))
  ) {
    throw new OxitoneError(ErrorCode.EditScopeConflict, "candidate changed music outside the selected edit scope");
  }
}
export function assertProjectAutomationEdit(
  before: ProjectSnapshot,
  after: ProjectSnapshot,
  ...
  if (canonicalEncode(musicalProject(expected)) !== canonicalEncode(musicalProject(after))) {
    throw new OxitoneError(
      ...
      "automation edit changed data outside the selected source and lanes",
    );
  }
}
...
export function assertProjectConfigurationEdit(
  before: ProjectSnapshot,
  after: ProjectSnapshot,
  usages: ConfigurationSite["usages"],
  config: InstrumentRef | EffectRef,
): void {
  const expected = structuredClone(before);
  for (const usage of usages) {
    const replace = <T extends InstrumentRef | EffectRef>(previous: T): T =>
      ({ ...structuredClone(config), ...(previous.instanceId ? { instanceId: previous.instanceId } : {}) }) as T;
...
export function assertProjectEquivalent(before: ProjectSnapshot, after: ProjectSnapshot): void {
  if (canonicalEncode(musicalProject(before)) !== canonicalEncode(musicalProject(after))) {
    throw new OxitoneError(
      ErrorCode.EditScopeConflict,
      "configuration edit changed another instance, binding, or project setting",
    );
  }
}
