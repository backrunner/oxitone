...
import { instrumentProjectSource, type SourceSite } from "../plugins/project-instrument.js";
...
export async function projectSourceLoader(
  files: ReadonlyMap<string, string>,
  ownership: SourceOwnership,
  key: string,
  sites: SourceSite[],
  reads: Map<string, SourceRead>,
  signal: AbortSignal,
  removed: readonly string[],
): Promise<ProjectSourceLoader> {
  ...
  return {
    async resolveLocal(specifier, directory, diskPath) {
      const target = resolve(directory, specifier),
        suffix = extname(target);
      const extensions = [".tsx", ".ts", ".jsx", ".js", ".css", ".json"];
      const candidates =
        suffix === ".js"
          ? [".ts", ".tsx", ".js", ".jsx"].map((ext) => target.slice(0, -3) + ext)
          : suffix === ".mjs"
            ? [target.slice(0, -4) + ".mts", target]
            : suffix
              ? [target]
    ...
    async read(path) {
      ...
      if (removedPaths.has(path))
        throw new OxitoneError(ErrorCode.EditTargetMissing, "import refers to a source removed in the draft");
      let evidence: SourceRead;
      try {
        [evidence] = (await captureSourceReads([path])) as [SourceRead];
      } catch (error) {
      ...
      if (evidence.sha256 !== null) {
        ...
        if (sourceHash(disk) !== evidence.sha256)
          throw new OxitoneError(ErrorCode.SourceChanged, "source changed during project build");
        if (!file) return disk;
      }
