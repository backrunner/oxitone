...
const excluded = new Set(["node_modules", ".pnpm", ".yarn", ".git", "dist", "target", "build", "native-generated", ".oxitone-source-save"]);
...
export class SourceOwnership {
  ...
  static async open(sourceRoots: readonly string[], files: readonly string[]): Promise<SourceOwnership> {
    if (sourceRoots.length === 0) throw new OxitoneError(ErrorCode.InvalidProject, "at least one source root is required");
    const roots: OwnedSource[] = [];
    for (const input of sourceRoots) {
      const path = resolve(input);
      const realPath = await realpath(path);
      if (segments(path).some((part) => excluded.has(part)) || segments(realPath).some((part) => excluded.has(part))) reject(path, "dependency or generated directory cannot be a source root");
    ...
    for (const input of files) {
      ...
      if ([...ownership.files.values()].some((other) => other.realPath === file.realPath)) reject(file.path, "duplicate aliases of one source file are not editable");
  ...
  private async inspect(input: string): Promise<OwnedSource> {
    const path = resolve(input);
    if (!/\.(?:ts|mts)$/.test(path) || /\.d\.(?:ts|mts)$/.test(path)) reject(path, "only TypeScript ESM authoring files are editable");
    if (segments(path).some((part) => excluded.has(part))) reject(path, "dependency and generated paths are read-only");
    const realPath = await realpath(path);
    if (segments(realPath).some((part) => excluded.has(part))) reject(path, "source resolves into a dependency or generated directory");
    const root = this.roots.filter((candidate) => inside(candidate.path, path) && inside(candidate.realPath, realPath))
    ...
    for (let parent = dirname(realPath); parent !== root.realPath; parent = dirname(parent)) {
      try { await lstat(resolve(parent, "package.json")); reject(path, "nested package is not an enrolled source root"); }
  ...
  async enroll(input: string, assertCurrent: () => void = () => {}): Promise<OwnedSource> {
    ...
    assertCurrent();
    if ([...this.files.values()].some(file => file.path !== owned.path && file.realPath === owned.realPath)) reject(input, "duplicate source aliases are not editable");
    this.files.set(owned.path, Object.freeze(owned)); this.created.add(owned.path);
    return { ...owned };
  }
  ...
  async assertCreatable(input: string): Promise<OwnedSource> {
    ...
    if (this.files.has(owned.path) && !this.created.has(owned.path)) reject(input, "source already belongs to this document");
    try { await lstat(owned.path); reject(input, "source file already exists"); }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    return owned;
  }

  /** Recovery validates existing sources against the enrolled roots before discovery reads text. */
  async assertRecoverySource(input: string): Promise<OwnedSource> { return this.inspect(input); }
  ...
  async assertCreationTarget(input: string): Promise<OwnedSource> {
    ...
    try {
      ...
      if (!info.isFile() || info.nlink !== 1) reject(owned.path, "created source must be an ordinary file without links");
    } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    return owned;
  }
  ...
  async assertCreationParent(input: string): Promise<OwnedSource> {
    ...
    if (!/\.(?:ts|mts)$/.test(path) || /\.d\.(?:ts|mts)$/.test(path)) reject(path, "only TypeScript ESM authoring files are editable");
    if (segments(path).some(part => excluded.has(part))) reject(path, "dependency and generated paths are read-only");
    const root = this.roots.filter(candidate => inside(candidate.path, path)).sort((a, b) => b.path.length - a.path.length)[0];
    if (!root || await realpath(root.path) !== root.realPath) reject(path, "source is outside the enrolled project roots");
    const parent = resolve(dirname(path));
    const parentReal = await realpath(parent);
    if (segments(parentReal).some(part => excluded.has(part)) || (parentReal !== root.realPath && !inside(root.realPath, parentReal))) reject(path, "new source parent is outside the enrolled project root");
    for (let directory = parentReal; directory !== root.realPath; directory = dirname(directory)) {
      try { await lstat(join(directory, "package.json")); reject(path, "nested package is not an enrolled source root"); }
      catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
    }
