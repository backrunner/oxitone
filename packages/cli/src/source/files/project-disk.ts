...
import { imageHash, type SourceSaveFile } from "../save/save-journal.js";
...
export function projectSaveFiles(
  files: ReadonlyMap<string, string>,
  disk: ReadonlyMap<string, string>,
): SourceSaveFile[] {
  return [...new Set([...files.keys(), ...disk.keys()])]
    .filter((path) => files.get(path) !== disk.get(path))
...
export function checkProjectText(files: ReadonlyMap<string, string>): void {
  ...
  if (
    files.size > 4096 ||
    sizes.some((size) => size > 8 * 1024 * 1024) ||
...
export async function readProjectDisk(
  files: ReadonlyMap<string, string>,
  disk: ReadonlyMap<string, string>,
  check: () => void,
) {
  const nextFiles = new Map(files),
    nextDisk = new Map(disk);
  ...
  for (const path of new Set([...disk.keys(), ...files.keys()])) {
    ...
    let text: string;
    try {
      text = await readSourceText(path);
    } catch (error) {
      if (baseline === undefined && (error as NodeJS.ErrnoException).code === "ENOENT") {
        check();
        continue;
      }
      throw error;
    ...
    if (total > SOURCE_PROJECT_LIMIT)
      throw new OxitoneError(ErrorCode.BudgetExceeded, "project disk text exceeds 32 MiB");
    check();
    if (text === baseline) continue;
    if (files.get(path) !== baseline && files.get(path) !== text)
      conflicts.push({ path, baseline: baseline ?? "", disk: text, diskHash: sourceHash(text) });
    else {
      nextFiles.set(path, text);
      nextDisk.set(path, text);
      changed = true;
    }
  }
...
export async function checkConflict(
  conflict: DocumentView["conflicts"][number] | undefined,
  diskHash: string,
): Promise<void> {
