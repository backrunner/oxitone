...
export async function runEvaluationProcess(
  ...
  signal: AbortSignal,
  timeoutMs: number,
  workerName: "project-worker" | "plugin-worker",
): Promise<unknown> {
  ...
  return new Promise((accept, reject) => {
    ...
    const timer = setTimeout(
      () => stop(new OxitoneError(ErrorCode.BudgetExceeded, "source execution timed out")),
      timeoutMs,
    );
    ...
    child.stdio[3]?.on("data", (chunk: Buffer) => {
      bytes += chunk.length;
      if (bytes > MAX_RESULT)
        stop(new OxitoneError(ErrorCode.BudgetExceeded, "source evaluation result exceeds 64 MiB"));
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        reject(
          readEvaluationFailure(Buffer.concat(chunks)) ??
            new OxitoneError(ErrorCode.DraftInvalid, diagnostics || `source execution exited ${code}`),
        );
        return;
      }
      try {
