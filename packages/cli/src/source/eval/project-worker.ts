...
try {
  ...
  Object.defineProperty(globalThis, key, {
    configurable: true,
    value(handle: string, value: unknown) {
  ...
  try {
    ...
    if (!project || typeof project.snapshot !== "function")
      throw new Error("DAW entry must export a Project or a factory returning one");
    ...
    const automationSites = [...captures]
      .filter(([, capture]) => capture.count === 1)
      .flatMap(([handle, capture]) => {
        const matching = lanes.filter((lane) => capture.values.has(lane.source));
        return matching.length
          ? [{ handle, lanes: matching.map((lane) => lane.id), source: matching[0]!.source.toSpec() }]
          : [];
      });
    const laneCaptures = lanes.flatMap((lane) =>
      [...captures]
        .filter(([, capture]) => capture.count === 1 && capture.values.has(lane))
        .map(([handle]) => ({ handle, laneId: lane.id })),
    );
    for (const pattern of patterns)
      for (const [handle, capture] of captures) {
        ...
        if (pattern.parts.length === 0 && capture.values.has(pattern))
          sites.push({ handle, patternId: pattern.id, invocations: capture.count, source: pattern.toSource() });
      }
    const arrangementOrder = {
      patterns: patterns.map((p) => p.id),
      tracks: tracks.map((t) => t.id),
      channels: project.channels.map((c: { id: string }) => c.id),
      mixerChannels: project.mixerChannels.map((b: { id: string }) => b.id),
      samples: project.samples.map((s: { id: string }) => s.id),
      automation: lanes.map((l) => l.id),
      patternClips: tracks.flatMap((t) => t.clips).map((c) => c.id),
      sampleClips: project.sampleClips.map((c: { id: string }) => c.id),
      automationClips: project.automationClips.map((c: { id: string }) => c.id),
    };
    const projectCaptures = [...captures]
      .filter(([, c]) => c.count === 1 && c.values.has(project))
      .map(([handle]) => handle);
    const result = JSON.stringify({
      ...
      arrangementOrder,
      projectCaptures,
      ...configurationCaptures(project, captures),
      reads: [...tracking.reads.values()],
