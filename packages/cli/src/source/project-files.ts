...
export async function readSourceText(path: string, limit = SOURCE_FILE_LIMIT): Promise<string> {
  ...
  if (!metadata.isFile() || metadata.size > limit) throw new OxitoneError(ErrorCode.BudgetExceeded, "source file exceeds 8 MiB");
...
export async function projectSourceFiles(roots: readonly string[]): Promise<{ ownership: SourceOwnership; files: Map<string, string> }> {
  ...
  const visit = async (directory: string, root: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      else if ((entry.isFile() || entry.isSymbolicLink()) && /(?<!\.d)\.(?:ts|mts)$/.test(entry.name)) candidates.push(path);
      if (candidates.length > 4096) throw new OxitoneError(ErrorCode.BudgetExceeded, "project exceeds 4096 source files");
    }
  };
  for (const input of roots) { const root = resolve(input); await visit(root, root); }
  const paths: string[] = [], realPaths = new Set<string>();
  for (const path of new Set(candidates)) {
    try {
      await SourceOwnership.open(roots, [path]);
      const actual = await realpath(path);
      if (!realPaths.has(actual)) { paths.push(path); realPaths.add(actual); }
    } catch (error) { if (!OxitoneError.isOxitoneError(error) || error.code !== ErrorCode.EditNotRepresentable) throw error; }
  }
  ...
  for (const path of paths) {
    ...
    if (total > SOURCE_PROJECT_LIMIT) throw new OxitoneError(ErrorCode.BudgetExceeded, "project source text exceeds 32 MiB");
    files.set(path, text);
  }
