...
async function readImage(path: string): Promise<string | null> {
  try {
    return await readSourceText(path);
  } catch (error) {
...
export async function prepareImages(
  files: readonly SourceSaveFile[],
  ownership: SourceOwnership,
): Promise<SaveImage[]> {
  if (!files.length || files.length > 64)
    throw new OxitoneError(ErrorCode.BudgetExceeded, "source save requires 1..64 files");
  ...
  for (const file of files) {
    ...
    if (images.some((image) => image.realPath === owned.realPath))
      saveConflict("duplicate source file in save transaction");
    images.push({
      ...owned,
      before,
      after: file.text,
      beforeHash: file.baselineHash,
      afterHash: imageHash(file.text),
...
function checkBudget(images: readonly SaveImage[]): void {
  if (
    images.reduce(
      (bytes, image) => bytes + Buffer.byteLength(image.before ?? "") + Buffer.byteLength(image.after ?? ""),
      0,
    ) > MAX_BYTES
  ) {
...
export async function clearJournal(directory: string): Promise<void> {
  await rm(journalPath(directory));
...
export async function readJournal(directory: string): Promise<SaveJournal | undefined> {
  ...
  try {
    ...
    if (!info.isFile() || info.nlink !== 1 || info.size > MAX_BYTES * 6 + 65536)
      saveConflict("invalid source save journal file");
  } catch (error) {
  ...
  if (
    ...
    saveConflict("unsupported source save journal");
  for (const image of value.files) {
    const isText = (text: unknown) => typeof text === "string" || text === null;
    if (
      !image ||
      typeof image.path !== "string" ||
      typeof image.realPath !== "string" ||
      !isText(image.before) ||
      !isText(image.after) ||
      (image.before === null && image.after === null) ||
      (image.stagingId !== undefined &&
        (typeof image.stagingId !== "string" || !/^[a-f0-9-]{36}$/.test(image.stagingId))) ||
      ((image.before === null || image.after === null) && image.stagingId === undefined) ||
      !Number.isInteger(image.mode) ||
      image.mode < 0 ||
      image.mode > 0o777 ||
      imageHash(image.before) !== image.beforeHash ||
      imageHash(image.after) !== image.afterHash
    )
      saveConflict("source recovery image is corrupt");
  }
  if (new Set(value.files.map((image) => image.realPath)).size !== value.files.length)
    saveConflict("duplicate recovery file");
...
export async function checkImage(image: SaveImage, ownership: SourceOwnership): Promise<string | null> {
  const owned =
    image.before === null || image.after === null
      ? await ownership.assertCreationTarget(image.path)
      : await ownership.assertRecoverySource(image.path);
  ...
  if (hash !== image.beforeHash && hash !== image.afterHash)
    saveConflict("source recovery conflicts with external edits: " + image.path);
  return hash;
}

export async function publishImage(image: SaveImage, text: string | null, currentHash: string | null): Promise<void> {
  if (text === null) {
    await rm(image.realPath);
    await syncDirectory(dirname(image.realPath));
...
export async function recoverJournal(directory: string, ownership: SourceOwnership): Promise<boolean> {
  ...
  for (const image of journal.files) {
    if ((await checkImage(image, ownership)) !== (journal.phase === "committed" ? image.afterHash : image.beforeHash))
      saveConflict("source changed during recovery");
  }
