    ...
    await withSaveLock(this.directory, async () => {
      ...
      try {
        ...
        for (const image of images) {
          await this.checkRoot();
          if ((await checkImage(image, this.ownership)) !== image.beforeHash)
            saveConflict("source changed before publication");
          request.assertCurrent?.();
        ...
        for (const image of images)
          if ((await checkImage(image, this.ownership)) !== image.afterHash)
            saveConflict("source changed after publication");
