...
export function saveConflict(message: string): never { throw new OxitoneError(ErrorCode.SourceChanged, message); }
...
export async function durableCreate(path: string, text: string, mode: number, temporary: string): Promise<void> {
  ...
  try {
    ...
    catch (error) { if ((error as NodeJS.ErrnoException).code === "EEXIST") saveConflict("new source appeared during Save"); throw error; }
  } finally { if (created) { await rm(temporary, { force: true }); await syncDirectory(dirname(path)); } }
}
...
export async function withSaveLock<T>(directory: string, action: () => Promise<T>): Promise<T> {
  ...
  try {
    try {
      ...
      let pid: number;
      try { pid = (JSON.parse(await readFile(join(lock, "owner.json"), "utf8")) as { pid: number }).pid; }
      catch { saveConflict("save lock owner is unknown; preserve recovery files for inspection"); }
      if (!Number.isSafeInteger(pid) || pid <= 0) saveConflict("invalid save lock owner");
      try { process.kill(pid, 0); saveConflict("another document service holds the project save lock"); }
      catch (error) { if ((error as NodeJS.ErrnoException).code !== "ESRCH") throw error; }
      // The claim stays inside the old lock so a second opener cannot steal a recovery in progress.
      try { await mkdir(join(lock, "recovering")); } catch { saveConflict("source recovery is already claimed; preserve the journal"); }
      acquired = true;
      await durableReplace(join(lock, "owner.json"), JSON.stringify({ pid: process.pid }));
    }
    return await action();
  } finally { if (acquired) { await rm(lock, { recursive: true, force: true }); await syncDirectory(directory); } }
}
