...
export function captureSpecifier(key: string): string {
  return `oxitone-capture:${key}`;
}
...
export function projectCaptureModule(key: string): Plugin {
  return {
    name: "oxitone-capture-module",
    setup(api) {
      api.onResolve({ filter: /^oxitone-capture:/ }, (args) =>
        args.path === captureSpecifier(key) ? { path: key, namespace: "oxitone-capture" } : undefined,
      );
      api.onLoad({ filter: /.*/, namespace: "oxitone-capture" }, () => ({
        loader: "js",
        contents: `export const capture = globalThis[${JSON.stringify(key)}];`,
      }));
    },
  };
}
