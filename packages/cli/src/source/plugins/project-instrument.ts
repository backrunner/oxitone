...
import { sourceHash, sourceProgram } from "../syntax/program.js";
import type { ExpressionAnchor } from "../editing/pattern-writer.js";
...
export function instrumentProjectSource(
  fileName: string,
  text: string,
  key: string,
  offset: number,
): { text: string; sites: SourceSite[] } {
  ...
  const visit = (node: ts.Node): void => {
    ...
    let label = "reference",
      scope: SourceSite["scope"] = "reference";
    if (ts.isVariableDeclaration(node) && node.initializer && ts.isIdentifier(node.name)) {
      ...
      expression = node.initializer;
      label = node.name.getText(file).slice(0, 120);
      ...
      for (const item of node.elements)
        if (!ts.isSpreadElement(item) && !ts.isOmittedExpression(item)) add(item, "array item", "reference");
    } else if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
      // Arguments are isolated local use sites, even when the factory implementation lives in npm.
      for (const arg of node.arguments ?? [])
        if (!ts.isSpreadElement(arg)) add(arg, node.expression.getText(file).slice(0, 120), "reference");
    }
  ...
  const add = (expression: ts.Expression, label: string, scope: SourceSite["scope"]) => {
    ...
    if (offset + sites.length >= 4096)
      throw new OxitoneError(ErrorCode.BudgetExceeded, "project exceeds 4096 source boundaries");
    const handle = String(offset + sites.length);
    sites.push({
      handle,
      fileName,
      anchor: { sourceHash: hash, start, end, expression: text.slice(start, end) },
      label,
      scope,
    });
    patches.push(
      { at: start, text: `${imported.binding}(${JSON.stringify(handle)},(`, close: false },
      { at: end, text: "))", close: true },
    );
  };
  visit(file);
