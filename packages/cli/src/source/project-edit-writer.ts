...
import { arrangementEditSchema, canonicalEncode, ErrorCode, OxitoneError, projectEditSchema, type ArrangementEdit, type ProjectEdit, type RegisterPluginOptions } from "@oxitone/protocol";
...
export function restoreProject(before: ProjectEvaluation): Project {
  ...
  for (const key of ["patterns", "tracks", "channels", "mixerChannels", "samples", "automation", "patternClips", "sampleClips", "automationClips"] as const) {
    snapshot[key]?.sort((a, b) => before.arrangementOrder[key].indexOf(a.id) - before.arrangementOrder[key].indexOf(b.id));
  }
...
export function appendProjectEdit(before: ProjectEvaluation, files: ReadonlyMap<string, string>, method: "arrange" | "configure", edit: unknown, registration?: RegisterPluginOptions): Map<string, string> {
  ...
  if (!site || rank(site.label) > 1) throw new OxitoneError(ErrorCode.EditNotRepresentable, "Editing requires a captured Project return or default export");
  const text = files.get(site.fileName)!;
  if (sourceHash(text) !== site.anchor.sourceHash) throw new OxitoneError(ErrorCode.SourceChanged, "Project source changed");
  ...
  if (!registration && ts.isCallExpression(expression) && ts.isPropertyAccessExpression(expression.expression)
    && expression.expression.name.text === method && expression.arguments.length === 1
    && text.slice(expression.expression.expression.end, expression.end) === `.${method}(${expression.arguments[0]!.getText()})`) {
    // JSON only: don't remove comments, evaluation or computed values from authored calls.
    try {
      const previous = method === "configure" ? projectEditSchema.parse(JSON.parse(expression.arguments[0]!.getText())) : arrangementEditSchema.parse(JSON.parse(expression.arguments[0]!.getText()));
      ...
      if (scalarTarget(previous) !== undefined && scalarTarget(previous) === scalarTarget(next)) {
        let merged = "values" in previous && "values" in next
          ? { ...next, values: { ...previous.values, ...next.values } }
          : previous.kind === "track" && next.kind === "track"
            ? { ...next, enabled: next.enabled ?? previous.enabled, mute: next.mute ?? previous.mute, solo: next.solo ?? previous.solo }
            : next;
        ...
        while (ts.isParenthesizedExpression(receiver) && receiver.getText() === `(${receiver.expression.getText()})`) receiver = receiver.expression;
        base = receiver.getText();
        edit = merged;
      }
    } catch { /* Preserve unrecognized authored expressions. */ }
  }
  if (registration) {
...
function scalarTarget(edit: ProjectEdit | ArrangementEdit): string | undefined {
  if ("action" in edit) return ["resize", "fitSample", "enable"].includes(edit.action) && "clip" in edit ? `${edit.kind}/${edit.action}/${edit.resource}/${edit.clip}` : undefined;
