  ...
  async start(): Promise<void> {
    ...
    this.unsubscribe = this.document.subscribe((view) => {
      if (view.acceptedRevision !== this.revision && this.document?.frame) {
        this.revision = view.acceptedRevision;
        this.send(this.document.frame);
      }
      this.send({
        protocolVersion: "1.0",
        type: "document",
        message: { documentProtocolVersion: "2.0", type: "event", view },
      });
    });
    if (this.options.watch !== false)
      this.stopWatching = watchProjectDocument(this.document, this.entry, (error) =>
        this.send({ protocolVersion: "1.0", type: "diagnostic", code: "SourceChanged", message: String(error) }),
      );
  ...
  receive(requests: readonly DocumentRequest[]): void {
    ...
    for (const request of requests)
      void Promise.resolve()
        .then(() => (this.closed ? undefined : this.dispatcher?.submit(request)))
        .then((message) => {
          if (!this.closed && message) this.send({ protocolVersion: "1.0", type: "document", message });
        })
        .catch((error) => {
          if (!this.closed)
            this.send({ protocolVersion: "1.0", type: "diagnostic", code: "DraftInvalid", message: String(error) });
        });
  }
  rejectRevision(): void {
    this.revision = -1;
