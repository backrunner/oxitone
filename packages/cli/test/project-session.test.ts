...
const source =
  "import { Project, chord } from '@oxitone/core'; const phrase = chord(60, 'major'); const project = new Project(); project.addTrack('Lead').pattern(phrase).at({ bar: 1 }); export default project;\n";
...
async function open(entry: string) {
  ...
  cleanup.push(async () => document.close());
...
it("captures async local factories with their original this, module URL, exports and call count", async () => {
  const text =
    "import { Project } from '@oxitone/core'; import { make } from './phrase.js'; export default async function () { await Promise.resolve(); const project = new Project(); project.addTrack('Lead').pattern(make()).at({ bar: 1 }); return project; }";
  const { root, entry } = await fixture(text),
    path = join(root, "phrase.ts");
  await writeFile(
    path,
    "import { chord } from '@oxitone/core'; const factory = { pitch: 60, make() { return chord(this.pitch, 'major'); } }; let calls = 0; export function make() { if (++calls !== 1) throw Error('ran twice'); if (!import.meta.url.endsWith('/phrase.ts')) throw Error('asset base lost'); const selected = factory.make(); return selected; }",
  );
...
it("rejects a repeatedly executed boundary and contains failing subscribers", async () => {
  const { entry } = await fixture(
    "import { Project, chord } from '@oxitone/core'; function make() { const selected = chord(60, 'major'); return selected; } const project = new Project(); const track = project.addTrack('Lead'); for (let bar = 1; bar < 3; bar++) track.pattern(make()).at({ bar }); export default project;",
  );
  const document = await open(entry);
  document.subscribe(() => {
  ...
  await expect(
    document.edit(0, "unavailable-repeated-boundary", [{ select: { degree: 1 }, set: { pitch: 70 } }]),
  ).rejects.toMatchObject({ code: "EditTargetMissing" });
...
it("supersedes pending source and DAW evaluations without publishing them after close", async () => {
  ...
  const rejected = expect(slow).rejects.toMatchObject({ code: "SourceChanged" });
  await entered();
  await document.changeCode(1, entry, source.replace("chord(60", "chord(72"));
  await rejected;
  ...
  const gated =
    source +
    `if (phrase.notes[0].pitch === 61) { (await import('node:fs')).writeFileSync(${JSON.stringify(marker)}, 'entered'); await new Promise(resolve => setTimeout(resolve, 5000)); }`;
  ...
  for (let count = 0; ; count++) {
    try {
      ...
      if (count > 250 || (error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    await new Promise((resolve) => setTimeout(resolve, 20));
  }
  await document.changeCode(3, entry, source.replace("chord(60", "chord(74"));
  await cancelled;
  ...
  const closed = expect(closing).rejects.toMatchObject({ code: "SourceChanged" });
  document.close();
  await closed;
...
it("terminates runaway project workers and honors cancellation", async () => {
  const { root } = await fixture(),
    bundle = join(root, "runaway.mjs");
  await writeFile(bundle, "export default () => { while (true) {} };");
  await expect(
    runEvaluationProcess(bundle, "timeout", root, new AbortController().signal, 1000, "project-worker"),
  ...
  const rejected = expect(pending).rejects.toMatchObject({ code: "SourceChanged" });
  controller.abort();
  await rejected;
});
...
it("watches atomic saves, resolves dirty conflicts and rebuilds changed imported source", async () => {
  ...
  const errors: unknown[] = [],
    stop = watchProjectDocument(document, entry, (error) => errors.push(error));
  cleanup.push(async () => stop());
  const external = text + "// atomic save\n";
  await writeFile(entry + ".tmp", external);
  await rename(entry + ".tmp", entry);
  await until(
    () =>
      document.view.status === "ready" && document.view.files.find((file) => file.path === entry)?.text === external,
  );
