...
it("keeps a same-line directive prologue while inserting the required runtime import", () => {
  ...
  const result = materializePatternReference({
    fileName: "/tmp/song.ts",
    text,
    anchor: anchorPatternExpression("/tmp/song.ts", text, start, start + expression.length),
...
it("captures and writes a project whose author shadows globalThis without changing its bindings", async () => {
  ...
  try {
    ...
    const text =
      "import { Project, chord } from '@oxitone/core'; const globalThis = { pitch: 60 }; const phrase = chord(globalThis.pitch, 'major'); const project = new Project(); project.addTrack('Lead').add(phrase).at({ bar: 1 }); export default project;";
    ...
    document = await ProjectDocument.open({ entry });
    expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
    await document.edit(0, document.view.sites.find((site) => site.label === "phrase")!.handle, [
      { select: { degree: 2 }, set: { pitch: 66 } },
    ]);
    await document.save(1);
...
it("materializes a helper-only local module using the SDK package actually installed by the project", async () => {
  ...
  try {
    ...
    const local = join(root, "phrase.ts"),
      entry = join(root, "song.ts");
    await writeFile(local, "import { make } from 'arrangement-fixture'; export const phrase = make();");
    const source =
      "import { Project } from '@oxitone/core'; import { phrase } from './phrase.js'; const project = new Project(); project.addTrack('Lead').add(phrase).at({ bar: 1 }); export default project;";
    ...
    await document.confirmMaterialize(0, document.view.materialization!.planId);
    await document.save(1);
