...
it("discovers unused multi-plugin packages without executing JS, validates only the selected native library, and exposes failures", async () => {
  ...
  try {
    const pkg = join(root, "node_modules/catalog-fixture");
    await mkdir(pkg, { recursive: true });
    await writeFile(join(root, "package.json"), '{"dependencies":{"catalog-fixture":"1.2.3"}}');
    await writeFile(
      join(pkg, "package.json"),
      '{"name":"catalog-fixture","version":"1.2.3","type":"module","exports":"./index.js","oxitone":{"plugins":"plugins.json"}}',
    );
      ...
      JSON.stringify({
        ...
        plugins: [
          plugin,
          {
            ...
            manifest: { ...manifest, pluginId: "fixture.other" },
            platforms: {},
          },
          {
            ...plugin,
            displayName: "Missing",
            manifest: { ...manifest, pluginId: "fixture.missing" },
            platforms: { [platform]: { library: "missing.dylib", sha256 } },
          },
        ],
      }),
    );
    const discovery = await discoverProjectPlugins(root);
    expect(discovery.plugins.map((plugin) => plugin.entry.availability)).toEqual([
      "available",
      "unsupported",
      "missing",
    ]);
    const catalog = new ProjectPluginCatalog(root);
    const frame: PreviewSnapshotFrame = {
      protocolVersion: "1.0",
      type: "snapshot",
      hash: "0".repeat(64),
    ...
    expect(catalog.view().find((plugin) => plugin.handle === gain.handle)).toMatchObject({
      validation: "verified",
      sha256,
    });
    ...
    await expect(catalog.verify(gain.handle, frame, new AbortController().signal, () => {})).rejects.toMatchObject({
      code: "PluginManifestMismatch",
    });
    expect(catalog.view().find((plugin) => plugin.handle === gain.handle)!.validation).toBe("failed");
    await writeFile(
      join(pkg, "package.json"),
      '{"name":"catalog-fixture","version":"1.2.3","oxitone":{"plugins":"../../package.json"}}',
    );
...
it("runs plugin install, repair and uninstall as validated no-shell package tasks", async () => {
  ...
  try {
    ...
    expect(calls).toEqual([
      ["add", "--save-exact", "@acme/synth@2.1.0"],
      ["update", "--save-exact", "@acme/synth@2.2.0"],
      ["install", "--force", "--filter", "@acme/synth"],
      ["remove", "@acme/synth"],
    ]);
    await expect(lifecycle.run({ kind: "install", packageName: "--evil" }, signal)).rejects.toMatchObject({
      code: "PluginManifestMismatch",
    });
    await expect(
      lifecycle.run({ kind: "upgrade", packageName: "@acme/synth", version: "../../escape" }, signal),
    ).rejects.toMatchObject({ code: "PluginManifestMismatch" });
    await expect(
      lifecycle.run({ kind: "install", packageName: "@acme/synth", version: "../../escape" }, signal),
    ).rejects.toMatchObject({ code: "PluginManifestMismatch" });
  } finally {
    await rm(root, { recursive: true, force: true });
  }
...
it("restores package manifests when a lifecycle task fails after partial mutation", async () => {
  ...
  try {
    ...
    const lifecycle = new PluginLifecycle(root, {
      async run(_args) {
    ...
    await expect(
      lifecycle.run({ kind: "upgrade", packageName: "@acme/synth", version: "9.9.9" }, new AbortController().signal),
