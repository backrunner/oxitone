...
describe("project source ownership", () => {
  ...
  it("rechecks symbolic links and rejects hard links to other content", async () => {
    ...
    try {
      ...
      await writeFile(original, "export default 1;");
      await writeFile(outside, "export default 2;");
      ...
      const internal = join(dependency, "notes.ts");
      await writeFile(internal, "");
      const disguised = join(root, "notes.ts");
      await symlink(internal, disguised);
  ...
  it("requires separate enrollment of nested workspace package sources", async () => {
    ...
    try {
      const pkg = join(root, "packages/arrangements");
      await mkdir(pkg, { recursive: true });
      await writeFile(join(pkg, "package.json"), '{"name":"arrangements"}');
      const source = join(pkg, "index.ts");
      await writeFile(source, "export const phrase = 1;");
  ...
  it("enforces the aggregate source budget while reading files sequentially", async () => {
    ...
    try {
      for (let index = 0; index < 5; index++)
        await writeFile(join(root, `part-${index}.ts`), Buffer.alloc(7 * 1024 * 1024, 0x78));
      await expect(projectSourceFiles([root])).rejects.toMatchObject({ code: "BudgetExceeded" });
    } finally {
      await rm(root, { recursive: true, force: true });
    }
