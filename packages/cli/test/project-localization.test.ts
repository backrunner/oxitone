...
it("localizes an npm chord/arp factory at one placement, preserves evaluation and imports, and rejects dependency drift after review", async () => {
  ...
  try {
    ...
    const pkg = join(root, "node_modules/arrangement-fixture");
    await mkdir(pkg);
    const dependency = join(pkg, "index.js"),
      manifest = join(pkg, "package.json");
    const factory =
      "import { chord, arp } from '@oxitone/core'; export let calls = 0; export function phrase() { calls++; return arp(chord(60, 'major'), 'upDown', 0.25).repeat(2); }";
    ...
    await document.edit(
      0,
      site.handle,
      [{ select: { iteration: 1, note: { step: 2 } }, set: { start: 20, duration: 0.5 } }],
      clip,
    );
    ...
    const initialSite = document.view.sites.find(
      (site) => site.expression === "chorus()" && site.placements[0] === clip,
    )!;
    await document.planMaterialize(
      initialRevision,
      initialSite.handle,
      [{ select: { iteration: 1, note: { step: 2 } }, remove: true }],
      clip,
    );
    const stale = document.view.materialization!;
    expect(stale.afterText).toContain("Pattern as LocalPattern1");
    expect(stale.retainsOriginalEvaluation).toBe(true);
    expect(stale.afterNotes).toBe(7);
    expect(await readFile(entry, "utf8")).toBe(source);
    await writeFile(dependency, factory + "\n");
    await expect(document.confirmMaterialize(initialRevision, stale.planId)).rejects.toMatchObject({
      code: "SourceChanged",
    });
    ...
    await expect(document.confirmMaterialize(document.view.revision, plan.planId)).rejects.toMatchObject({
      code: "SourceChanged",
    });
    await document.planMaterialize(
      document.view.revision,
...
it("keeps shared exports while localizing one reference, then runs the detached notes without the removed dependency", async () => {
  ...
  try {
    ...
    await mkdir(pkg);
    const factory =
      "import { chord, arp } from '@oxitone/core'; export function makePhrase() { return arp(chord(60, 'major'), 'upDown', 0.25).repeat(2); }";
    ...
    const site = document.view.sites.find(
      (site) => site.scope === "reference" && site.expression === "phrase" && site.placements.length === 1,
    )!;
    await document.planMaterialize(
      0,
      site.handle,
      [{ select: { iteration: 1, note: { step: 1 } }, remove: true }],
      site.placements[0],
    );
    const review = document.view.materialization!;
    expect(review.retainsOriginalEvaluation).toBe(false);
    expect(review.afterNotes).toBe(7);
    expect(review.afterText).toContain("Pattern as LocalPattern1");
    expect(review.afterText).toContain("export const shared = phrase;");
    await document.confirmMaterialize(0, review.planId);
    ...
    const independent = review.afterText
      .replace("import { makePhrase as chorus } from 'arrangement-fixture';\n", "")
