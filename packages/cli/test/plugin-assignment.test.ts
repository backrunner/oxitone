...
it("registers an unused npm effect in local TS, preserves package bytes, follows instance selection and reopens", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-assign-package-")); let document: ProjectDocument | undefined;
  try {
    ...
    const pkg = join(root, "store/package"); await mkdir(pkg, { recursive: true });
    ...
    const compile = async (source: string, binary: string) => {
      const input = binary + ".c"; await writeFile(input, source);
      await promisify(execFile)("cc", ["-std=c11", "-shared", "-fPIC", "-O2", "-I", join(repo, "include"), input, "-o", binary]);
    ...
    const binary = join(pkg, library), sha256 = await compile(original, binary);
    await writeFile(join(root, "package.json"), '{"dependencies":{"gain-pack":"1.0.0"}}');
    const packageJson = '{"name":"gain-pack","version":"1.0.0","type":"module","exports":"./index.js","oxitone":{"plugins":"plugins.json"}}';
    ...
    const metadata = JSON.stringify({ formatVersion: 1, plugins: [{ displayName: "Gain", vendor: "Fixture", manifest,
    ...
    await writeFile(entry, `import { Project, effect, createAutomationNamespace } from '@oxitone/core';
...
`);
    ...
    await document.editConfiguration(document.view.revision, source.handle, { kind: "parameters", values: { gain: .7 } }, use);
    expect(effects()[1]!.parameters.gain).toBe(.7);
    const binding = document.frame!.snapshot.automation[0]!.target.entityId;
    const snapshot = document.frame!.snapshot, text = document.view.files[0]!.text;
    ...
    await expect(document.assignPlugin(document.view.revision, { plugin: plugin.handle, owner, target: "channelInsert", instance: binding })).rejects.toThrow();
    ...
    expect(createHash("sha256").update(await readFile(binary)).digest("hex")).toBe(sha256);
