...
const view = (text: string, revision: number, sessionId = "session-a"): DocumentView => ({ sessionId, projectRoot: "/tmp", revision, acceptedRevision: revision, savedRevision: 0,
  status: "ready", saving: false, modified: text !== "A", sites: [], automationSites: [], configurationSites: [], rackSites: [], plugins: [], conflicts: [], files: [{ path: fileName, text }] });
...
it("finishes an already proposed remote edit before applying a newer DAW revision", () => {
  ...
  buffers.reconcile(view("A", 0)); buffers.reconcile(view("B", 1));
  ...
  buffers.reconcile(view("C", 2));
  expect(buffers.values.get(fileName)).toMatchObject({ text: "B", baselineText: "B", apply: { text: "C", expectedVersion: 2 } });
...
it("retains synchronized but unsaved editor text when a restarted service loads older disk contents", () => {
  const buffers = new EditorBuffers();
  buffers.open({ fileName, text: "A", baselineText: "A", version: 1 });
  buffers.reconcile(view("A", 0)); buffers.change(fileName, "B", 2);
...
it("honors a correlated code acknowledgement even when a later DAW event arrives in the same socket batch", async () => {
  ...
  const event = (text: string, revision: number) => encodeFrame({ documentProtocolVersion: "2.0", type: "event", view: view(text, revision) });
  const server = createServer(socket => {
    clients.add(socket); socket.on("close", () => clients.delete(socket));
    const decoder = new FrameDecoder(); socket.write(event("A", 0));
    socket.on("data", data => {
      for (const raw of decoder.push(data as Buffer)) {
        const request = raw as DocumentRequest;
        socket.write(Buffer.concat([event("B", 1), encodeFrame({ documentProtocolVersion: "2.0", type: "response", sessionId: "session-a",
          requestId: request.requestId, accepted: true, revision: 1 }), event("C", 2)]));
      }
    });
  });
  ...
  try {
    ...
    editor.close(); for (const client of clients) client.destroy();
    await new Promise<void>(accept => server.close(() => accept())); await rm(root, { recursive: true, force: true });
  }
});
...
it("serializes semantic requests and retains typing until the in-flight command completes", async () => {
  ...
  const clients = new Set<Socket>(), requests: DocumentRequest[] = [];
  ...
  const server = createServer(socket => {
    ...
    socket.on("data", data => {
      for (const raw of decoder.push(data as Buffer)) {
        const request = raw as DocumentRequest; requests.push(request);
        const answer = () => socket.write(Buffer.concat([event(request.operation.kind === "code" ? request.operation.text : "A", requests.length),
          encodeFrame({ documentProtocolVersion: "2.0", type: "response", sessionId: "session-a", requestId: request.requestId, accepted: true, revision: requests.length })]));
        if (request.operation.kind === "createFile") release = answer;
        else answer();
      }
    });
  });
  const socket = join(root, "socket"); await new Promise<void>(accept => server.listen(socket, accept));
  const editor = new EditorDocumentSession(socket);
  try {
    editor.openBuffer({ fileName, text: "A", baselineText: "A", version: 1 });
    await until(() => editor.view.state === "connected");
    ...
    await expect(editor.request({ kind: "installPlugin", packageName: "synth" })).rejects.toMatchObject({ code: "SourceChanged" });
    expect(requests.map(request => request.operation.kind)).toEqual(["createFile"]);
    release!(); expect((await pending).accepted).toBe(true); await editor.flush();
