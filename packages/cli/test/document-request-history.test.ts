...
it("keeps a long-lived client writable after thousands of requests without replaying expired commands", async () => {
  ...
  const request = (sequence: number, kind = "query") => ({
    documentProtocolVersion: "2.0",
    sessionId: "session",
    requestId: `stream/editor-a/${sequence}`,
    baseRevision: 0,
    operation: { kind },
  });
  expect(await dispatcher.submit(request(1, "save"))).toMatchObject({ accepted: true });
  for (let sequence = 2; sequence <= 5000; sequence++)
    expect(await dispatcher.submit(request(sequence))).toMatchObject({ accepted: true });
  expect(await dispatcher.submit(request(1, "save"))).toMatchObject({
    accepted: false,
...
it("bounds client streams and rejects unsupported or reused command identities", async () => {
  ...
  const dispatcher = new DocumentDispatcher(document);
  const request = (requestId: string, kind = "query") => ({
    documentProtocolVersion: "2.0",
    sessionId: "session",
    requestId,
    baseRevision: 0,
  ...
  expect(await dispatcher.submit(request("old-request", "save"))).toMatchObject({
    accepted: false,
    error: { code: "SourceChanged" },
  });
  expect(await dispatcher.submit(request("stream/editor/1", "save"))).toMatchObject({ accepted: true });
  expect(await dispatcher.submit(request("stream/editor/1"))).toMatchObject({
    accepted: false,
    error: { code: "SourceChanged" },
  });
  ...
  for (let client = 1; client < 64; client++)
    expect(await dispatcher.submit(request(`stream/client-${client}/1`))).toMatchObject({ accepted: true });
  expect(await dispatcher.submit(request("stream/overflow/1"))).toMatchObject({
    accepted: false,
