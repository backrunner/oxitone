...
async function project(entryText?: string) {
  const root = await mkdtemp(join(tmpdir(), "oxitone-full-document-")); cleanup.push(() => rm(root, { force: true, recursive: true }));
...
it("automatically maps real project Patterns across modules, edits shared definitions and saves all dirty TS", async () => {
  ...
  expect(site.references).toBe(2); expect(site.outputs).toHaveLength(3);
  ...
  try { expect(reopened.frame?.snapshot.patterns[0]?.notes[1]?.pitch).toBe(65); expect(reopened.frame?.snapshot.name).toBe("Edited"); }
  finally { reopened.close(); }
  await document.undo(2); await document.undo(3);
...
it("rejects a use-site edit that affects only one shared placement when the requested scope is the definition", async () => {
  ...
  await expect(document.edit(0, site.handle, [{ select: { degree: 1 }, set: { pitch: 61 } }])).rejects.toMatchObject({ code: "EditScopeConflict" });
  expect(document.view.files).toEqual(before); expect(document.view.revision).toBe(0);
});
it("rejects changed non-note project fields and keeps invalid code with the last accepted graph", async () => {
  const { document, entry } = await project("import { Project, chord } from '@oxitone/core'; const phrase = chord(60, 'major'); const project = new Project({ name: String(phrase.notes[0].pitch) }); project.addTrack('Lead').pattern(phrase).at({ bar: 1 }); export default project;");
  ...
  expect(document.view.status).toBe("invalid"); expect(document.frame).toEqual(frame);
  await expect(document.save(1)).rejects.toMatchObject({ code: "DraftInvalid" });
  await document.undo(1); expect(document.view.status).toBe("ready");
});
...
it("edits only the selected placement and reviews materialization before any buffer or disk changes", async () => {
  ...
  expect(snapshot.patterns.find(pattern => pattern.id === snapshot.patternClips.find(clip => clip.id === placement)!.patternId)!.notes[0]!.pitch).toBe(61);
  expect(snapshot.patterns.find(pattern => pattern.id === snapshot.patternClips.find(clip => clip.id !== placement)!.patternId)!.notes[0]!.pitch).toBe(60);
  ...
  expect(review.afterText).toContain("new Pattern("); expect(document.view.files).toEqual(before);
  expect(await readFile(entry, "utf8")).toBe(review.beforeText);
  await document.confirmMaterialize(2, review.planId); await document.save(3);
  ...
  const reopened = await ProjectDocument.open({ entry });
  try { expect(reopened.frame!.snapshot.patterns.map(pattern => pattern.notes.length).sort()).toEqual([2, 3]); }
  finally { reopened.close(); }
});
...
it("keeps conflicts across code changes and Undo, rejects stale disk resolution and resolves the exact displayed disk", async () => {
  ...
  const external = original.replace("'Song'", "'Disk'"); await writeFile(entry, external);
  ...
  expect(document.view.status).toBe("conflict");
  await document.undo(document.view.revision); expect(document.view.status).toBe("conflict");
  await expect(document.save(document.view.revision)).rejects.toMatchObject({ code: "DraftInvalid" });
  await writeFile(entry, external + "\n");
  await expect(document.resolveDiskConflict(document.view.revision, entry, conflict.diskHash, "keep-draft")).rejects.toMatchObject({ code: "SourceChanged" });
  await document.synchronizeDisk();
  await document.resolveDiskConflict(document.view.revision, entry, document.view.conflicts[0]!.diskHash, "keep-draft");
  expect(document.view.status).toBe("ready"); expect(document.frame!.snapshot.name).toBe("Draft");
  await document.save(document.view.revision); expect(await readFile(entry, "utf8")).toContain("'Draft'");
});
...
it("automatically merges disjoint draft and disk edits and rejects overlapping ones", async () => {
  ...
  await document.synchronizeDisk();
  await expect(document.resolveDiskConflict(document.view.revision, entry, document.view.conflicts[0]!.diskHash, "merge"))
...
it("creates a new owned TypeScript source file through the journal and reopens it", async () => {
  ...
  try { expect(reopened.view.files.some(file => file.path === created)).toBe(true); }
  finally { reopened.close(); }
});

it("rejects oversized code before mutating the draft, history or revision", async () => {
  const { document, entry } = await project(); const before = document.view;
  await expect(document.changeCode(0, entry, "x".repeat(8 * 1024 * 1024 + 1))).rejects.toMatchObject({ code: "BudgetExceeded" });
  expect(document.view).toEqual(before); await document.undo(0); expect(document.view).toEqual(before);
});
