...
it("grows only changed timing, leaves pre-existing tails and deletion alone", () => {
  ...
  expect(noteEditExtent(source, [{ select, set: { velocity: .9, start: 0, duration: 9 } }])).toBeUndefined();
  expect(noteEditExtent(source, [{ select, set: { start: 8, duration: .25 } }])).toBe(9);
  expect(noteEditExtent(source, [{ select, remove: true }])).toBeUndefined();
  expect(noteEditExtent(source, [{ insert: { start: 12, pitch: 67, duration: 1, velocity: .7 } }])).toBe(13);
});
...
it("coalesces literal edit options and retains the extended length through subsequent edits", () => {
  ...
  let current = writePatternEdit({ text, fileName: "song.ts", source: chord(60, "major").toSource(),
    anchor: anchorPatternExpression("song.ts", text, text.indexOf(expression), text.length - 1),
    operations: [{ select: { degree: 1 }, set: { start: 8 } }], lengthBeats: 12 });
  for (const lengthBeats of [16, 20, 24]) current = writePatternEdit({ ...current, fileName: "song.ts", lengthBeats,
    operations: [{ select: { degree: 1 }, set: { start: lengthBeats - 1 } }] });
  current = writePatternEdit({ ...current, fileName: "song.ts", operations: [{ select: { degree: 1 }, set: { velocity: .2 } }] });
