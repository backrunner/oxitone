...
it("edits independent Pattern parts and moves reusable automation through Save, Undo and reopen", async () => {
  ...
  try {
    ...
    expect(document.frame!.snapshot.patterns.find(p => p.notes.some(n => n.pitch === 36))!.notes[0]!.pitch).toBe(36);
    ...
    await document.edit(document.view.revision, lead.handle, [{ select: { degree: 1 }, set: { start: 12, duration: 1 } }]);
    expect(document.frame!.snapshot.patterns.find(p => p.parts)!.lengthBeats.numerator).toBe(13);
    expect(document.frame!.snapshot.patterns.find(p => p.notes.some(n => n.pitch === 36))!.lengthBeats.numerator).toBe(4);
    ...
    await document.arrange(document.view.revision, { action: "place", kind: "automation", resource: 0, track: 0, startBeat: 2, durationBeats: 4 });
    await document.arrange(document.view.revision, { action: "place", kind: "automation", resource: 0, track: 1, startBeat: 8, durationBeats: 4 });
    await document.arrange(document.view.revision, { action: "move", kind: "automation", resource: 0, clip: 0, track: 1, startBeat: 5 });
