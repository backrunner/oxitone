...
it("maps repeated shared addEffect inputs to returned instances, preserves bindings after reorder, and saves ID-free code", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-instance-document-"));
...
`;
    ...
    const before = document.frame!.snapshot,
      target = before.automation[0]!.target.entityId;
    expect(before.channels[0]!.effectChain[1]!.instanceId).toBe(target);
    const local = document.view.configurationSites.find(
      (site) =>
        site.scope === "reference" &&
        site.expression === "shared" &&
        site.usages.length === 1 &&
        site.usages[0]!.handle === target,
    )!;
    expect(local).toBeDefined();
    await document.editConfiguration(0, local.handle, { kind: "parameters", values: { feedback: 0.6 } }, target);
    expect(document.frame!.snapshot.channels[0]!.effectChain[1]!.parameters.feedback).toBe(0.6);
    ...
    document = await ProjectDocument.open({ entry });
    expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
    expect(document.frame!.snapshot.channels[0]!.effectChain[1]!).toMatchObject({
      instanceId: target,
