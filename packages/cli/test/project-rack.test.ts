...
it("reviews and detaches a real npm C-plugin serial rack, keeps bindings and side effects, then edits one local effect and reopens", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-rack-document-")); let document: ProjectDocument | undefined;
  try {
    ...
    const repo = fileURLToPath(new URL("../../../", import.meta.url)), fixtures = join(repo, "crates/render/tests/fixtures");
...
export let calls = 0; export function rack() { calls++; return [
  ...
  { pluginId: 'fixture.gain', pluginVersion: '1.0.0', parameters: { gain: .4 }, bypass: true }
    ...
    const before = document.frame!.snapshot, first = before.channels[0]!.id;
    const site = () => document!.view.rackSites.find(site => site.scope === "reference" && site.usages.length === 1 && site.usages[0]!.owner === first)!;
    ...
    expect(document.view.revision).toBe(0); expect(document.view.files[0]!.text).toBe(source);
    ...
    const local = () => document!.view.configurationSites.find(site => site.scope === "reference" && site.usages.length === 1 && site.usages[0]!.owner === first && site.usages[0]!.index === 1)!;
    for (const gain of [.5, .6, .9]) await document.editConfiguration(document.view.revision, local().handle, { kind: "parameters", values: { gain } }, local().usages[0]!.handle);
    expect(document.frame!.snapshot.channels[0]!.effectChain[1]!.parameters.gain).toBe(.9);
    ...
    await expect(document.editConfiguration(revision, local().handle, { kind: "parameters", values: { gain: 3 } })).rejects.toBeDefined();
    expect(document.view.revision).toBe(revision);
    await document.save(revision); document.close(); document = await ProjectDocument.open({ entry });
    expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
    expect(document.frame!.snapshot.channels[0]!.effectChain[1]!.parameters.gain).toBe(.9);
    expect(document.frame!.snapshot.automation).toEqual(before.automation);
    expect(await readFile(join(pkg, "index.js"), "utf8")).toBe(factory);
    expect(createHash("sha256").update(await readFile(binary)).digest("hex")).toBe(binaryHash);
