...
export async function until(check: () => boolean | Promise<boolean>, timeout = 30_000): Promise<void> {
  ...
  while (!(await check())) {
    if (Date.now() > end) throw new Error("Preview condition timed out");
    await delay(50);
  }
}
...
export class Client {
  ...
  constructor(readonly socket: Socket) {
    ...
    socket.on("data", (chunk: Buffer) => {
      try {
        for (const value of decoder.push(chunk)) this.replies.shift()?.resolve(value as Record<string, unknown>);
      } catch (error) {
        socket.destroy(error as Error);
      }
    });
    socket.on("error", (error) => {
      for (const reply of this.replies.splice(0)) reply.reject(error);
    });
    socket.on("close", () => {
      for (const reply of this.replies.splice(0)) reply.reject(new Error("Preview disconnected"));
    });
  }
  request(frame: unknown): Promise<Record<string, unknown>> {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => this.socket.destroy(new Error(`Preview response timed out: ${JSON.stringify(frame).slice(0, 120)}`)),
        30_000,
      );
      this.replies.push({
        resolve: (value) => {
  ...
  query(): Promise<Record<string, unknown>> {
    return this.request({ protocolVersion: "1.0", type: "query" });
  }
