...
it("writes shared or single-lane source ranges, reduces repeated drawing and reopens the actual saved project", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-automation-document-"));
  ...
  try {
    ...
    const definition = document.view.automationSites.find(
    ...
    let site = document.view.automationSites.find(
      (site) => site.scope === "reference" && site.label === "first.automate",
    )!;
    const lane = site.lanes[0]!;
    const other = document.frame!.snapshot.automation.find((item) => item.id !== lane)!;
    for (let i = 0; i < 4; i++) {
      await document.editAutomationRange(
        document.view.revision,
        site.handle,
        { start: 2, end: 3, points: [{ beat: 0, value: 0.2 + i / 10 }] },
        lane,
      );
      site = document.view.automationSites.find(
        (site) => site.scope === "reference" && site.label === "first.automate",
      )!;
      expect(document.frame!.snapshot.automation.find((item) => item.id !== lane)).toEqual(other);
    }
    const shaped = documentRequestSchema.parse({
      documentProtocolVersion: "2.0",
      sessionId: document.view.sessionId,
      ...
      operation: {
        ...
        edit: {
          ...
          points: [
            ...
            { beat: 0.5, value: 0.8, curve: { kind: "step" } },
            { beat: 1, value: 0.5 },
          ],
        },
      },
    ...
    expect(shapedSource).toMatchObject({
      kind: "replaceRange",
      replacement: {
        kind: "curve",
        points: [
          { value: 0.2, curve: { kind: "bezier", out: [1 / 3, 0.6], in: [2 / 3, 0.9] } },
          { value: 0.8, curve: { kind: "step" } },
          { value: 0.5 },
        ],
      },
    });
    ...
    expect(document.frame!.snapshot.automation[0]!.source).toMatchObject({
      kind: "replaceRange",
      replacement: { points: [{ value: 0.5 }] },
    });
    ...
    expect(text.match(/replaceRange\(/g)).toHaveLength(1);
    expect(text).toContain("a.sine({ periodBeats: 4 })");
...
it("edits a unique playlist automation clip in clip-local time and rejects shared clips", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-automation-clip-document-"));
  let document: ProjectDocument | undefined;
  try {
    await mkdir(join(root, "node_modules/@oxitone"), { recursive: true });
    await symlink(
