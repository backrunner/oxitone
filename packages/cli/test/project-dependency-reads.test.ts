...
const workers = [
  { name: "source", Document: ProjectDocument },
  { name: "built", Document: BuiltProjectDocument },
];
async function fixture(Document: typeof ProjectDocument | typeof BuiltProjectDocument) {
  ...
  const pkg = join(root, "node_modules/preset");
  await mkdir(pkg);
  await writeFile(
    join(pkg, "package.json"),
    '{"name":"preset","version":"1.0.0","type":"module","exports":"./index.ts"}',
  );
  const entry = join(root, "song.ts"),
    marker = join(root, "calls");
  await writeFile(
    entry,
    "import { Project } from '@oxitone/core'; import { rack } from 'preset'; const project = new Project(); const channel = project.addChannel({ effectChain: rack() }); export default project;",
  );
  await writeFile(
    join(pkg, "index.ts"),
    `import { effect } from '@oxitone/core'; import { appendFileSync } from 'node:fs'; import { Setting } from './setting.js';
let calls = 0;
export function rack() { if (++calls !== 1) throw Error('factory executed twice'); appendFileSync(${JSON.stringify(marker)}, 'x'); return [effect('delay', { feedback: new Setting().feedback }), effect('delay')]; }`,
  ...
  const document = await Document.open({ entry });
  cleanup.push(async () => document.close());
  return { root, entry, marker, document, setting };
}

it.each(workers)(
  "$name worker executes npm TypeScript factories afresh and observes dependency byte changes",
  async ({ Document }) => {
    ...
    const reorder = () => {
      ...
      return document.editEffectOrder(
        view.revision,
        site.handle,
        channel.id,
    ...
    const original = await readFile(setting, "utf8"),
      previous = await stat(setting);
    await writeFile(setting, original.replace(".2", ".3"));
    await utimes(setting, previous.atime, previous.mtime);
    await expect(reorder()).rejects.toMatchObject({ code: "SourceChanged" });
    expect(await readFile(marker, "utf8")).toBe("xxx");
    await document.synchronizeDisk();
    expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
    expect(document.frame!.snapshot.channels[0]!.effectChain[0]!.parameters.feedback).toBe(0.3);
    expect(await readFile(marker, "utf8")).toBe("xxxx");
  },
);

it.each(workers)(
  "$name worker rejects a new package lock introduced after its directory was scanned",
  async ({ Document }) => {
