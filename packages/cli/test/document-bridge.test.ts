...
it("synchronizes unsaved editor buffers over the real socket and deduplicates a graphical transaction independently of events", async () => {
  ...
  try {
    ...
    socket.on("data", (chunk: Buffer) => {
      messages.push(...decoder.push(chunk).map((value) => documentMessageSchema.parse(value)));
    });
    ...
    const request = {
      ...
      requestId: "stream/editor/1",
      baseRevision: 0,
      operation: { kind: "code", fileName: entry, text: source.replace("chord(60", "chord(62") },
    };
    socket.write(encodeFrame(request));
    await until(() =>
      messages.some((message) => message.type === "response" && message.requestId === request.requestId),
    );
    expect(document.frame?.snapshot.patterns[0]?.notes[0]?.pitch).toBe(62);
    expect(await readFile(entry, "utf8")).toBe(source);
    const views = messages.filter(
      (message): message is Extract<DocumentMessage, { type: "event" }> => message.type === "event",
    );
    ...
    const first = dispatcher.submit(edit),
      duplicate = dispatcher.submit(edit);
    ...
    const save = await dispatcher.submit({
      ...request,
      requestId: "stream/editor/2",
      baseRevision: 2,
      operation: { kind: "save" },
    });
    expect(save.accepted).toBe(true);
    expect(await readFile(entry, "utf8")).toContain(".edit(");
