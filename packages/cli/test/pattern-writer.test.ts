...
function request(text: string, expression: string, pattern: Pattern, operations: NoteEdit[]) {
  ...
  return { text, fileName: "song.ts", source: pattern.toSource(), operations,
...
describe("Pattern source writer", () => {
  it("wraps exactly the selected expression and preserves surrounding bytes", () => {
    ...
    const result = writePatternEdit(request(text, expression, arp(chord(60, "major"), "upDown", 0.25), [
  ...
  it("rejects stale anchors, partial expressions, declaration names and invalid drafts", () => {
    ...
    expect(() => writePatternEdit({ ...initial, text: `// formatted\n${initial.text}` })).toThrowError(expect.objectContaining({ code: ErrorCode.SourceChanged }));
    expect(() => anchorPatternExpression("song.ts", initial.text, 0, 5)).toThrowError(expect.objectContaining({ code: ErrorCode.EditNotRepresentable }));
    expect(() => anchorPatternExpression("song.ts", initial.text, 6, 7)).toThrowError(expect.objectContaining({ code: ErrorCode.EditNotRepresentable }));
    expect(() => anchorPatternExpression("song.ts", "const p = chord(;", 10, 15)).toThrowError(expect.objectContaining({ code: ErrorCode.DraftInvalid }));
  });

  it("updates one literal edit list across repeated drags", () => {
    let current = writePatternEdit(request("export default arp([60,64], 'up', 1);", "arp([60,64], 'up', 1)", arp([60, 64], "up", 1), [{ select: { step: 1 }, set: { pitch: 65 } }]));
    for (let i = 0; i < 20; i++) current = writePatternEdit({ ...current, fileName: "song.ts", operations: [{ select: { step: 1 }, set: { pitch: 66 + i % 2 } }] });
  ...
  it("preserves comments inside an existing list without endlessly wrapping later drags", () => {
    ...
    let current = writePatternEdit(request(`export default ${expression};`, expression,
      arp([60, 64], "up", 1).edit([{ select: { step: 1 }, set: { pitch: 65 } }]), [{ select: { step: 0 }, set: { pitch: 61 } }]));
    for (let i = 0; i < 3; i++) current = writePatternEdit({ ...current, fileName: "song.ts", operations: [{ select: { step: 0 }, set: { pitch: 62 + i } }] });
  ...
  it("writes a shared reference locally and rebuilds the saved TS in a fresh module", async () => {
    ...
    try {
      ...
      const operations: NoteEdit[] = [{ select: { iteration: 1, note: { step: 2 } }, set: { pitch: 70 } }, { select: { iteration: 2, note: { step: 1 } }, remove: true }];
      const result = writePatternEdit({ fileName: entry, text, anchor: anchorPatternExpression(entry, text, start, start + 6), source: base.toSource(), operations });
