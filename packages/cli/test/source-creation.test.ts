...
it.each(["ts", "mts"])(
  "imports and re-exports unsaved .%s modules and edits their notes before Save",
  async (suffix) => {
    const { root, entry, document } = await fixture();
    const phrase = join(root, `phrase.${suffix}`),
      barrel = join(root, `barrel.${suffix}`),
      js = suffix === "ts" ? "js" : "mjs";
    await document.createFile(
      document.view.revision,
      `phrase.${suffix}`,
      "import { chord } from '@oxitone/core'; export const phrase = chord(60, 'major');\n",
    );
    await document.createFile(document.view.revision, barrel, `export { phrase as melody } from './phrase.${js}';\n`);
    await document.changeCode(
      document.view.revision,
      entry,
      `import { Project } from '@oxitone/core'; import { melody } from './barrel.${js}';
const project = new Project({ name: 'Draft modules' }); project.addTrack('Lead').pattern(melody).at({ bar: 1 }); export default project;`,
    ...
    const site = document.view.sites.find(
      (site) => site.fileName === phrase && site.label === "phrase" && site.scope === "definition",
    )!;
    ...
    try {
      expect(reopened.view.status).toBe("ready");
      expect(reopened.frame!.snapshot.patterns).toEqual(document.frame!.snapshot.patterns);
    } finally {
      reopened.close();
    }
  },
...
it("persists Undo/Redo of new empty files, including deletion after Save and recreation", async () => {
  ...
  await document.save(4);
  await expect(stat(created)).rejects.toMatchObject({ code: "ENOENT" });
  expect(document.view.modified).toBe(false);
  await document.redo(4);
  await document.save(5);
  expect(await readFile(created, "utf8")).toBe("");
});
...
it("uses the same .js-to-TypeScript resolution before and after saving a module beside existing JavaScript", async () => {
  ...
  await document.changeCode(
    1,
    entry,
    "import { Project } from '@oxitone/core'; import { name } from './phrase.js'; export default new Project({ name });",
  );
  expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
  ...
  const reopened = await ProjectDocument.open({ entry });
  try {
    expect(reopened.frame!.snapshot.name).toBe("TypeScript");
  } finally {
    reopened.close();
  }
});
...
it("restores JavaScript fallback when a saved TypeScript shadow is undone", async () => {
  ...
  await document.createFile(1, "phrase.ts", "export const name = 'TypeScript';");
  await document.save(2);
  ...
  const reopened = await ProjectDocument.open({ entry });
  try {
    expect(reopened.frame!.snapshot.name).toBe("JavaScript");
  } finally {
    reopened.close();
  }
});
...
it("preserves extensionless directory index precedence across Save", async () => {
  ...
  await document.changeCode(
    1,
    entry,
    "import { Project } from '@oxitone/core'; import { name } from './phrases'; export default new Project({ name });",
  );
  expect(document.view.status, document.view.diagnostic?.message).toBe("ready");
  expect(document.frame!.snapshot.name).toBe("TypeScript");
  await document.save(2);
...
it("rejects existing files and stale/oversized creation without changing the revision or ownership", async () => {
  ...
  await expect(document.createFile(0, created, "x".repeat(8 * 1024 * 1024 + 1))).rejects.toMatchObject({
    code: "BudgetExceeded",
  });
  ...
  await expect(document.createFile(1, join(root, "nested/new.ts"), "")).rejects.toMatchObject({
    code: "EditNotRepresentable",
  });
  await rm(created);
  await document.createFile(1, created, "");
  expect(document.view.revision).toBe(2);
});
...
it("keeps an externally created empty file intact and exposes its conflict with a new draft", async () => {
  ...
  await document.resolveDiskConflict(
    document.view.revision,
    created,
    document.view.conflicts[0]!.diskHash,
    "keep-draft",
  );
  await document.save(document.view.revision);
  expect(await readFile(created, "utf8")).toContain("n = 1");
});

it.each(["prepared", "committed"] as const)(
  "recovers %s creation with a fresh owner, including interrupted hard-link publication",
  async (phase) => {
    const { root, entry, created, directory } = await fixture();
    const ownership = await SourceOwnership.open([root], [entry]);
    await ownership.enroll(created);
    ...
    if (phase === "prepared") await expect(stat(created)).rejects.toMatchObject({ code: "ENOENT" });
      ...
      expect(await readFile(created, "utf8")).toBe("");
      expect((await stat(created)).nlink).toBe(1);
    }
  },
);
...
it("preserves existing empty files in recovery and loads the recovered preimage", async () => {
  ...
  const images = await prepareImages(
    [
      { path: created, baselineHash: sourceHash(""), text: "// filled" },
      { path: entry, baselineHash: sourceHash(text), text: text.replace("Original", "Partial") },
    ],
    ownership,
  );
  ...
  try {
    expect(await readFile(created, "utf8")).toBe("");
    expect(reopened.frame!.snapshot.name).toBe("Original");
    expect(reopened.view.modified).toBe(false);
  } finally {
    reopened.close();
  }
});
...
it("recovers a real killed creation before discovering and executing project buffers", async () => {
  ...
  const files = [
    { path: created, baselineHash: null, text: "// new" },
    { path: entry, baselineHash: sourceHash(text), text: text.replace("Original", "Partial") },
  ];
  const worker = join(root, "crash.mjs");
  await writeFile(
    worker,
    `import { SourceOwnership, SourceSaveStore } from ${JSON.stringify(new URL("../src/source/index.ts", import.meta.url).href)};
...
await store.save({ files, reads: [], assertCurrent() { if (++calls === 4) process.kill(process.pid, 'SIGKILL'); } });`,
  ...
  await expect(
    promisify(execFile)(process.execPath, ["--import", createRequire(import.meta.url).resolve("tsx"), worker]),
  ).rejects.toMatchObject({ signal: "SIGKILL" });
  expect(await readFile(created, "utf8")).toBe("// new");
  const reopened = await ProjectDocument.open({ entry });
  try {
    expect(reopened.frame!.snapshot.name).toBe("Original");
    expect(reopened.view.files.some((file) => file.path === created)).toBe(false);
  } finally {
    reopened.close();
  }
  await expect(stat(created)).rejects.toMatchObject({ code: "ENOENT" });
