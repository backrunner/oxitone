...
it("writes and reduces local npm chain permutations, keeps typed bindings and other uses, and rejects stale/legacy targets", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-effect-order-"));
  ...
  try {
    ...
    await writeFile(
      ...
      '{"name":"preset","version":"1.0.0","type":"module","exports":"./index.js"}',
    );
    const factory =
      "import { effect } from '@oxitone/core'; export function rack() { return [effect('delay', { feedback: .2 }), effect('delay', { feedback: .6 })]; }";
...
`;
    ...
    const before = document.frame!.snapshot,
      owner = before.channels[0]!.id;
    const order = before.channels[0]!.effectChain.map((ref) => ref.instanceId!);
    const site = () =>
      document!.view.effectOwnerSites!.find((site) => site.owner === owner && site.scope === "definition")!;
    ...
    expect(saved.match(/ordered\(/g)).toHaveLength(1);
    expect(saved).not.toMatch(/instanceId|ins_[a-f0-9]|__oxitone/);
    expect(saved).toContain("export { first }");
    await document.save(3);
    document.close();
    document = await ProjectDocument.open({ entry });
    expect(document.frame!.snapshot.channels[0]!.effectChain.map((ref) => ref.instanceId)).toEqual(
      [...order].reverse(),
    );
    expect(document.frame!.snapshot.automation).toEqual(before.automation);
    ...
    const legacy = code.replace(
      "first.effectInstances[0].param('feedback').automate(",
      "first.automate('insert.0.parameter.feedback', ",
    );
    await document.changeCode(0, entry, legacy);
    expect(document.view.status).toBe("ready");
    await expect(document.editEffectOrder(1, site().handle, owner, [...order].reverse())).rejects.toMatchObject({
