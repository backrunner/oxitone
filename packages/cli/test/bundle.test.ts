...
it("bundles nested TS/JSON and literal dynamic imports into one executable file with original asset URLs", async () => {
  ...
  const dir = await mkdtemp(join(root, "bundle-")), child = join(dir, "parts"), out = join(dir, "output", "song.mjs");
  ...
  try {
    ...
    await writeFile(join(child, "data.ts"), `import { readFileSync } from 'node:fs';
    ...
    await writeFile(entry, `import { Project } from '@oxitone/core';
    ...
    await rm(entry); await rm(join(child, "data.ts")); await rm(join(child, "metadata.json"));
    runner = new PreviewRunner(out, f => frames.push(f), { watch: false, debounceMs: 0 });
    await runner.start(); await until(() => frames.some(f => f.type === "snapshot"));
    const snapshot = frames.find(f => f.type === "snapshot")!;
    expect(snapshot.snapshot.name).toBe("Bundled assets"); expect(snapshot.assetBaseDir).toBe(dir);
...
it("retains the previous bundle after a broken build and rejects source overwrite", async () => {
  const root = fileURLToPath(new URL("../node_modules/.cache/", import.meta.url));
  await mkdir(root, { recursive: true }); const dir = await mkdtemp(join(root, "bundle-failed-"));
  try {
    const entry = join(dir, "song.mjs"), out = join(dir, "compiled.mjs");
    await writeFile(entry, "export const project = 42;"); await bundleProject(entry, out);
