...
it("saves coalesced mixer/tempo/track edits, copy/resize/delete and plugin assignments across Undo and reopen", async () => {
  ...
  try {
    ...
    await writeFile(entry, `import { Project, chord } from '@oxitone/core';
...
`);
    ...
    const config = async (values: { level?: number; pan?: number }) => document!.configure(document!.view.revision, { kind: "channel", index: 0, values });
    await config({ level: .5 }); await config({ level: .7 }); await config({ pan: -.3 });
    expect(lead()).toMatchObject({ level: .7, pan: -.3 });
    expect(document.frame!.snapshot.channels.find(c => c.name === "Bass")!.level).toBe(1);
    const draft = document.view.files[0]!.text, revision = document.view.revision;
    expect(draft.match(/\.configure\(/g)).toHaveLength(1);
    await config({ pan: -.3 });
    expect(document.view.revision).toBe(revision); expect(document.view.files[0]!.text).toBe(draft);
    ...
    expect(lead().mute).not.toBe(true); expect(lead().solo).not.toBe(true);
    await document.arrange(document.view.revision, { action: "duplicate", kind: "pattern", resource: 0, clip: 0, track: 0, startBeat: 4 });
    expect(document.frame!.snapshot.patternClips).toHaveLength(2);
    expect(document.frame!.snapshot.patternClips[1]).toMatchObject({ transpose: 12, velocityScale: .7 });
    await document.arrange(document.view.revision, { action: "resize", kind: "pattern", resource: 0, clip: 1, durationBeats: 2 });
    await document.arrange(document.view.revision, { action: "enable", kind: "pattern", resource: 0, clip: 1, enabled: false });
    await document.arrange(document.view.revision, { action: "enable", kind: "pattern", resource: 0, clip: 1, enabled: true });
    const plugin = document.view.plugins.find(p => p.pluginId === "oxitone.delay")!;
    await document.assignPlugin(document.view.revision, { plugin: plugin.handle, target: "channelInsert", owner: lead().id });
    ...
    await document.editConfiguration(document.view.revision, setting.handle, { kind: "parameters", values: { feedback: .45 } }, first);
    expect(lead().effectChain[0]!.parameters.feedback).toBe(.45);
    await document.assignPlugin(document.view.revision, { plugin: plugin.handle, target: "channelInsert", owner: lead().id });
    const ownerSite = document.view.effectOwnerSites!.find(s => s.owner === lead().id && s.scope === "definition")!;
    await document.editEffectOrder(document.view.revision, ownerSite.handle, lead().id, lead().effectChain.map(e => e.instanceId!).reverse());
    expect(lead().effectChain[1]!.instanceId).toBe(first);
    await document.configure(document.view.revision, { kind: "effectOrder", owner: "channel", index: 0, order: [1, 0] });
    ...
    await document.assignPlugin(document.view.revision, { plugin: plugin.handle, target: "channelInsert", owner: lead().id, instance: first });
    expect(lead().effectChain[0]!.instanceId).not.toBe(first);
    expect(document.view.configurationSites.some(s => s.usages.some(u => u.handle === lead().effectChain[0]!.instanceId))).toBe(true);
    await document.undo(document.view.revision); expect(lead().effectChain[0]!.instanceId).toBe(first);
    await document.redo(document.view.revision);
...
it("repairs an invalid missing-package draft through the real dispatcher and keeps failure invalid", async () => {
  const root = await mkdtemp(join(tmpdir(), "oxitone-controls-repair-")); let document: ProjectDocument | undefined;
  try {
    await mkdir(join(root, "node_modules/@oxitone"), { recursive: true });
    await symlink(fileURLToPath(new URL("../../core", import.meta.url)), join(root, "node_modules/@oxitone/core"), "dir");
    const entry = join(root, "song.ts");
    await writeFile(join(root, "package.json"), '{"name":"repair-test","dependencies":{}}');
    await writeFile(entry, "import { Project } from '@oxitone/core'; import { bpm } from 'missing-pattern'; export default new Project({ bpm });");
    let fail = true;
    document = await ProjectDocument.open({ entry, pluginTaskRunner: { async run() {
      if (fail) throw new Error("registry down");
      const pkg = join(root, "node_modules/missing-pattern"); await mkdir(pkg);
      await writeFile(join(pkg, "package.json"), '{"name":"missing-pattern","type":"module","exports":"./index.js"}');
    ...
    const request = (sequence: number) => ({ documentProtocolVersion: "2.0", sessionId: document!.sessionId, requestId: `stream/test/${sequence}`,
      baseRevision: document!.view.revision, operation: { kind: "installPlugin", packageName: "missing-pattern" } });
