...
import {
  ...
  materializePatternReference,
  writeLiteralPatternEdit,
  writePatternEdit,
...
for (const notes of [100, 1000]) {
  const source = arp(chord(60, "major"), "upDown", 0.25)
    .repeat(notes / 4)
    .toSource();
  const request = {
    fileName: "song.ts",
    ...
    anchor,
    source,
    operations: [{ select: { iteration: 2, note: { step: 1 } }, set: { pitch: 65 } }],
  };
  ...
  measure("literal-note-write", notes, () =>
    writeLiteralPatternEdit({
      ...detached,
      fileName: "song.ts",
      operations: [{ select: { at: { start: 2.25, pitch: 65 } }, set: { velocity: 0.3 } }],
    }),
  );
}
console.log(
