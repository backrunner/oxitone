...
const iterations = 40, warmup = 5, samples = [];
const timing = channel("oxitone.source.timing");
let phases, document;
const collect = ({ phase, milliseconds }) => { if (phases) phases[phase] = (phases[phase] ?? 0) + milliseconds; };
...
try {
  ...
  for (let i = 0; i < warmup + iterations; i++) {
    const view = document.view, owner = document.frame.snapshot.channels[0];
    const site = view.effectOwnerSites.find(site => site.owner === owner.id && site.scope === "definition");
    phases = {};
    await document.editEffectOrder(view.revision, site.handle, owner.id, owner.effectChain.map(effect => effect.instanceId).reverse());
  ...
  const measurements = Object.keys(samples[0]).map(phase => {
    ...
    return { phase, p50Ms: percentile(.5), p95Ms: percentile(.95), p99Ms: percentile(.99) };
  });
  console.log(JSON.stringify({ benchmark: "effect-order", cpu: cpus()[0]?.model, os: `${platform()} ${release()}`, node: process.version,
    iterations, warmup, notes: 100, placements: 2, effects: 2, sampleRate: 48000, blockSize: 128,
    nodeCompileCache: !!process.env.NODE_COMPILE_CACHE && process.env.NODE_DISABLE_COMPILE_CACHE !== "1",
    device: null, cpuUtilization: null, callbackP95: null, callbackP99: null, xruns: null, measurements, samples }, null, 2));
} finally { phases = undefined; timing.unsubscribe(collect); document?.close(); await rm(root, { recursive: true, force: true }); }
