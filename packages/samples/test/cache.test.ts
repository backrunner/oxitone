...
describe("normalized sample cache", () => {
  it("publishes deterministic float32 PCM, reuses immutable files and detects changed sources", () => {
    ...
    expect(imported).toMatchObject({ format: "wav", sampleRate: 24_000, channels: 2, frames: 2400n,
      assetUri: `assets/${imported.sha256}.wav`, provenance: { sourceChannels: 6,
        sourceFormat: "wav", sourceSampleRate: 24_000, cacheEncoding: "wav-f32-v1",
  ...
  it.skipIf(process.platform !== "darwin")("caches AAC and survives source deletion, project relocation and native round trips", async () => {
    ...
    const render = async (project: Project, name: string, assetBaseDir?: string) => {
      ...
      await project.renderWav({ path: output, ...(assetBaseDir === undefined ? {} : { assetBaseDir }),
        end: { seconds: 0.15 }, tailSeconds: 0, bitDepth: "float32" });
    ...
    try {
      expect(compile(engine, restored.snapshot(), { assetBaseDir: relocated }).samples[0]?.provenance).toEqual(imported.provenance);
    ...
    expect(readFileSync(join(next, "oxitone.project.json"))).toEqual(readFileSync(join(relocated, "oxitone.project.json")));
    const invalid = structuredClone(restored.snapshot());
    invalid.samples[0]!.provenance!.sourceSha256 = "wrong";
    expect(() => Project.fromSnapshot(invalid)).toThrowError(expect.objectContaining({ code: ErrorCode.InvalidProject }));
  });
  ...
  it("rejects corrupt or linked cache assets and invalid destinations without overwriting", () => {
    ...
    expect(() => cacheSample(path, cacheDir)).toThrowError(expect.objectContaining({ code: ErrorCode.AssetUnavailable, details: { path: cached.path } }));
    ...
    expect(() => cacheSample(path, cacheDir)).toThrowError(expect.objectContaining({ code: ErrorCode.AssetUnavailable }));
    ...
    for (const value of ["", "bad\0path"]) {
      ...
      expect(() => importSample(path, { cacheDir: value })).toThrowError(expect.objectContaining({ code: ErrorCode.InvalidProject }));
    }
    expect(() => importSample(path, { assetBaseDir: directory, cacheDir: "../escape-cache" })).toThrowError(expect.objectContaining({ code: ErrorCode.InvalidProject }));
