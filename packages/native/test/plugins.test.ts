...
function snapshot(gain?: number): ProjectSnapshot {
  return {
    ...
    patterns: [
      {
        id: "pat_plugin",
        lengthBeats: beatToWire(1),
        notes: [{ pitch: 64, start: beatToWire(0), duration: beatToWire(1), velocity: 0.5 }],
      },
    ],
    patternClips: [
      {
        id: "pcl_plugin",
        patternId: "pat_plugin",
        trackId: "trk_plugin",
        startBeat: beatToWire(0),
        durationBeats: beatToWire(1),
      },
    ],
    sampleClips: [],
    samples: [],
    ...
    channels: [
      {
        id: "chn_plugin",
        instrument: { pluginId: "oxitone.wavetable", pluginVersion: "1.0.0", parameters: {} },
        effectChain:
          gain === undefined ? [] : [{ pluginId: "fixture.gain", pluginVersion: "1.0.0", parameters: { gain } }],
        level: 0.05,
        pan: 0,
  ...
  it("applies insert automation and queued host parameters to dynamic effects", () => {
    ...
    try {
      ...
      for (const owner of ["chn_plugin", "mix_master"]) {
        ...
        if (owner === "mix_master") {
          input.mixerChannels.push({
            id: owner,
            level: 1,
            balance: 0,
            inserts: input.channels[0]!.effectChain,
            sends: [],
          });
        ...
        input.automation.push({
          id: "auto_gain",
          target: { entityId: owner, parameterId: "insert.0.parameter.gain" },
          source: { kind: "constant", value: 0.25 },
        }); // descriptor gain is 0..2; physical = 0.5
