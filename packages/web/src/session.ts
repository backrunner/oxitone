...
export class WebAudioSession {
  ...
  private constructor(
    ...
    worker.onmessage = ({
      ...
      if (data.type === "fault") {
        this.fail(new WebRuntimeError(data.error!.code, data.error!.message));
        return;
      }
  ...
  static async create(options: WebAudioOptions): Promise<WebAudioSession> {
    if (!globalThis.crossOriginIsolated || typeof SharedArrayBuffer === "undefined")
      throw new WebRuntimeError(
        "DeviceUnavailable",
        "Web Audio requires a secure, cross-origin isolated page (COOP: same-origin; COEP: require-corp)",
      );
    ...
    try {
      ...
      output = new AudioWorkletNode(context, "oxitone-output", {
        numberOfInputs: 0,
        numberOfOutputs: 1,
        outputChannelCount: [2],
        processorOptions: { buffer: ring.buffer, frames: ring.frames },
      });
      ...
      await session.request({
        type: "init",
        wasmUrl: new URL(options.wasmUrl, location.href).href,
        buffer: ring.buffer,
        frames: ring.frames,
  ...
  private request<T>(message: Record<string, unknown>): Promise<T> {
    if (this.disposed || this.failed)
      return Promise.reject(this.failed ?? new WebRuntimeError("InvalidProject", "Web Audio session is disposed"));
    const id = ++this.id;
  ...
  importSample(bytes: Uint8Array, format: SampleFormat): Promise<WasmSampleInfo> {
    return this.request({ type: "importSample", bytes, format });
  }
  update(input: ProjectInput): Promise<WasmState> {
    try {
      return this.request({ type: "compile", snapshot: snapshotOf(input) });
    } catch (error) {
  ...
  async play(frame?: bigint | number, loop?: WasmTransport["loop"]): Promise<WasmState> {
    ...
    return this.request({
      type: "transport",
      options: { command: "play", ...(frame === undefined ? {} : { frame }), ...(loop === undefined ? {} : { loop }) },
    });
  }
  pause(): Promise<WasmState> {
    return this.request({ type: "transport", options: { command: "pause" } });
  }
  stop(): Promise<WasmState> {
    return this.request({ type: "transport", options: { command: "stop" } });
  }
  seek(frame: bigint | number): Promise<WasmState> {
    return this.request({ type: "transport", options: { command: "seek", frame } });
  }
  ...
  state(): Promise<WasmState> {
    return this.request({ type: "state" });
  }
  diagnostics(): WebAudioDiagnostics {
    return {
      underruns: Atomics.load(this.ring.header, UNDERRUNS),
      bufferedFrames: this.ring.buffered(),
      sampleRate: this.context.sampleRate,
      running: !!Atomics.load(this.ring.header, RUNNING),
    };
  }
  async dispose(): Promise<void> {
    ...
    for (const item of this.pending.values()) {
      clearTimeout(item.timer);
      item.reject(new WebRuntimeError("InvalidProject", "Web Audio session disposed"));
    }
    this.pending.clear();
