...
import type { SampleFormat, ProjectInput, WasmRenderOptions, WasmSampleInfo, WasmState, WasmTransport } from "./types.js";
...
interface Abi extends WebAssembly.Exports {
  ...
  oxi_response_ptr(): number; oxi_response_len(): number;
  oxi_binary_ptr(): number; oxi_binary_len(): number;
  oxi_left_ptr(): number; oxi_right_ptr(): number;
  oxi_process(): number; oxi_allocations(): number; oxi_deallocations(): number;
}
...
export function frameString(value: bigint | number): string {
  if (typeof value === "number" && !Number.isSafeInteger(value)) throw new WebRuntimeError("InvalidProject", "frame must be a safe integer or bigint");
  const frame = BigInt(value);
  // Reserve headroom for block advancement; JS control positions stay exactly representable.
  if (frame < 0n || frame > BigInt(Number.MAX_SAFE_INTEGER)) throw new WebRuntimeError("InvalidProject", "frame must be between zero and 2^53-1");
...
export class WasmEngine {
  ...
  private constructor(instance: WebAssembly.Instance) {
    ...
    if (this.abi.oxi_abi_version() !== 1) throw new WebRuntimeError("ProtocolVersionUnsupported", "unsupported Wasm ABI");
  }
  static async create(source: BufferSource | WebAssembly.Module | string | URL): Promise<WasmEngine> {
    const bytes = typeof source === "string" || source instanceof URL
      ? await (async () => { const response = await fetch(source); if (!response.ok) throw new WebRuntimeError("AssetUnavailable", `Wasm fetch failed: ${response.status}`); return response.arrayBuffer(); })()
      : source;
    const module = bytes instanceof WebAssembly.Module ? bytes : await WebAssembly.compile(bytes);
    if (WebAssembly.Module.imports(module).length) throw new WebRuntimeError("PluginAbiMismatch", "Oxitone Wasm must have no host imports");
  ...
  private refresh(): void {
    ...
    this.pcm = [new Float32Array(abi.memory.buffer, abi.oxi_left_ptr(), this.blockSize),
  ...
  command<T = unknown>(command: Record<string, unknown>, data: Uint8Array = new Uint8Array()): T {
    ...
    if (json.length > 16 * 1024 * 1024 || data.length > 64 * 1024 * 1024) throw new WebRuntimeError("InvalidProject", "Wasm input exceeds byte limit");
    let ptr = 0, binary = 0;
    try {
      ptr = abi.oxi_alloc(json.length);
      if (!ptr) throw new WebRuntimeError("InvalidProject", "Wasm input allocation rejected");
      if (data.length) { binary = abi.oxi_alloc(data.length); if (!binary) throw new WebRuntimeError("InvalidProject", "Wasm asset allocation rejected"); }
      ...
      const response = JSON.parse(this.decoder.decode(new Uint8Array(abi.memory.buffer,
        abi.oxi_response_ptr(), abi.oxi_response_len()))) as { ok: boolean; protocolVersion: string; value: T; error?: { code: string; message: string } };
      if (response.protocolVersion !== "1.0") throw new WebRuntimeError("ProtocolVersionUnsupported", "unsupported Wasm response");
      ...
      if (this.abi) {
        ...
        if (command.type === "compile" && abi.oxi_left_ptr()) {
          const state = this.decoder.decode(new Uint8Array(abi.memory.buffer, abi.oxi_response_ptr(), abi.oxi_response_len()));
  ...
  compile(input: ProjectInput): WasmState { return this.command({ type: "compile", snapshot: snapshotOf(input) }); }
  importSample(bytes: Uint8Array, format: SampleFormat): WasmSampleInfo {
    return this.command({ type: "importSample", format }, bytes);
  }
  state(): WasmState { return this.command({ type: "state" }); }
  transport(options: WasmTransport): WasmState {
    return this.command({ type: "transport", command: options.command,
      ...(options.frame === undefined ? {} : { frame: frameString(options.frame) }),
      ...(options.loop === undefined ? {} : { loopRegion: [frameString(options.loop.startFrame), frameString(options.loop.endFrame)] }) });
  }
  setParameter(entityId: string, parameterId: string, value: number, atFrame?: bigint | number): void {
    this.command({ type: "setParameter", entityId, parameterId, value,
  ...
  resolveBeatDuration(input: ProjectInput, startBeat: number, durationSeconds: number): number {
    const beats = this.command<{ numerator: number; denominator: number }>({ type: "resolveBeatDuration",
      snapshot: snapshotOf(input), startBeat: beatToWire(startBeat), durationSeconds });
  ...
  exportMidi(options: { ppq?: number; tempoEventResolutionTicks?: number } = {}): Uint8Array {
    this.command({ type: "exportMidi", options }); return this.binary();
  }
  memoryDiagnostics() {
    const abi = this.requireAbi();
    return { bytes: abi.memory.buffer.byteLength, allocations: abi.oxi_allocations(), deallocations: abi.oxi_deallocations() };
  }
  dispose(): void {
    if (!this.abi) return;
    this.command({ type: "dispose" }); this.abi = undefined;
