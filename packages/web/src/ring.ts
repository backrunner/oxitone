...
export const WRITE = 0, READ = 1, RUNNING = 2, UNDERRUNS = 3, EPOCH = 4, ACK = 5, FLUSH_AT = 6;
export const HEADER_WORDS = 8;
export class PcmRing {
  readonly header: Int32Array;
  readonly left: Float32Array;
  readonly right: Float32Array;
  constructor(readonly buffer: SharedArrayBuffer, readonly frames: number) {
  ...
  static create(frames: number): PcmRing {
    if (!Number.isInteger(frames) || frames < 512 || frames > 65536 || (frames & (frames - 1))) throw new RangeError("ringFrames must be a power of two between 512 and 65536");
  ...
  write(left: Float32Array, right: Float32Array): boolean {
    if (Atomics.load(this.header, ACK) !== Atomics.load(this.header, EPOCH) ||
        left.length > this.frames - this.buffered()) return false;
    const write = Atomics.load(this.header, WRITE) >>> 0;
    for (let i = 0; i < left.length; i++) {
      const index = (write + i) & (this.frames - 1);
      this.left[index] = left[i]!; this.right[index] = right[i]!;
    }
  ...
  read(left: Float32Array, right: Float32Array): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      left[i] = this.left[index]!; right[i] = this.right[index]!;
    }
    ...
    if (Atomics.load(this.header, EPOCH) !== epoch) { left.fill(0); right.fill(0); }
  }
}
