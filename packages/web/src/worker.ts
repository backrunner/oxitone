...
function flush(at: bigint): void {
  ring!.flush();
  cursor.reset(ring!, at);
  primed = false;
}
function pump(): void {
  ...
  try {
    ...
    playing = false;
    Atomics.store(ring.header, RUNNING, 0);
...
async function handle(request: Request): Promise<unknown> {
  ...
  switch (request.type) {
    case "importSample":
      return engine.importSample(request.bytes as Uint8Array, request.format as SampleFormat);
    case "compile": {
      ...
      if (snapshot.blockSize > Math.min(2048, ring.frames))
        throw new Error("Project blockSize exceeds the Web Audio render horizon");
    ...
    case "setParameter":
      engine.setParameter(
        ...
        request.parameterId as string,
        request.value as number,
        request.atFrame as bigint | undefined,
      );
      return null;
    case "state":
      return { ...engine.state(), cursor: audibleFrame().toString() };
    case "dispose":
      playing = false;
      flush(0n);
      ...
      return null;
    default:
      throw new Error("Unknown Web Audio worker request");
  }
}
scope.onmessage = ({ data }) => {
  serial = serial.then(async () => {
    try {
      scope.postMessage({ id: data.id, ok: true, value: await handle(data) });
    } catch (error) {
      scope.postMessage({
        id: data.id,
        ok: false,
