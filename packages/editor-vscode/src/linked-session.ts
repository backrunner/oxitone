...
export interface SessionMemory { socket: string; baselines: Record<string, string>; baselineSessions?: Record<string, string> }
...
export class LinkedSession implements vscode.Disposable {
  ...
  constructor(readonly id: string, readonly memory: SessionMemory, private readonly changed: () => void) {
    ...
    this.subscriptions.push(
      ...
      vscode.workspace.onDidCloseTextDocument(document => {
        ...
        this.opened.delete(path); this.served.delete(path); this.failures.delete(path); this.editor.closeBuffer(path);
        if (!retainBaseline) { delete memory.baselines[path]; if (memory.baselineSessions) delete memory.baselineSessions[path]; }
        this.changed();
      }),
    );
  ...
  owns(uri: vscode.Uri): boolean { return uri.scheme === "oxitone" && uri.authority === this.id; }
  get failure(): Error | undefined { return this.failures.values().next().value; }
  read(fileName: string, text: string): void { if (!this.opened.has(fileName)) this.served.set(fileName, text); }
  private document(fileName: string): vscode.TextDocument | undefined {
    return vscode.workspace.textDocuments.find(document => this.owns(document.uri) && sourcePath(document.uri) === fileName && !document.isClosed);
  }
  private open(document: vscode.TextDocument): void {
    ...
    this.editor.openBuffer({ fileName, text: document.getText(), version: document.version,
      baselineText: restored ? this.memory.baselines[fileName]! : readBaseline,
      ...(restored ? { baselineSessionId: this.memory.baselineSessions?.[fileName] ?? "unknown-restored-session" } : {}) });
  ...
  private change(document: vscode.TextDocument): void {
    ...
    if (buffer && document.version > buffer.version) this.editor.changeBuffer(path, document.getText(), document.version);
  }
  private observe(document: vscode.TextDocument): void {
    if (!this.owns(document.uri)) return;
    const path = sourcePath(document.uri);
    try { this.change(document); this.failures.delete(path); }
    catch (error) { this.failures.set(path, error instanceof Error ? error : new Error(String(error))); this.changed(); }
  }
  private async apply(fileName: string, proposal: { text: string; expectedVersion: number }): Promise<void> {
    ...
    try {
      ...
      this.failures.set(fileName, error instanceof Error ? error : new Error(String(error))); this.changed();
    } finally {
      this.applying.delete(fileName);
      const next = this.editor.view.buffers.find(buffer => buffer.fileName === fileName)?.apply;
      if (next && (next.text !== proposal.text || next.expectedVersion !== proposal.expectedVersion)) void this.apply(fileName, next);
    }
  }
  async ready(): Promise<void> {
    if (this.view.state === "connected") return;
    await new Promise<void>((accept, reject) => {
      const timer = setTimeout(() => { unsubscribe(); reject(new Error("DAW connection timed out. Check the document socket path.")); }, 15_000);
      const unsubscribe = this.editor.subscribe(view => {
        if (view.state === "connected" || view.state === "closed") {
          ...
          if (view.state === "connected") accept(); else reject(new Error("DAW connection closed"));
        }
      });
    });
  }
  async flush(): Promise<void> {
    ...
    for (const document of vscode.workspace.textDocuments) {
      ...
      if (!buffer || buffer.version !== document.version || buffer.text !== document.getText()) throw new Error("The actual editor text has not synchronized. Resolve its diagnostic before saving.");
    }
  ...
  save(fileName?: string, content?: string): Promise<void> {
    const operation = this.saveQueue.then(async () => {
      ...
      if (fileName && (!document || document.getText() !== content)) throw new Error("Save text is stale. Retry saving the current linked editor.");
      await this.flush();
      if (this.view.document?.modified) await this.editor.command("save");
      if (document && (document.version !== version || document.getText() !== content)) throw new Error("The editor changed during Save; newer typing remains unsaved.");
      if (this.view.document?.status !== "ready") throw new Error("Resolve the project diagnostic or conflict before saving.");
    });
  ...
  async openSource(fileName: string): Promise<vscode.TextEditor> {
    await this.ready();
    return vscode.window.showTextDocument(await vscode.workspace.openTextDocument(sourceUri(this.id, fileName)), { preview: false });
  }
  async createFile(fileName: string, text = ""): Promise<void> {
    ...
    if (!this.view.document?.files.some(file => file.path === fileName)) {
      await new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => { unsubscribe(); reject(new Error("The DAW did not publish the new source file")); }, 5000);
        const unsubscribe = this.editor.subscribe(view => {
          if (view.document?.files.some(file => file.path === fileName)) { clearTimeout(timer); queueMicrotask(() => unsubscribe()); resolve(); }
        });
      });
    }
  }
  async installPlugin(packageName: string, version?: string): Promise<void> {
    await this.ready();
    const response = await this.editor.request({ kind: "installPlugin", packageName, ...(version === undefined ? {} : { version }) });
    if (!response.accepted) throw new Error(response.error?.message ?? "The DAW rejected the plugin installation");
  }
  dispose(): void {
    this.disposed = true; this.unsubscribe(); this.editor.close();
