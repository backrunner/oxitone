...
export class SourceFileSystem implements vscode.FileSystemProvider, vscode.Disposable {
  ...
  private session(uri: vscode.Uri): LinkedSession {
    ...
    if (!session || !session.owns(uri))
      throw vscode.FileSystemError.Unavailable("Connect to this Oxitone DAW session first.");
    return session;
  }
  private async text(uri: vscode.Uri): Promise<string> {
    ...
    if (session.view.state !== "connected") {
      const cached =
        session.view.document?.files.find((file) => file.path === path)?.text ?? session.memory.baselines[path];
  ...
  async writeFile(uri: vscode.Uri, content: Uint8Array): Promise<void> {
    const session = this.session(uri);
    if (!session.view.document?.files.some((file) => file.path === sourcePath(uri)))
      throw vscode.FileSystemError.NoPermissions("Only existing project sources can be saved.");
    try {
      await session.save(sourcePath(uri), new TextDecoder("utf-8", { fatal: true }).decode(content));
    } catch (error) {
      throw vscode.FileSystemError.Unavailable(String(error));
    }
  }
  ...
  createDirectory(): never {
    throw vscode.FileSystemError.NoPermissions("Create project files on disk, then reopen the DAW.");
  }
  delete(): never {
    throw vscode.FileSystemError.NoPermissions("Linked sources cannot be deleted from the editor.");
  }
  rename(): never {
    throw vscode.FileSystemError.NoPermissions("Linked sources cannot be renamed from the editor.");
  }
  dispose(): void {
