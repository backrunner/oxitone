...
export class Presentation implements vscode.TreeDataProvider<Item>, vscode.Disposable {
  ...
  constructor(private readonly sessions: ReadonlyMap<string, LinkedSession>) {
    this.status.command = "oxitone.openSource";
  }
  refresh(): void {
    ...
    const state = conflicts
      ...
      : (selected?.failure?.message ??
        view.error?.message ??
        view.document?.diagnostic?.message ??
        (view.state !== "connected"
          ? view.state
          : view.submitting
            ? "synchronizing"
            : view.buffers.some((buffer) => buffer.apply)
              ? "applying DAW changes"
              : view.document?.status !== "ready"
                ? view.document?.status
                : view.document.modified
                  ? "modified"
                  : "saved"));
  ...
  getChildren(parent?: Item): Item[] {
    ...
    if (parent)
      return (parent.session.view.document?.files ?? []).map((file) => ({
  ...
  getTreeItem(item: Item): vscode.TreeItem {
    if (!item.fileName) {
      const node = new vscode.TreeItem(
        basename(item.session.view.document?.files[0]?.path ?? item.session.memory.socket),
        vscode.TreeItemCollapsibleState.Expanded,
      );
      node.id = item.session.id;
      node.description = item.session.view.state;
      node.tooltip = item.session.memory.socket;
      return node;
    }
    const node = new vscode.TreeItem(sourceUri(item.session.id, item.fileName));
    node.description = item.session.view.buffers.some((buffer) => buffer.fileName === item.fileName && buffer.conflict)
      ? "Conflict"
      : item.fileName;
    node.command = { command: "oxitone.openSource", title: "Open source", arguments: [item] };
    return node;
  }
  dispose(): void {
    this.status.dispose();
...
export class ConflictReview implements vscode.TextDocumentContentProvider, vscode.Disposable {
  ...
  provideTextDocumentContent(uri: vscode.Uri): string {
    return this.snapshots.get(uri.toString()) ?? "This review expired. Open a new conflict review.";
  }
  async review(session: LinkedSession): Promise<void> {
    ...
    const selected = await vscode.window.showQuickPick(
      conflicts.map((buffer) => ({ label: basename(buffer.fileName), description: buffer.fileName, buffer })),
      { title: "Oxitone editor conflicts" },
    );
    if (!selected) return;
    const buffer = selected.buffer,
      conflict = buffer.conflict!;
    const remote = this.snapshot(conflict.remote, "DAW"),
      base = this.snapshot(conflict.baseline, "Base");
    await vscode.commands.executeCommand(
      "vscode.diff",
      remote,
      sourceUri(session.id, buffer.fileName),
      "Oxitone: DAW ↔ Editor",
    );
    let action: string | undefined;
    do {
      action = await vscode.window.showWarningMessage(
        conflict.reason === "sessionChanged"
          ? "The restarted DAW has different source. Review the difference before replacing your retained editor text."
          : "Both the DAW and editor changed this source. Choose which text to keep after reviewing the difference.",
        "Keep Editor Text",
        "Use DAW Text",
        "View Base",
      );
    ...
    if (action)
      session.editor.resolveConflict(
        buffer.fileName,
        conflict.version,
        conflict.revision,
        action === "Use DAW Text" ? "remote" : "local",
      );
  }
  dispose(): void {
