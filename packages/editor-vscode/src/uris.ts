...
export function connectionId(socket: string): string {
  return createHash("sha256").update(socket).digest("hex").slice(0, 24);
}
...
export function sourcePath(uri: vscode.Uri): string {
