...
export async function checkSourceCreation(
  ...
  assert(
    await created.edit((edit) =>
      edit.insert(
        new vscode.Position(0, 0),
        "import { chord } from '@oxitone/core'; export const melody = chord(60, 'major');\n",
      ),
    ),
  );
  await until(
    () =>
      peer.view!.status === "ready" && peer.view!.files.find((file) => file.path === path)!.text.includes("chord(60"),
    "new source buffer evaluation",
  );
  const entry = await vscode.window.showTextDocument(entryEditor.document, { preview: false });
  assert(
    await entry.edit((edit) =>
      edit.replace(
        new vscode.Range(entry.document.positionAt(0), entry.document.positionAt(entry.document.getText().length)),
  ...
  await until(
    () => peer.view!.status === "ready" && peer.view!.sites.some((site) => site.fileName === path),
    "import unsaved module from linked entry",
  );
  const site = peer.view!.sites.find(
    (site) => site.fileName === path && site.label === "melody" && site.scope === "definition",
  )!;
  assert(
    (await peer.request({ kind: "notes", site: site.handle, edits: [{ select: { degree: 2 }, set: { pitch: 65 } }] }))
      .accepted,
  );
