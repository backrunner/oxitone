...
async function until(check: () => boolean, label: string): Promise<void> {
  ...
  while (!check()) { if (Date.now() > deadline) throw new Error(`Timed out: ${label}`); await new Promise(resolve => setTimeout(resolve, 20)); }
}
async function start(entry: string, socket: string): Promise<ChildProcess> {
  const child = spawn(process.env.OXITONE_TEST_NODE!, [process.env.OXITONE_TEST_SERVICE!, entry, socket], { stdio: ["ignore", "pipe", "pipe"] });
  let output = ""; child.stdout!.on("data", data => { output += String(data); }); child.stderr!.on("data", data => { output += String(data); });
  await until(() => { if (child.exitCode !== null) throw new Error(output); return output.includes("READY"); }, "Document Service start");
  return child;
}
async function stop(child: ChildProcess): Promise<void> { child.kill("SIGTERM"); await until(() => child.exitCode !== null || child.signalCode !== null, "Document Service stop"); }

export async function run(): Promise<void> {
  const root = process.env.OXITONE_TEST_ROOT!, entry = join(root, "song.ts"), socket = join(root, "document");
  const initial = await readFile(entry, "utf8");
  let service = await start(entry, socket);
  let dawTask: vscode.TaskExecution | undefined;
  const taskMonitor = vscode.tasks.onDidStartTask(event => { if (event.execution.task.name === "DAW") dawTask = event.execution; });
  ...
  try {
    ...
    assert((await peer.request({ kind: "notes", site: site.handle, edits: [{ select: { degree: 2 }, set: { pitch: 71 } }] })).accepted);
    await until(() => editor.document.getText() === peer.view!.files[0]!.text, "DAW edit in actual VS Code buffer");
    const generated = editor.document.getText(); assert.notEqual(generated, typed);
    assert.equal(await readFile(entry, "utf8"), initial);
    ...
    assert((await peer.request({ kind: "code", fileName: entry, text: `${generated}\n// DAW change while hidden` })).accepted);
    ...
    const stale = new vscode.WorkspaceEdit(); stale.replace(editor.document.uri, range(), "stale text", { label: "Apply Oxitone DAW change", needsConfirmation: false });
    ...
    await until(() => peer.view!.status === "ready" && peer.view!.files[0]!.text === acknowledged, "acknowledge unsaved source");
    const priorSession = peer.view!.sessionId;
    await stop(service); service = await start(entry, socket);
    await until(() => peer.state === "connected" && peer.view!.sessionId !== priorSession, "service restarts from older disk source");
    ...
    await until(() => peer.view!.status === "ready" && peer.view!.files[0]!.text === acknowledged, "reconcile matching source");
    assert(await editor.document.save());
    await stop(service);
    ...
    await until(() => peer.state === "connected" && peer.view!.status === "ready" && peer.view!.files[0]!.text === offline, "reconnect dirty editor");
    ...
    await until(() => !editor.document.isDirty && peer.view!.files[0]!.text === autoSaved && !peer.view!.modified, "auto save through journal");
    ...
    await assert.rejects(Promise.resolve(vscode.workspace.fs.writeFile(editor.document.uri.with({ path: `${root}/node_modules/evil.ts` }), Buffer.from("no"))));
    ...
    await launch.update("previewArguments", ["--headless", "--viewer", process.env.OXITONE_TEST_VIEWER], vscode.ConfigurationTarget.Global);
    await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(vscode.Uri.file(entry)), { preview: false });
    ...
    console.log("PASS Oxitone Extension Host: unsaved TS, visible/hidden DAW edits, journal Save/auto save, Undo/Redo, version race, unsaved restart conflict, offline reconnect, dependency guard, production CLI task with simulated GPUI.");
  } finally { taskMonitor.dispose(); dawTask?.terminate(); peer.close(); await stop(service); }
}
