...
import { canonicalEncode, checkProtocolVersion, ErrorCode, OxitoneError, PROJECT_FORMAT_VERSION,
...
function failure(error: unknown, path: string): never {
  ...
  if (error instanceof SyntaxError) throw new OxitoneError(ErrorCode.InvalidProject, "malformed project JSON", { details: { path } });
  throw new OxitoneError(ErrorCode.AssetUnavailable, error instanceof Error ? error.message : "project file operation failed", { details: { path } });
}
...
function checkResourceReferences(snapshot: ProjectSnapshot): void {
  ...
  const plugins = snapshot.channels.flatMap((c) => [c.instrument, ...c.effectChain])
    .concat(snapshot.mixerChannels.flatMap((bus) => bus.inserts));
  for (const plugin of plugins) for (const id of Object.values(plugin.resources ?? {})) {
...
export async function saveProject(snapshot: ProjectSnapshot, directory: string, options: SaveProjectOptions = {}): Promise<void> {
  ...
  try {
    ...
    for (const sample of portable.samples) {
      ...
      if (hash(bytes) !== sample.sha256) throw new OxitoneError(ErrorCode.AssetUnavailable,
        "sample content hash does not match descriptor", { details: { path: source } });
      ...
      try {
        ...
        try { await link(temp, target); } catch (error) {
          ...
          if ((await lstat(target)).isSymbolicLink()) throw new OxitoneError(ErrorCode.InvalidProject, "cached asset must not be a symbolic link");
          if (hash(await readFile(target)) !== sample.sha256) throw new OxitoneError(ErrorCode.AssetUnavailable,
            "existing cached asset has an unexpected hash", { details: { path: target } });
        }
      } finally { await unlink(temp).catch(() => {}); }
      sample.assetUri = uri;
    }
    const assetDir = await open(resolve(root, "assets"), "r");
    try { await assetDir.sync(); } finally { await assetDir.close(); }
    await atomicWrite(resolve(root, FILE), canonicalEncode(projectFileSchema.parse({
      ...portable, formatVersion: PROJECT_FORMAT_VERSION, projectId: portable.id,
    })));
  } catch (error) { failure(error, root); }
}
...
export async function loadProject(directory: string): Promise<LoadedProject> {
  ...
  try {
    ...
    if (typeof raw !== "object" || raw === null || !("formatVersion" in raw) || raw.formatVersion !== PROJECT_FORMAT_VERSION) {
    ...
    if (!parsed.success) throw new OxitoneError(ErrorCode.InvalidProject, "invalid project manifest", { details: { path: FILE } });
    ...
    for (const sample of snapshot.samples) {
      ...
      if (hash(await readFile(asset)) !== sample.sha256) throw new OxitoneError(ErrorCode.AssetUnavailable,
        "sample content hash does not match manifest", { details: { path: asset } });
    }
