import { multisamplerStateSchema, multisamplerOptionsSchema, OxitoneError, ErrorCode,
  type InstrumentRef, type MultisamplerOptions } from "@oxitone/protocol";
import { Sample } from "./sample.js";
import { sampler } from "./instruments.js";
...
export function multisampler(regions: readonly SampleRegion[], options: MultisamplerOptions = {}): InstrumentRef {
  if (!Array.isArray(regions) || !regions.length || regions.length > 256 || regions.some(r => !(r?.sample instanceof Sample))) {
  ...
  const state = parseAuthoring(multisamplerStateSchema, { version: 1, regions: regions.map((r, i) => ({
    resource: `region_${i}`, rootKey: r.rootKey, keyRange: r.keyRange, velocityRange: r.velocityRange ?? [1, 127], gain: r.gain ?? 1,
  })) }, "multisampler.state");
  const cells = new Uint8Array(128 * 128);
  for (const r of state.regions) {
    const [low, high] = r.keyRange, [soft, loud] = r.velocityRange;
    if (low > high || soft > loud) throw new OxitoneError(ErrorCode.InvalidProject, "inverted multisampler region");
    for (let key = low; key <= high; key++) for (let v = soft; v <= loud; v++) {
      const index = key * 128 + v;
      if (cells[index]) throw new OxitoneError(ErrorCode.InvalidProject, "overlapping multisampler key/velocity regions");
      cells[index] = 1;
    }
  }
  const parameters = sampler(regions[0]!.sample, base).parameters;
  if (transpose !== undefined) parameters.transpose = transpose;
  return { pluginId: "oxitone.multisampler", pluginVersion: "1.0.0", parameters, state,
