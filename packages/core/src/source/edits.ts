...
function matchesExpected(note: SourceEvent["note"], expect: { [K in keyof SourceNote]?: SourceNote[K] | undefined }): boolean {
...
export function resolveEdits(base: readonly SourceEvent[], operations: readonly NoteEdit[]): SourceEvent[] {
  ...
  const indexed = indexSelectors(base, operations.flatMap((op) => "select" in op ? [op.select] : []));
  const targets = operations.map((op, operation) => {
    if ("insert" in op) return -1;
    const target = indexed.get(selectorKey(op.select));
    if (target === undefined || target === -1) {
      throw new OxitoneError(target === undefined ? ErrorCode.EditTargetMissing : ErrorCode.EditTargetAmbiguous,
        "note selector must resolve to exactly one base output", { details: { path: "pattern.edit", operation, select: op.select } });
    }
    const event = base[target];
    if (event && op.expect && !matchesExpected(event.note, op.expect)) {
      throw new OxitoneError(ErrorCode.SourceChanged, "note expectation no longer matches", { details: { path: "pattern.edit", operation, select: op.select } });
    }
    return target;
  });
  ...
  operations.forEach((op, i) => {
    ...
    if (prior !== undefined && prior !== action) throw new OxitoneError(ErrorCode.EditScopeConflict,
      "cannot remove and update the same note in one edit", { details: { path: "pattern.edit", operation: i } });
  ...
  operations.forEach((op, i) => {
    if ("insert" in op) {
      ...
      result.push({ note: op.insert, select: { inserted: inserted++ }, origin: { kind: "insert", note: op.insert, occurrence } });
      return;
    }
    const target = targets[i];
    if (target === undefined) return;
    if ("remove" in op) { result[target] = undefined; return; }
    const event = result[target];
    if (!event) return;
    const note = { ...event.note };
    if ("set" in op) Object.assign(note, Object.fromEntries(Object.entries(op.set).filter(([, value]) => value !== undefined)));
    else for (const [key, amount] of Object.entries(op.shift)) {
...
export function mergeSets(previous: readonly NoteEdit[], next: readonly NoteEdit[]): NoteEdit[] | undefined {
  ...
  for (const op of all) {
    ...
    if (prior && "set" in prior) prior.set = { ...prior.set, ...defined };
      ...
      merged.push(copy); bySelector.set(key, copy);
    }
  }
  return merged;
}
