...
function checkSelectors(node: unknown): void {
  ...
  for (const operation of node.operations as unknown[]) {
    ...
    while (typeof select === "object" && select !== null && "note" in select) {
      if (++depth > PATTERN_SOURCE_LIMITS.depth) throw new OxitoneError(ErrorCode.BudgetExceeded, "note selector exceeds depth budget");
      select = select.note;
    }
  }
}
...
export function createSource(node: PatternSourceNode, inputs: readonly SourceValue[] = []): SourceValue {
  ...
  if (depth > PATTERN_SOURCE_LIMITS.depth) throw new OxitoneError(ErrorCode.BudgetExceeded, "pattern source exceeds depth budget");
  ...
  if (!Number.isFinite(output.lengthBeats) || output.lengthBeats <= 0) throw new OxitoneError(ErrorCode.InvalidProject, "source length must be finite and positive");
  const events = output.events.map((event) => { validateNote(sourceNoteInput(event.note)); return event; })
...
export function serializeSource(value: SourceValue, name?: string): PatternSourceDocument {
  ...
  function visit(source: SourceValue): number {
    ...
    if (index >= PATTERN_SOURCE_LIMITS.nodes) throw new OxitoneError(ErrorCode.BudgetExceeded, "pattern source exceeds node budget");
    totalEvents += source.events.length;
    if (totalEvents > PATTERN_SOURCE_LIMITS.events * 8) throw new OxitoneError(ErrorCode.BudgetExceeded, "pattern graph exceeds total event budget");
    nodes.push(withReferences(source.node, refs));
    indexes.set(source, index);
    return index;
  }
  const root = visit(value);
  return structuredClone({ formatVersion: PATTERN_SOURCE_FORMAT, nodes, root, ...(name === undefined ? {} : { name }) });
}

export function restoreSource(input: unknown): { source: SourceValue; name?: string } {
  if (typeof input === "object" && input !== null && "formatVersion" in input && input.formatVersion !== PATTERN_SOURCE_FORMAT) {
  ...
  if (typeof input === "object" && input !== null && "nodes" in input && Array.isArray(input.nodes)) {
    if (input.nodes.length > PATTERN_SOURCE_LIMITS.nodes) throw new OxitoneError(ErrorCode.BudgetExceeded, "pattern source exceeds node budget");
  ...
  if (document.root !== document.nodes.length - 1) throw new OxitoneError(ErrorCode.InvalidProject, "source root must be the final node");
  const reachable = new Set<number>([document.root]);
  for (let i = document.nodes.length - 1; i >= 0; i--) {
    const node = document.nodes[i];
    if (!node || !reachable.has(i)) throw new OxitoneError(ErrorCode.InvalidProject, "source contains unreachable nodes");
  ...
  for (const node of document.nodes) {
    ...
    const source = createSource(withReferences(node, inputs.map((_, index) => index)), inputs);
    totalEvents += source.events.length;
    if (totalEvents > PATTERN_SOURCE_LIMITS.events * 8) throw new OxitoneError(ErrorCode.BudgetExceeded, "pattern graph exceeds total event budget");
    values.push(source);
  }
