...
export function resolveSource(node: PatternSourceNode, inputs: readonly SourceValue[]): SourceOutput {
  if (node.kind === "literal") {
    ...
    return { lengthBeats: node.lengthBeats, events: node.notes.map((note) => {
  ...
  if (node.kind === "concat") {
    ...
    inputs.forEach((input, segment) => {
      for (const event of input.events) events.push({ ...event,
        note: { ...event.note, start: event.note.start + offset }, select: { segment, note: event.select } });
  ...
  if (node.kind === "edit") return { events: resolveEdits(input.events, node.operations), lengthBeats: node.lengthBeats ?? input.lengthBeats };
  if (node.kind === "repeat") {
    ...
    for (let iteration = 0; iteration < node.count; iteration++) {
      for (const event of input.events) events.push({
        ...
        select: { iteration, note: event.select }, origin: { kind: "repeat", iteration, input: event.origin },
      });
    }
    return { events, lengthBeats: input.lengthBeats * node.count };
  }
  if (node.kind === "slice") {
    if (node.end <= node.start || node.end > input.lengthBeats) throw new OxitoneError(ErrorCode.InvalidProject, "slice must be a positive window within the source length");
    const events = input.events.flatMap((event) => {
      ...
      return end > start ? [{ ...event, note: { ...event.note, start: start - node.start, duration: end - start } }] : [];
    });
    return { events, lengthBeats: node.end - node.start };
  }
  return { lengthBeats: input.lengthBeats, events: input.events.map((event) => ({ ...event, note: {
    ...event.note,
    ...(node.kind === "transpose" ? { pitch: Math.max(0, Math.min(127, event.note.pitch + node.semitones)) }
