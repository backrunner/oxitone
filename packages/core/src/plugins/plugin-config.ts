import { effectRefSchema, instrumentRefSchema, ErrorCode, OxitoneError, type EffectRef, type InstrumentRef } from "@oxitone/protocol";
...
export class PluginConfig {
  ...
  constructor(kind: "instrument" | "effect", input: InstrumentRef | EffectRef) {
    if (kind !== "instrument" && kind !== "effect") throw new OxitoneError(ErrorCode.InvalidProject, "plugin configuration kind must be instrument or effect");
    this.#kind = kind;
    const schema = kind === "instrument" ? instrumentRefSchema.omit({ instanceId: true }).strict() : effectRefSchema.omit({ instanceId: true }).strict();
    const config = { ...input }; delete config.instanceId;
  ...
  get kind(): "instrument" | "effect" { return this.#kind; }
  toSpec(): InstrumentRef | EffectRef { return structuredClone({ ...this }); }
  ...
  withHost(settings: { mix?: number | undefined; bypass?: boolean | undefined }): PluginConfig {
    if (this.kind !== "effect") throw new OxitoneError(ErrorCode.InvalidProject, "only an effect has insert mix/bypass settings");
    return new PluginConfig("effect", { ...this.toSpec(), ...Object.fromEntries(Object.entries(settings).filter(([, value]) => value !== undefined)) });
  }
}
...
function assertSerializable(value: unknown, seen: Set<object>, depth: number): void {
  ...
  if (typeof value !== "object" || seen.has(value) || (!Array.isArray(value) && ![Object.prototype, null].includes(Object.getPrototypeOf(value)))) {
