  ...
  get config(): PluginConfig {
    ...
    const ref =
      this.kind === "instrument"
        ? spec.instrument
        : (spec.effectChain ?? spec.inserts)?.find((ref) => ref.instanceId === this.id);
    if (!ref || ref.instanceId !== this.id)
      throw new OxitoneError(ErrorCode.EditTargetMissing, "plugin instance is no longer attached");
    const { instanceId: _, ...config } = ref;
    return pluginConfig(this.kind, config);
  }
  param(parameterId: string): InstanceParameter {
    return new InstanceParameter(this, parameterId, "plugin");
  }
  get host(): { param: (parameterId: "mix" | "bypass") => InstanceParameter } {
    if (this.kind !== "effect")
      throw new OxitoneError(ErrorCode.AutomationTargetInvalid, "instrument has no insert host parameters");
    return { param: (parameterId) => new InstanceParameter(this, parameterId, "effectHost") };
  }
  /** @internal */
  belongsTo(owner: PluginInstanceOwner): boolean {
  ...
  set(parameter: string, scope: "plugin" | "effectHost", value: number | boolean): void {
    ...
    if (scope === "plugin") {
      if (typeof value !== "number")
        throw new OxitoneError(ErrorCode.InvalidProject, "plugin parameters use physical numeric values");
...
export class InstanceParameter {
  ...
  constructor(
    ...
    private readonly scope: "plugin" | "effectHost",
  ) {
    if (!parameter) throw new OxitoneError(ErrorCode.AutomationTargetInvalid, "parameter ID is empty");
  }
  set(value: number | boolean): void {
    this.instance.set(this.parameter, this.scope, value);
  }
  automate(source: AutomationSource, options: AutomationLaneOptions = {}) {
    return this.instance.automate(this.parameter, this.scope, source, options);
  }
}
