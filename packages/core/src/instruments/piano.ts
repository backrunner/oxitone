...
export interface PianoBank {
  layers: readonly (readonly PianoSample[])[];
...
function regions(bank: PianoBank, soft: boolean): SampleRegion[] {
  const fail = () => {
    throw new OxitoneError(
      ...
      "piano requires 2–8 aligned velocity layers and a valid key range",
    );
  };
  if (
    !bank ||
    !Array.isArray(bank.layers) ||
    bank.layers.length < 2 ||
    bank.layers.length > 8 ||
    !Array.isArray(bank.keyRange) ||
    bank.keyRange.length !== 2 ||
    bank.keyRange.some((k) => !Number.isInteger(k) || k < 0 || k > 127) ||
    bank.keyRange[0] > bank.keyRange[1]
  )
    fail();
  if (
    bank.layers.some(
      (layer) =>
        !Array.isArray(layer) ||
        !layer.length ||
        layer.length * bank.layers.length > 256 ||
        layer.some(
          (s) =>
            !s ||
            !(s.sample instanceof Sample) ||
            (s.gain !== undefined && (!Number.isFinite(s.gain) || s.gain < 0 || s.gain > 4)),
        ),
    )
  )
    fail();
  ...
  if (
    !keys.length ||
    keys.some((k, i) => !Number.isInteger(k) || k < 0 || k > 127 || (i > 0 && k === keys[i - 1])) ||
    layers.some((layer) => layer.length !== keys.length || layer.some((s, i) => s.rootKey !== keys[i]))
  )
    fail();
  ...
  return selected.flatMap((layer, index) =>
    layer.flatMap((s, i) => {
      const low = Math.max(bank.keyRange[0], i ? Math.floor((keys[i - 1]! + s.rootKey) / 2) + 1 : bank.keyRange[0]);
      const high = Math.min(
        bank.keyRange[1],
        i + 1 < keys.length ? Math.floor((s.rootKey + keys[i + 1]!) / 2) : bank.keyRange[1],
      );
      if (low > high) return [];
      return [
        {
          ...
          velocityRange: [
            Math.floor((index * 127) / selected.length) + 1,
            Math.floor(((index + 1) * 127) / selected.length),
          ] as [number, number],
        },
      ];
    }),
...
export function grandPiano(bank: PianoBank, options: MultisamplerOptions = {}) {
  ...
  return multisampler(regions(bank, false), {
...
export function softPiano(bank: PianoBank, options: MultisamplerOptions = {}) {
  ...
  return multisampler(regions(bank, true), {
