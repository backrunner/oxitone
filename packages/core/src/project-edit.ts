...
import { orderEffects } from "./effect-order.js";
...
export function configure(project: Project, input: ProjectEdit): void {
  ...
  const require = <T>(value: T | undefined): T => {
    if (value === undefined) throw new OxitoneError(ErrorCode.EditTargetMissing, "Project edit target no longer exists");
    return value;
  };
  if (edit.kind === "tempo") {
    if (project.automationLanes.some(lane => lane.target.entityId === project.id && lane.target.parameterId === "tempo")) {
      throw new OxitoneError(ErrorCode.EditScopeConflict, "Tempo is automated; edit its automation instead");
    }
    if (project.tempoMap.length !== 1) throw new OxitoneError(ErrorCode.EditScopeConflict, "Edit the tempo map to preserve its tempo changes");
    project.setTempo(edit.bpm); return;
  }
  if (edit.kind === "track") {
  ...
  if (edit.kind === "effectOrder") {
    orderEffects(require(edit.owner === "channel" ? project.channels[edit.index] : project.mixerChannels[edit.index]), edit.order);
    return;
  }
  ...
  if (edit.kind === "effect") {
    ...
    if (edit.slot === undefined) {
      if (!edit.config) throw new OxitoneError(ErrorCode.EditTargetMissing, "Adding an effect requires its configuration");
    ...
    if (!edit.config) { owner.removeEffect(instance); return; }
    ...
    const chain = [...("effectChain" in spec ? spec.effectChain ?? [] : "inserts" in spec ? spec.inserts ?? [] : [])];
  ...
  if (values.pan !== undefined) {
    if ("pan" in owner) owner.pan = values.pan; else owner.balance = values.pan;
  }
