...
export function effect<K extends EffectKind>(
  kind: K,
  parameters?: EffectParameters<K>,
  options: EffectOptions = {},
): EffectRef {
  if (!Object.hasOwn(effectParameterSchemas, kind))
    throw new OxitoneError(ErrorCode.InvalidProject, "unknown built-in effect kind");
  const values = parseAuthoring<Record<string, number | undefined>>(
    effectParameterSchemas[kind],
    parameters === undefined ? {} : parameters,
    "effect.parameters",
  );
  return parseAuthoring(
    effectRefSchema,
    {
      pluginId: effectPluginIds[kind],
      pluginVersion: "1.0.0",
...
export function convolver(
  impulseSampleId: string,
  parameters: EffectParameters<"convolver"> = {},
  options: EffectOptions = {},
): EffectRef {
