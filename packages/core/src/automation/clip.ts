...
export class AutomationClip {
  ...
  constructor(private readonly project: Project, input: AutomationClipSpec) {
    ...
    if (beatFromWire(spec.startBeat) < 0 || spec.durationBeats === undefined || beatFromWire(spec.durationBeats) <= 0) {
      throw new OxitoneError(ErrorCode.InvalidProject, "Automation clips require a nonnegative start and positive duration");
    }
    this.spec = structuredClone(spec);
  }
  get id(): EntityId { return this.spec.id; }
  get laneId(): EntityId { return this.spec.laneId; }
  get trackId(): EntityId { return this.spec.trackId; }
  get startBeat(): number { return beatFromWire(this.spec.startBeat); }
  get durationBeats(): number { return beatFromWire(this.spec.durationBeats!); }
  set durationBeats(value: number) {
    this.project.assertMutable();
    if (!Number.isFinite(value) || value <= 0) throw new OxitoneError(ErrorCode.InvalidProject, "Automation clip duration must be positive");
    this.spec.durationBeats = beatToWire(value); this.project.touch();
  }
  ...
  relocate(track: Track, startBeat: number): void {
    this.project.assertMutable();
    if (!this.project.tracks.includes(track) || track.tempo !== undefined || !Number.isFinite(startBeat) || startBeat < 0) {
  ...
  toSpec(): AutomationClipSpec { return structuredClone(this.spec); }
}
