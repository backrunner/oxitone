...
export function arrange(project: Project, input: ArrangementEdit): void {
  ...
  if (track?.tempo !== undefined)
    throw new OxitoneError(ErrorCode.EditNotRepresentable, "Playlist editing requires a Track following project tempo");
  if (edit.kind === "automation") {
    ...
    if (edit.action === "place") {
      ...
      if (clip.laneId !== lane.id)
        throw new OxitoneError(ErrorCode.EditTargetMissing, "automation clip does not belong to lane");
  ...
  if (edit.action !== "place") {
    if (edit.kind === "pattern") {
      ...
      if (clip.pattern !== require(project.patterns[edit.resource]))
        throw new OxitoneError(ErrorCode.EditTargetMissing, "Pattern clip resource changed");
      if ("track" in edit && clip.track.tempo !== undefined)
        throw new OxitoneError(
          ErrorCode.EditNotRepresentable,
          "Moving or copying a clip requires project tempo at both ends",
        );
      if (edit.action === "remove") clip.track.detachClip(clip);
      else if (edit.action === "enable") clip.enabled(edit.enabled);
      else if (edit.action === "resize") {
        if (clip.track.tempo !== undefined)
          throw new OxitoneError(ErrorCode.EditNotRepresentable, "Resize requires project tempo");
      ...
      if (clip.toSpec().sampleId !== require(project.samples[edit.resource]).id)
        throw new OxitoneError(ErrorCode.EditTargetMissing, "Sample clip resource changed");
      if ("track" in edit && clip.track.tempo !== undefined)
        throw new OxitoneError(
          ErrorCode.EditNotRepresentable,
          "Moving or copying a clip requires project tempo at both ends",
      ...
      if (edit.action === "remove") clip.track.detachSampleClip(clip);
        ...
        if (clip.track.tempo !== undefined)
          throw new OxitoneError(ErrorCode.EditNotRepresentable, "Fit requires project tempo");
