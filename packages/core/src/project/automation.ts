...
export class ProjectAutomation {
  ...
  constructor(private readonly project: Project, private readonly claimId: (prefix: string) => string, private readonly entityIds: () => Set<string>) {}
  /**
  ...
  addAutomationLane(
    ...
    const instances = target.scope ? [...this.project.channels.flatMap(channel => target.scope === "effectHost" ? [...channel.effectInstances] : [channel.instrumentInstance, ...channel.effectInstances]),
      ...this.project.mixerChannels.flatMap(bus => [...bus.effectInstances])] : [];
    validateLaneTarget(this.project.id, target.scope ? new Set(instances.map(instance => instance.id)) : this.entityIds(), this.lanes, target, source);
  ...
  get automationClips(): readonly AutomationClip[] { return [...this.clips]; }
  createAutomationClip(lane: AutomationLane, track: Track, startBeat: number, durationBeats?: number): AutomationClip {
    ...
    if (lane.target.entityId === this.project.id || track.tempo !== undefined || !Number.isFinite(startBeat) || startBeat < 0 || !Number.isFinite(duration) || duration <= 0) {
      throw new OxitoneError(ErrorCode.InvalidProject, "Automation clips require a non-tempo lane, project-time Track and positive duration");
    }
    const clip = new AutomationClip(this.project, { id: this.claimId("acl_"), laneId: lane.id, trackId: track.id, startBeat: beatToWire(startBeat), durationBeats: beatToWire(duration) });
    lane.usePlaylist();
  ...
  removeAutomationLane(lane: AutomationLane): void {
    ...
    if (index < 0) throw new OxitoneError(ErrorCode.EditTargetMissing, "automation lane belongs to another project or has been removed");
    if (this.clips.some(clip => clip.laneId === lane.id)) throw new OxitoneError(ErrorCode.InvalidProject, "remove placements before their automation lane");
    this.lanes.splice(index, 1); this.project.touch();
  }
