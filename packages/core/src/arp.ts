...
export interface ArpVelocityCurve { from: number; to: number }
...
export function arp(notes: Pattern | readonly (Pitch | NoteInput)[], order: ArpOrder, rate: Beat, options: ArpOptions = {}): Pattern {
  const { seed, id, name, ...music } = options;
  if (seed !== undefined && ((typeof seed === "number" && !Number.isSafeInteger(seed)) ||
    seed < 0 || BigInt(seed) > 0xffff_ffff_ffff_ffffn)) {
    throw new OxitoneError(ErrorCode.InvalidProject, "arp seed must be a safe integer or u64 bigint");
  }
  const input = notes instanceof Pattern ? patternSourceOf(notes) : createSource({ kind: "literal", lengthBeats: 1,
    notes: notes.map((note) => ({ pitch: typeof note === "number" ? note : note.pitch, start: 0, duration: 1, velocity: 1 })),
  });
  return patternFromSource(createSource({ kind: "arp", input: 0, order, rate,
    options: { ...music, ...(seed === undefined ? {} : { seed: String(seed) }) },
  }, [input]), { ...(id === undefined ? {} : { id }), ...(name === undefined ? {} : { name }) });
}
