...
it("hydrates Track M/S independently of shared Channels and rejects partial invalid edits", () => {
  const p = new Project(), c = p.addChannel(), a = p.addTrack().use(c), b = p.addTrack().use(c);
  p.configure({ kind: "track", index: 0, mute: true, solo: true });
  expect(a.toSpec()).toMatchObject({ mute: true, solo: true });
  expect(b.mute).toBe(false); expect(c.mute).toBe(false); expect(c.solo).toBe(false);
  ...
  const restored = Project.fromSnapshot(before), track = restored.tracks[0]!;
  expect(track.mute).toBe(true); expect(track.solo).toBe(true);
  track.mute = false; track.solo = false;
  expect(track.toSpec().mute).toBeUndefined(); expect(track.toSpec().solo).toBeUndefined();
});

it("validates mixer values before mutation and preserves untouched effect identities", () => {
  const p = new Project(), c = p.addChannel(), b = p.addMixerChannel();
  const bound = c.addEffect(effect("delay", { feedback: .3 }));
  const removed = c.addEffect(effect("delay"));
  bound.param("feedback").automate(createAutomationNamespace().constant(.4));
  const before = p.snapshot();
  expect(() => p.configure({ kind: "channel", index: 0, values: { level: .4, pan: 2 } })).toThrow();
  expect(p.snapshot()).toEqual(before);
  p.configure({ kind: "channel", index: 0, values: { level: .6, pan: -.4, mute: true, solo: true } });
  expect(c.toSpec()).toMatchObject({ level: .6, pan: -.4, mute: true, solo: true });
  p.configure({ kind: "bus", index: p.mixerChannels.indexOf(b), values: { level: .8, pan: .3 } });
  expect(b.toSpec()).toMatchObject({ level: .8, balance: .3 });
  const configured = p.snapshot();
  expect(() => p.configure({ kind: "effect", owner: "channel", index: 0, slot: 0, config: effect("reverb") })).toThrow();
...
it("copies all placement options and moves exclusive ends without reordering same-track clips", () => {
  const p = new Project(), t = p.addTrack(), other = p.addTrack();
  const pattern = new Pattern({ lengthBeats: 4, notes: [{ pitch: 60, start: 0, duration: 1, velocity: .8 }] });
  const clip = t.pattern(pattern).at({ bar: 1 }).last({ bar: 3 }).transpose(12).velocityScale(.7).probability(.5).enabled(false);
  ...
  expect(copy).toEqual({ ...clip.toSpec(), id: copy.id, trackId: other.id, startBeat: { numerator: 20, denominator: 1 }, lastBeat: { numerator: 28, denominator: 1 } });
...
it("copies sample settings and automation enable state, fits duration, and rejects mixed Track clocks", () => {
  const p = new Project(), t = p.addTrack(), destination = p.addTrack();
  const sample = p.addSample({ assetUri: "sample.wav", sha256: "00".repeat(32), format: "wav", sampleRate: 48000, channels: 2, frames: 48000 });
  const clip = t.sample(sample).at({ bar: 1 }, { durationBeats: 4, tempoSync: "repitch", gain: .6, pan: -.2, rate: 1.25, enabled: false });
  ...
  expect(copy.toSpec()).toEqual({ ...original, id: copy.id, trackId: destination.id, startBeat: { numerator: 8, denominator: 1 } });
  p.arrange({ action: "fitSample", kind: "sample", resource: 0, clip: 1, durationBeats: 2 });
  expect(copy.durationBeats).toBe(2); expect(copy.tempoSync).toBe("repitch");
  expect(clip.toSpec()).toEqual(original);
  t.tempo = 80;
  const before = p.snapshot();
  expect(() => p.arrange({ action: "move", kind: "sample", resource: 0, clip: 0, track: 1, startBeat: 4 })).toThrow(/both ends/);
  expect(p.snapshot()).toEqual(before);
  const lane = p.addAutomationLane({ entityId: p.addChannel().id, parameterId: "level" }, createAutomationNamespace().constant(.5));
  const automation = p.createAutomationClip(lane, destination, 0, 4); automation.enabled = false;
  ...
  expect(p.automationClips[1]!.toSpec()).toMatchObject({ enabled: false, durationBeats: { numerator: 2, denominator: 1 } });
