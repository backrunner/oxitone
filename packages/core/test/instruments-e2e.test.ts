  ...
  it("renders all helper options and restores slice resources through project files", async () => {
    ...
    try {
      ...
      const channel = project.addChannel({ instrument: wavetable({
        oscA: { wave: "sine", pitch: 0, unison: 2, detune: 8, spread: 0.5 },
        oscB: { wave: "triangle", pitch: -12, unison: 1, detune: 0, spread: 0 }, mix: 0.2,
        ...
        voiceMode: "poly", glide: 0, level: 0.5, pan: 0,
      }) });
      ...
      const sample = project.addSample({ ...info, assetUri: source, frames: BigInt(info.frames), musicalLengthBeats: 0.5 });
      channel.instrument = sampler(sample, { rootKey: 60, loop: "forward", startSeconds: 0,
      ...
      restored.instrument = { ...restored.instrument, state: { ...(restored.instrument.state as object), tempoSync: "stretch" } };
