...
it("registers independent Channel parts atomically and restores their source identity", () => {
  ...
  const melody = new Pattern({ lengthBeats: 4, notes: [{ pitch: 60, start: 0, duration: 1, velocity: 0.8 }] });
  const bass = melody.transpose(-12);
  const pattern = new Pattern({
    lengthBeats: 4,
    parts: [
      { channelId: a.id, pattern: melody },
      { channelId: b.id, pattern: bass },
    ],
  });
