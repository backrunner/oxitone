...
describe("generated source edits", () => {
  it("retains the chord input and deletes an arp step without retiming or recurving", () => {
    ...
    expect(edited.outputs.map((event) => event.origin)).toEqual([
      source.outputs[0]?.origin,
      source.outputs[2]?.origin,
      source.outputs[3]?.origin,
    ]);
  ...
  it("distinguishes original degrees from voiced order and clamped duplicates", () => {
    ...
    expect(source.outputs.map((event) => event.origin)).toEqual([
      { kind: "chord", degree: 3, voice: 0 },
      { kind: "chord", degree: 2, voice: 1 },
      { kind: "chord", degree: 1, voice: 2 },
    ]);
    ...
    expect(
      chord(124, "aug")
        .edit([{ select: { degree: 2 }, set: { pitch: 120 } }])
        .notes.map((n) => n.pitch),
    ).toEqual([124, 120, 127]);
  });
  ...
  it("keeps a shared DAG and scopes repeat and concat edits", () => {
    ...
    const changed = song.edit([
      { select: { iteration: 1, note: { segment: 0, note: { step: 2 } } }, set: { pitch: 70 } },
    ]);
  ...
  it("keeps origins and selectors through moves, transforms and source windows", () => {
    ...
    expect(window.notes.map((n) => [n.start, n.duration])).toEqual([
      [0, 0.5],
      [0.5, 1],
      [1.5, 1],
      [2.5, 0.5],
    ]);
    expect(window.outputs.map((event) => event.origin)).toEqual(
      source.outputs.slice(0, 4).map((event) => event.origin),
    );
  ...
  it("inserts without resizing or altering old notes, and permits later editing", () => {
    ...
    expect(two.edit([{ select: { inserted: 1 }, set: { pitch: 70 } }]).notes.find((n) => n.start === 1.5)?.pitch).toBe(
      70,
    );
  });
  ...
  it("freezes source outputs and isolates options, edit input, and serialized copies", () => {
    ...
    const document = edited.toSource();
  ...
  it("reduces repeated absolute drags to one edit node and one set", () => {
    ...
    expect(source.toSource().nodes.at(-1)).toMatchObject({
      kind: "edit",
      operations: [{ select: { step: 1 }, set: { pitch: 69 } }],
    });
...
describe("source validation", () => {
  it("preserves sequential writes through degree/voice aliases instead of unsafe reduction", () => {
    ...
    expect(
      nested
        .edit([{ select: { iteration: 1, note: { segment: 0, note: { voice: 0 } } }, set: { pitch: 50 } }])
        .notes.filter((n) => n.pitch === 50),
    ).toHaveLength(1);
  });

  it("treats undefined optional edit fields as omitted, including during reduction", () => {
    const edited = chord(60, "major")
      .edit([{ select: { degree: 1 }, set: { pitch: 65, chance: 0.5 } }])
  ...
  it("rejects absent, ambiguous and conflicting edits without changing the source", () => {
    ...
    expect(() => source.edit([{ select: { at: { start: 0, pitch: 60 } }, remove: true }])).toThrowError(
      expect.objectContaining({ code: ErrorCode.EditTargetAmbiguous }),
    );
    expect(source.edit([{ select: { at: { start: 0, pitch: 60 }, occurrence: 1 }, remove: true }]).notes).toHaveLength(
      1,
    );
    const phrase = arp([60], "up", 1);
    expect(() => phrase.edit([{ select: { step: 1 }, remove: true }])).toThrowError(
      expect.objectContaining({ code: ErrorCode.EditTargetMissing }),
    );
    expect(() =>
      phrase.edit([
        { select: { step: 0 }, set: { pitch: 61 } },
        { select: { step: 0 }, remove: true },
      ]),
    ...
    expect(() => phrase.edit([{ select: { step: 0 }, expect: { pitch: 61 }, set: { pitch: 62 } }])).toThrowError(
      expect.objectContaining({ code: ErrorCode.SourceChanged }),
    );
    expect(() => phrase.edit([{ select: { step: 0 }, shift: { pitch: -100 } }])).toThrowError(
  ...
  it("rejects unknown formats, fields, cycles, forward references and unreachable nodes", () => {
    ...
    expect(() => Pattern.fromSource({ ...document, formatVersion: 2 })).toThrowError(
  ...
  it("bounds expansion, source depth and recursive selectors", () => {
    expect(() => chord(60, "major").repeat(100_000)).toThrowError(
      expect.objectContaining({ code: ErrorCode.BudgetExceeded }),
    );
    ...
    const cycle: { iteration: number; note?: unknown } = { iteration: 0 };
    cycle.note = cycle;
    const document: PatternSourceDocument = chord(60, "major").toSource();
    expect(() =>
      Pattern.fromSource({
        ...document,
        nodes: [...document.nodes, { kind: "edit", input: 0, operations: [{ select: cycle, remove: true }] }],
        root: 1,
      }),
    ).toThrowError(expect.objectContaining({ code: ErrorCode.BudgetExceeded }));
