...
const bank = (): PianoBank => {
  const p = new Project();
  return { keyRange: [21, 108], layers: Array.from({ length: 4 }, (_, layer) =>
    [21, 48, 72, 108].map(rootKey => ({ rootKey, sample: p.addSample({
      assetUri: `${rootKey}-${layer}.wav`, format: "wav", sha256: "00".repeat(32),
      frames: 48000, sampleRate: 48000, channels: 2,
    }) }))) };
};
it("maps all 88 keys and 127 velocities without gaps or overlaps and uses softer recorded strikes", () => {
  ...
  const grand = grandPiano(source), soft = softPiano(source);
  for (const instrument of [grand, soft]) {
    const { regions } = multisamplerStateSchema.parse(instrument.state);
    for (let key = 21; key <= 108; key++) for (let v = 1; v <= 127; v++) {
      expect(regions.filter(r => key >= r.keyRange[0] && key <= r.keyRange[1] &&
        v >= r.velocityRange[0] && v <= r.velocityRange[1])).toHaveLength(1);
    }
  }
...
it("rejects mismatched layers and preserves options validation", () => {
  const b = bank(), invalid = expect.objectContaining({ code: "InvalidProject" });
  for (const candidate of [{ ...b, layers: [b.layers[0]!] }, { ...b, keyRange: [108, 21] },
    { ...b, layers: [b.layers[0]!, b.layers[1]!.slice(1)] },
    { ...b, layers: [null, b.layers[0]] }, { ...b, layers: [[null], [null]] },
