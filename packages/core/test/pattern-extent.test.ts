...
it("edits an output and phrase length atomically, preserving its generator and origins", () => {
  ...
  expect(edited.outputs.find((output) => JSON.stringify(output.select) === JSON.stringify(select))!.origin).toEqual(
    base.outputs[0]!.origin,
  );
