...
const source = createAutomationNamespace().constant(0.25);
...
it("creates independent instances from shared config and keeps host and plugin parameters separate", () => {
  const project = new Project();
  const config = effect("delay", { feedback: 0.3 });
  const channel = project.addChannel({
    instrument: wavetable({ level: 0.8 }),
    effectChain: [config, config],
    level: 0.6,
  });
  const [first, second] = channel.effectInstances;
  expect(first!.id).not.toBe(second!.id);
  expect(first).toBe(channel.effectInstances[0]);
  first!.param("feedback").set(0.7);
  second!.host.param("mix").set(0.2);
  second!.host.param("bypass").set(true);
  channel.instrumentInstance.param("level").set(0.4);
  expect(channel.level).toBe(0.6);
  expect(channel.instrument.parameters.level).toBe(0.4);
  expect(channel.effectChain).toMatchObject([
    { parameters: { feedback: 0.7 } },
    { parameters: { feedback: 0.3 }, mix: 0.2, bypass: true },
  ]);
  expect(config.parameters.feedback).toBe(0.3);
  expect(first!.config.toSpec()).not.toHaveProperty("instanceId");
  expect(second!.host.param("mix").automate(source).target).toEqual({
    entityId: second!.id,
    parameterId: "mix",
    scope: "effectHost",
  });
  expect(channel.instrumentInstance.param("level").automate(source).target.entityId).toBe(
    channel.instrumentInstance.id,
  );
...
it("rejects cross-owner/stale permutations, legacy index bindings and identity reuse by replacements", () => {
  const project = new Project();
  const channel = project.addChannel({ effectChain: [effect("delay"), effect("delay")] });
  const [first, second] = channel.effectInstances;
  ...
  expect(() => {
    channel.effectChain = [{ ...channel.effectChain[0]!, pluginVersion: "2.0.0" }, channel.effectChain[1]!];
  }).toThrowError(conflict);
...
it("failed mutations preserve subsequent allocation and musical IDs, and typed snapshots restore exactly", () => {
  ...
  const left = build(),
    right = build();
  expect(() => {
    left.channel.effectChain = [effect("delay")];
  }).toThrowError(conflict);
  left.project.removeAutomationLane(left.lane);
  right.project.removeAutomationLane(right.lane);
  ...
  const noEffects = new Project({ seed: 42 });
  noEffects.addChannel();
  const withEffects = new Project({ seed: 42 });
  withEffects.addChannel({ effectChain: [effect("delay")] });
