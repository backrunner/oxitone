...
it("maps native timbre and modulation controls without changing omitted defaults", () => {
  ...
  const options: WavetableOptions = {
    oscA: { wave: "organ", morphTo: "glass", position: 0.4, phase: 0.25, phaseSpread: 0.7 },
    sub: { level: 0.3, octave: -2 },
    noise: { level: 0.1 },
    lfo: { shape: "triangle", rateHz: 2.5, pitch: 0.1, cutoff: 12, positionA: 0.3, positionB: -0.2, level: 0.4 },
  };
  expect(wavetable(options).parameters).toEqual({
    "oscA.wavetable": 4,
    "oscA.morphTo": 5,
    "oscA.position": 0.4,
    "oscA.phase": 0.25,
...
it("maps independent octave, level, Sub waves and the bounded modulation matrix", () => {
  for (const [index, wave] of (["sine", "triangle", "saw", "square", "pulse", "rounded"] as const).entries()) {
    expect(
      wavetable({ oscA: { octave: -4, level: 0 }, oscB: { octave: 4 }, sub: { wave, octave: 3, level: 0.5 } })
        .parameters,
    ).toEqual({
      "oscA.octave": -4,
      "oscA.level": 0,
  ...
  expect(
    wavetable({
      oscA: { bank: "digital", warpMode: "sync", warp: 0.4, unison: 16 },
      amp: { decayCurve: -0.5 },
      lfo2: { shape: "ramp" },
      macros: [0.8],
      fm: 0.3,
      modulation: [{ source: "macro1", target: "fm", amount: -0.4, curve: 0.2 }],
    }).parameters,
  ).toMatchObject({
    "oscA.bank": 2,
    "oscA.warpMode": 3,
...
it("rejects invalid waveform, unbounded sources and invalid modulation ranges", () => {
  for (const options of [
    ...
    expect(() => wavetable(options as WavetableOptions)).toThrowError(
