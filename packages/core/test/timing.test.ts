...
describe("Rust effective clock authoring queries", () => {
  it("converts seconds across a tempo step at a nonzero starting beat", () => {
    ...
    for (const seconds of [-1, NaN, Infinity])
      expect(() => project.beatsForSeconds(0, seconds)).toThrowError(
  ...
  it("fits through tempo lane loops and holds their final value", () => {
    ...
    const source = createAutomationNamespace().curve(
      [
        { beat: 0, value: value(120) },
        { beat: 0.5, value: value(240) },
        { beat: 1, value: value(60) },
      ],
      "step",
    );
    project.addAutomationLane({ entityId: project.id, parameterId: "tempo" }, source, {
      loop: { lengthBeats: 1, count: 2 },
    });
  ...
  it("rejects excessive bake horizons before allocating the grid", () => {
    ...
    expect(() => project.beatsForSeconds(0, 1e20)).toThrowError(
      expect.objectContaining({ code: ErrorCode.TempoMapComplexity }),
    );
  });
});
