...
describe("built-in instrument authoring", () => {
  it("maps named controls and leaves omitted native defaults intact", () => {
    ...
    expect(
      wavetable({
        oscA: { wave: "triangle", unison: 3 },
        oscB: { wave: "sine", pitch: 12 },
        filter: { type: "bandpass", cutoff: 1000 },
        filterEnvelope: { attack: 0.2, amount: 12 },
        voiceMode: "legato",
        mix: 0.5,
      }).parameters,
    ).toEqual({
      ...
      "filter.type": 2,
      "filter.cutoff": 1000,
      "filterEnv.attack": 0.2,
      "filterEnv.amount": 12,
      voiceMode: 2,
      "osc.mix": 0.5,
    });
    const asset = sample();
    expect(sampler(asset, { rootKey: 36, loop: "forward", startSeconds: 0.25 }).resources).toEqual({
      sample: asset.id,
    });
    expect(sampler(asset, { rootKey: 36, loop: "forward", startSeconds: 0.25 }).parameters).toEqual({
      rootKey: 36,
      loop: 1,
      start: 0.25,
    });
  ...
  it("creates portable immutable slice state with precise frame and beat markers", () => {
    ...
    const options: SlicerOptions = {
      slices: [
        { start: { frames: 0n }, end: { beat: 0.25 }, reverse: true },
        { start: { frames: 0xffff_ffff_ffff_fffen } },
      ],
      tempoSync: "repitch",
      level: 0.5,
    };
    const instrument = slicer(asset, options);
    expect(instrument.state).toEqual({
      sampleId: asset.id,
      tempoSync: "repitch",
      playMode: "oneshot",
      slices: [
        { start: { frames: "0" }, end: { beat: { numerator: 1, denominator: 4 } }, reverse: true },
    ...
    expect(slicer(asset, { slices: { onset: { algorithm: "onset-v1" } } }).state).toHaveProperty(
      "slices.onset.algorithm",
      "onset-v1",
    );
  });
  ...
  it("rejects unknown controls, invalid values and unsafe frame numbers with stable errors", () => {
    ...
    for (const options of [
      ...
      { slices: [] },
      { slices: [null] },
      { slices: { grid: 2 }, tempoSync: "stretch" },
      { slices: [{ start: { frames: 0, beat: 0 } }] },
    ]) {
  ...
  it("replaces a channel instrument once and preserves snapshots on failed edits", () => {
    ...
    expect(() => {
      channel.instrument = { ...instrument, parameters: { level: NaN } };
