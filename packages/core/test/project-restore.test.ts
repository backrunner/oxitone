...
const fixture = (): ProjectSnapshot =>
  JSON.parse(
    readFileSync(new URL("../../../schemas/fixtures/project-snapshot.canonical.json", import.meta.url), "utf8"),
  );
...
describe("editable project restoration", () => {
  it("preserves exact rational values, note order/IDs, plugin state, sends and implicit Master", () => {
    ...
    snapshot.channels[1]!.instrument = {
    ...
    const migrated = project.snapshot();
    const legacy = structuredClone(migrated);
    const refs = legacy.channels
      .flatMap((channel) => [channel.instrument, ...channel.effectChain])
      .concat(legacy.mixerChannels.flatMap((bus) => bus.inserts));
    expect(new Set(refs.map((ref) => ref.instanceId)).size).toBe(refs.length);
    expect(refs.every((ref) => typeof ref.instanceId === "string")).toBe(true);
    refs.forEach((ref) => {
    ...
    expect(project.tracks[0]!.clips[0]!.pattern.notes.map((note) => note.id)).toEqual(
      original.patterns[0]!.notes.map((note) => note.id),
    );
    snapshot.patterns[0]!.notes.reverse();
    (snapshot.channels[1]!.instrument.state as { slices: { grid: number } }).slices.grid = 8;
    expect(project.snapshot()).toEqual(migrated);
    ...
    expect(edited.mixerChannels).toEqual(migrated.mixerChannels);
    ...
    track
      .add(
        new Pattern({
          id: "pat_kept",
          lengthBeats: 1,
          notes: [{ id: "not_kept", pitch: 60, start: 0, duration: 1, velocity: 1 }],
        }),
      )
      .at({ bar: 1 });
      ...
      newTrack
        .add(
          new Pattern({
            id: "pat_bad",
            lengthBeats: 1,
            notes: [{ id: "not_kept", pitch: 61, start: 0, duration: 1, velocity: 1 }],
          }),
        )
        .at({ bar: 1 }),
  ...
  it("keeps maximum u64 revisions readable and rejects edits without mutation", () => {
    ...
    const operations = [
      () => restored.setTempo(80),
      () => restored.addChannel(),
      () => restored.addTrack(),
      () => {
        restored.master.level = 0.5;
      },
      () => {
        restored.channels[0]!.pan = 0.2;
      },
      () => {
        restored.tracks[0]!.tempo = 90;
      },
      () => restored.tracks[0]!.clips[0]!.transpose(12),
      () => {
  ...
  it("rejects malformed snapshots, duplicate IDs, inconsistent ownership and invalid edits", () => {
    ...
    const changes: ((snapshot: ProjectSnapshot) => void)[] = [
      (s) => {
        s.protocolVersion = "2.0";
      },
      (s) => {
        s.tracks[0]!.id = s.tracks[1]!.id;
      },
      (s) => {
        s.patterns[0]!.notes[0]!.id = s.channels[0]!.id;
      },
      (s) => {
        s.tracks[0]!.patternClipIds = [];
      },
      (s) => {
        s.tracks[1]!.patternClipIds.push(s.patternClips[0]!.id);
      },
      (s) => {
        s.patternClips[0]!.patternId = "pat_missing";
      },
      (s) => {
        s.samples[0]!.edits = { startFrame: s.samples[0]!.frames };
      },
      (s) => {
        s.sampleClips[0]!.durationBeats = beatToWire(0);
      },
      (s) => {
        s.patternClips[0]!.durationBeats = beatToWire(0);
      },
      (s) => {
        s.channels[0]!.instrument.resources = { sample: "smp_missing" };
      },
      (s) => {
        s.id = "mix_master";
      },
    ...
    try {
      Project.fromSnapshot({ ...fixture(), protocolVersion: "2.0" });
    } catch (error) {
      expect(error).toMatchObject({ code: ErrorCode.ProtocolVersionUnsupported });
    }
  });
  ...
  it("preserves audible WAV and seeded MIDI output across restore and later edits", async () => {
    ...
    try {
      ...
      expect((await restored.renderWav({ path: after, end: { seconds: 1 }, tailSeconds: 0 })).files[0]!.peakDbfs).toBe(
        -144,
      );
    } finally {
      dispose(engine);
