...
describe("editable project restoration", () => {
  it("preserves exact rational values, note order/IDs, plugin state, sends and implicit Master", () => {
    ...
    const refs = legacy.channels.flatMap(channel => [channel.instrument, ...channel.effectChain]).concat(legacy.mixerChannels.flatMap(bus => bus.inserts));
    ...
    refs.forEach(ref => { delete ref.instanceId; });
