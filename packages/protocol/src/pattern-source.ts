...
export const sourceNoteSchema = z.strictObject({
  pitch, start: beat, duration: positive, velocity: unit,
  offVelocity: unit.optional(), chance: unit.optional(), voice: index.optional(),
...
export const noteSelectorSchema: z.ZodType<NoteSelector> = z.lazy(() => z.union([
  z.strictObject({ step: index }),
  z.strictObject({ degree: z.number().int().min(1).max(3) }),
  z.strictObject({ voice: index }),
  z.strictObject({ at: z.strictObject({ start: beat, pitch, voice: index.optional() }), occurrence: index.optional() }),
...
export const noteEditSchema = z.union([
  ...
  z.strictObject({ select: noteSelectorSchema, expect: patch.optional(), shift: z.strictObject({
    pitch: z.number().int().optional(), start: z.number().finite().optional(),
    duration: z.number().finite().optional(), velocity: z.number().finite().optional(),
  }) }),
...
export const chordSourceOptionsSchema = z.strictObject({
  inversion: index.optional(), voicing: z.enum(["close", "open"]).optional(),
  duration: positive.optional(), velocity: unit.optional(), start: beat.optional(), lengthBeats: positive.optional(),
});
export const arpSourceOptionsSchema = z.strictObject({
  seed: z.string().regex(/^(0|[1-9][0-9]{0,19})$/).optional(),
  gate: positive.max(1).optional(), octaves: z.number().int().min(1).max(PATTERN_SOURCE_LIMITS.events).optional(),
  velocity: unit.optional(), velocityCurve: z.strictObject({ from: unit, to: unit }).optional(),
...
export const patternSourceNodeSchema = z.discriminatedUnion("kind", [
  z.strictObject({ kind: z.literal("literal"), notes: z.array(sourceNoteSchema).max(PATTERN_SOURCE_LIMITS.events), lengthBeats: positive }),
  z.strictObject({ kind: z.literal("chord"), root: pitch, quality: z.enum(["major", "minor", "dim", "aug", "sus2", "sus4"]), options: chordSourceOptionsSchema }),
  z.strictObject({ kind: z.literal("arp"), input: index, order: z.enum(["up", "down", "upDown", "random"]), rate: positive, options: arpSourceOptionsSchema }),
  z.strictObject({ kind: z.literal("concat"), inputs: z.array(index).min(1).max(PATTERN_SOURCE_LIMITS.nodes) }),
  z.strictObject({ kind: z.literal("repeat"), input: index, count: z.number().int().min(1).max(PATTERN_SOURCE_LIMITS.events) }),
  ...
  z.strictObject({ kind: z.literal("edit"), input: index, operations: z.array(noteEditSchema).max(PATTERN_SOURCE_LIMITS.events), lengthBeats: positive.optional() }),
]);
