...
export const presetSchema = z.discriminatedUnion("kind", [
  instrumentRefSchema.extend({ ...common, kind: z.literal("instrument") }),
  effectRefSchema.extend({ ...common, kind: z.literal("effect") }),
  z.object({ ...common, kind: z.literal("channel"), instrument: instrumentRefSchema,
    effectChain: z.array(effectRefSchema), level: z.number().finite().min(0).max(2),
