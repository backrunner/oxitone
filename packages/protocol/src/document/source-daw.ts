...
import {
  configurationEditSchema,
  configurationSiteSchema,
  effectOwnerSiteSchema,
  rackSiteSchema,
  rackMaterializationReviewSchema,
...
export const arrangementOrderSchema = z.object({
  patterns: z.array(z.string()),
  tracks: z.array(z.string()),
  channels: z.array(z.string()),
  mixerChannels: z.array(z.string()),
  samples: z.array(z.string()),
  automation: z.array(z.string()),
  patternClips: z.array(z.string()),
  sampleClips: z.array(z.string()),
  automationClips: z.array(z.string()).default([]),
});
...
export const patternSiteSchema = z.object({
  ...
  expression: z.string(),
  label: z.string(),
  scope: z.enum(["definition", "reference"]),
...
export const automationSiteSchema = z.object({
  handle,
  fileName: z.string(),
  expression: z.string(),
  label: z.string(),
  scope: z.enum(["definition", "reference"]),
  lanes: z.array(handle).max(100_000),
  clips: z.array(handle).max(100_000),
  source: automationSourceSchema,
});
export type AutomationSite = z.infer<typeof automationSiteSchema>;
export const automationRangeEditSchema = z.object({
  start: z.number().finite().nonnegative(),
  end: z.number().finite().positive(),
  fadeBeats: z.number().finite().nonnegative().optional(),
  points: z
    .array(
      z.object({
...
export const materializationReviewSchema = z.object({
  ...
  beforeText: z.string(),
  afterText: z.string(),
  affectedClips: z.array(handle).max(100_000),
  beforeNotes: revision,
  afterNotes: revision,
  losesGeneratorLink: z.literal(true),
  retainsDependencyImports: z.literal(true),
...
export const documentViewSchema = z.object({
  ...
  sessionId: handle,
  revision,
  acceptedRevision: z.number().int().min(-1),
  savedRevision: z.number().int().min(-1),
  status: z.enum(["building", "ready", "invalid", "conflict", "closed"]),
  modified: z.boolean(),
  saving: z.boolean(),
  ...
  conflicts: z
    .array(z.object({ path: z.string(), baseline: z.string(), disk: z.string(), diskHash: z.string().length(64) }))
    .max(4096),
...
const metadata = {
  documentProtocolVersion: z.literal(DOCUMENT_PROTOCOL_VERSION),
  sessionId: handle,
  requestId: handle,
  baseRevision: revision,
};
export const documentRequestSchema = z.object({
  ...
  operation: z.discriminatedUnion("kind", [
    ...
    z.object({
      ...
      plugin: z.string(),
      owner: z.string(),
      target: z.enum(["instrument", "channelInsert", "busInsert"]),
    ...
    z.object({
      ...
      site: handle,
      placement: handle.optional(),
      edits: z.array(noteEditSchema).max(100_000),
    }),
    z.object({
      kind: z.literal("automationRange"),
      site: handle,
      lane: handle.optional(),
      clip: handle.optional(),
      edit: automationRangeEditSchema,
    }),
    z.object({
      kind: z.literal("configuration"),
      site: handle,
      usage: handle.optional(),
      edit: configurationEditSchema,
    }),
    ...
    z.object({
      kind: z.literal("planMaterialize"),
      site: handle,
      placement: handle.optional(),
      edits: z.array(noteEditSchema).max(100_000),
    }),
    ...
    z.object({
      kind: z.literal("resolveConflict"),
      fileName: z.string(),
      diskHash: z.string().length(64),
      resolution: z.enum(["use-disk", "keep-draft", "merge"]),
    }),
    z.object({ kind: z.literal("undo") }),
    z.object({ kind: z.literal("redo") }),
    z.object({ kind: z.literal("save") }),
    z.object({ kind: z.literal("query") }),
    z.object({ kind: z.literal("refreshPlugins") }),
    z.object({ kind: z.literal("verifyPlugin"), plugin: handle }),
    z.object({
      kind: z.literal("installPlugin"),
      packageName: z.string().min(1).max(256),
      version: z.string().min(1).max(128).optional(),
    }),
    z.object({
      kind: z.literal("upgradePlugin"),
      packageName: z.string().min(1).max(256),
      version: z.string().min(1).max(128).optional(),
    }),
