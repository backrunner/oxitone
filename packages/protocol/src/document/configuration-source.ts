...
export const configurationUsageSchema = z.object({
  handle: z.string(),
  owner: z.string(),
  label: z.string(),
...
export const configurationSiteSchema = z.object({
  handle: z.string(),
  fileName: z.string(),
  expression: z.string(),
  label: z.string(),
  ...
  usages: z.array(configurationUsageSchema).max(100_000),
  config: instrumentRefSchema.merge(effectRefSchema),
});
...
export const configurationEditSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    kind: z.literal("host"),
    values: z.object({ mix: z.number().finite().min(0).max(1).optional(), bypass: z.boolean().optional() }).strict(),
  }),
]);
export type ConfigurationEdit = z.infer<typeof configurationEditSchema>;
export const effectOwnerSiteSchema = z.object({
  handle: z.string(),
  fileName: z.string(),
  expression: z.string(),
  label: z.string(),
...
export const rackSiteSchema = z.object({
  handle: z.string(),
  fileName: z.string(),
  expression: z.string(),
  label: z.string(),
  scope: z.enum(["definition", "reference"]),
  usages: z.array(rackUsageSchema).max(100_000),
  effects: z.array(effectRefSchema).max(4096),
});
export type RackSite = z.infer<typeof rackSiteSchema>;
export const rackMaterializationReviewSchema = z.object({
  planId: z.string(),
  baseRevision: z.number().int().nonnegative(),
  fileName: z.string(),
  beforeText: z.string(),
  afterText: z.string(),
  affectedOwners: z.array(z.string()),
