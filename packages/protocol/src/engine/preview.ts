...
export const previewResponseSchema = z.discriminatedUnion("type", [
  z.object({
    ...
    revision: frameWireSchema.nullable(),
    seenRevision: frameWireSchema.nullable(),
    cursor: frameWireSchema,
    audibleFrame: frameWireSchema,
    playing: z.boolean(),
    xruns: z.number().int().nonnegative(),
    pluginFaults: z.number().int().nonnegative(),
    tracks: z.number().int().nonnegative(),
    patterns: z.number().int().nonnegative(),
    documentProtocolVersion: z.literal("2.0").optional(),
