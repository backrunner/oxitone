...
export const drumInstrument = (): InstrumentRef => ({
  pluginId: "example.drums",
  pluginVersion: "1.0.0",
  parameters: { volume: 0.85, decay: 1 },
});
...
function drumBar(bar: number): Pattern {
  ...
  const hit = (pitch: number, start: number, velocity: number) =>
    notes.push({ pitch, start, velocity, duration: 0.08 });
  if (bar === 15) {
    hit(36, 0, 1);
    hit(46, 0, 0.7);
  } else {
...
export function createDrumSong(): Project {
  ...
  const drums = project.addChannel({
    name: "Native drum machine",
    instrument: drumInstrument(),
    level: 1,
    effectChain: [referenceGain],
  });
  const drumTrack = project.addTrack("Drums · MIDI 10").use(drums);
  drumTrack.midiChannel = 10;
  const bass = project.addChannel({
    name: "Round bass",
    level: 0.65,
    instrument: wavetable({
      oscA: { wave: "triangle" },
      filter: { cutoff: 700, resonance: 0.12 },
  ...
  const keys = project.addChannel({
    ...
    effectChain: [
      { pluginId: "oxitone.delay", pluginVersion: "1.0.0", parameters: { timeBeats: 0.75, feedback: 0.25 }, mix: 0.2 },
    ],
  });
  const keyTrack = project.addTrack("Am7 · Fmaj7 · Cmaj7 · G6").use(keys);
  const lead = project.addChannel({
    name: "Bell motif",
    level: 0.45,
    pan: 0.22,
    instrument: wavetable({
    ...
    effectChain: [
      { pluginId: "oxitone.delay", pluginVersion: "1.0.0", parameters: { timeBeats: 0.5, feedback: 0.3 }, mix: 0.25 },
    ],
  });
  ...
  for (let bar = 0; bar < 16; bar++) {
    ...
    if (bar >= 2 && bar !== 8) {
      ...
      bassTrack
        .add(
          new Pattern({
            lengthBeats: 4,
            notes: beats.map((start, i) => ({
              pitch: root + (i === 4 ? 12 : 0),
              start,
              duration: bar === 15 ? 1.5 : 0.45,
              velocity: 0.78,
            })),
          }),
        )
    ...
    keyTrack
      .add(
        new Pattern({
          lengthBeats: 4,
          notes: starts.flatMap((start) =>
            chord.map((pitch) => ({
              pitch,
              start,
              duration: bar === 15 ? 1.5 : 0.8,
              velocity: bar < 2 ? 0.48 : 0.62,
            })),
          ),
        }),
      )
        ...
        .add(
          new Pattern({
            ...
            notes: melody.map((pitch, i) => ({
              pitch,
              start: [0.5, 1.25, 2.5, 3.25][i]!,
              duration: 0.35,
              velocity: i % 2 ? 0.55 : 0.7,
            })),
          }),
        )
        .at({ bar: bar + 1 });
