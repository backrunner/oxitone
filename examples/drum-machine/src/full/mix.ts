...
export function createMix(p: Project, bank: PianoBank) {
  ...
  const keys = p.addChannel({ name: "Soft Piano · intimate intro", instrument: softPiano(bank), mixerChannelId: keysBus.id, level: 0.9, effectChain: [hp(150), lp(8000)] });
  const ornaments = p.addChannel({ name: "Soft Piano · quiet ornament", instrument: softPiano(bank, { velocitySensitivity: 0.62 }),
    mixerChannelId: keysBus.id, level: 0.5, effectChain: [hp(250), lp(6200)] });
  const grand = p.addChannel({ name: "Grand Piano · open breakdown", instrument: grandPiano(bank), mixerChannelId: keysBus.id, level: 0.52, effectChain: [hp(180)] });
  ...
  const pulse = p.addChannel({ name: "Sky · upper pulse texture", instrument: pulseChords(), mixerChannelId: synthBus.id, level: 0.48,
    effectChain: [hp(1200), effect("spreader", { width: 1.15, amount: 0.12, bassMonoHz: 1000 })] });
  const edge = p.addChannel({ name: "Sky · distorted chord edge", instrument: chordEdge(), mixerChannelId: music.id, level: 0.43,
  ...
  const inputDb: [number, number][] = [[0, 16], [95.08, 16], [95.35, 8],
    [160, 8], [161, 12], [220, 12], [224, 16], [287.08, 16], [287.35, 8],
    [352, 8], [353, 10], [382, 10], [384, 14], [416, 14]];
  p.master.automate("insert.4.parameter.inputDb", automation.polyline(
    inputDb.map(([beat, db]) => ({ beat, value: (db + 12) / 36 }))));
  return { keys, ornaments, saw, body, pulse, edge, sub, growl, vowel, lead, air, sparkle, arp, lift, fall,
    pluck, reese, shimmer, grand, foundation, laser, percussion, subBus, lowBus, music, hall, echo, throws, leadBus };
}
