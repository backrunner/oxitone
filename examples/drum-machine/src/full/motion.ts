...
const curve = (points: Map<number, number>) =>
  automation.polyline([...points].sort(([a], [b]) => a - b).map(([beat, value]) => ({ beat, value })));
...
export function applyMotion(mix: Mix, hits: number[], chordAttacks: number[], kicks: number[]) {
  ...
  for (const [bus, floor] of [
    ...
    for (const [i, beat] of ordered.entries()) {
      ...
      points.set(beat, floor);
      points.set(beat + recovery * (snare ? 0.32 : 0.2), floor + 0.02);
      points.set(beat + recovery * 0.58, 0.36);
      points.set(beat + recovery, 0.5);
    }
    for (const end of [96, 288]) {
      points.set(end - 1.2, 0.5);
      points.set(end - 0.9, 0);
  ...
  for (const [bus, floor] of [
    ...
    for (const beat of kicks) {
      points.set(beat - 0.012, 0.5);
      points.set(beat, floor);
      points.set(beat + 0.075, floor);
      points.set(beat + 0.18, 0.35);
      points.set(beat + 0.28, 0.5);
    }
    bus.automate("level", curve(points));
  }
  ...
  for (const beat of chordAttacks) {
    ...
    brightness.set(beat, hz(open * 0.55));
    brightness.set(beat + 0.08, hz(open));
  ...
  mix.air.automate(
    "filter.cutoff",
    automation.polyline([
      { beat: 0, value: hz(1500) },
      { beat: 32, value: hz(1400) },
      { beat: 94, value: hz(7200) },
      { beat: 96, value: hz(2800) },
      { beat: 160, value: hz(1100) },
      { beat: 224, value: hz(1800) },
      { beat: 286, value: hz(8300) },
      { beat: 288, value: hz(3200) },
      { beat: 352, value: hz(2400) },
  ...
  mix.lift.automate(
    ...
    automation.polyline([
      { beat: 0, value: hz(800) },
      { beat: 48, value: hz(800) },
      { beat: 94, value: hz(13000) },
      { beat: 96, value: hz(800) },
      { beat: 240, value: hz(800) },
      { beat: 286, value: hz(14500) },
      { beat: 288, value: hz(800) },
    ]),
  );
  // Release the roll and pad before the downbeat; the new kick arrives into a pocket.
  const liftGain = new Map<number, number>([[0, 0.095]]),
    padGain = new Map<number, number>([[0, 0.08]]);
  for (const end of [96, 288]) {
    liftGain.set(end - 2, 0.095);
    liftGain.set(end - 0.9, 0);
    liftGain.set(end, 0);
    liftGain.set(end + 0.01, 0.095);
    padGain.set(end - 2, 0.08);
    padGain.set(end - 0.9, 0);
    padGain.set(end, 0);
    padGain.set(end + 0.25, 0.08);
  }
  mix.lift.automate("level", curve(liftGain));
  mix.air.automate("level", curve(padGain));
  const throws = new Map<number, number>([[0, 0]]);
  for (const bar of [31, 39, 79, 87]) {
    const beat = bar * 4;
    throws.set(beat + 3.2, 0);
    throws.set(beat + 3.25, 0.26);
    throws.set(beat + 3.85, 0.26);
    throws.set(beat + 3.98, 0);
  }
