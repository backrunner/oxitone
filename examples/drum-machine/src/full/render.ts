...
for (const [song, create] of [[dubstep, createDubstepSong]] as const) {
  ...
  try {
    ...
    const result = renderWav(engine, snapshot, {
      path: join(outputRoot, `${song.slug}.wav`),
      end: { beat: beatToWire(song.bars * 4) },
      tailSeconds: 3,
      bitDepth: 24,
      dither: "tpdf",
    });
    ...
    const file = result.files[0]!,
      expectedSeconds = (song.bars * 4 * 60) / song.bpm + 3;
    ...
    assert(
      sectionLevels.every((s) => s.rmsDbfs > -52),
      "Silent arrangement section",
    );
    assert(
      sectionLevels.every((s) => Math.abs(s.dc) < 0.002 && s.stereoCorrelation > 0),
      "DC or stereo cancellation regression",
    );
    ...
    assert(
      impacts.every((i) => i.arrivalLiftDb! > 12),
      "Pre-drop gap no longer clears the downbeat",
    );
    assert(
      impacts.every((i) => i.snareMedianMidLiftDb > 0),
      "Snare sustain is buried in the drop mix",
    );
    const diagnostics = getPluginDiagnostics(engine);
    assert(
      diagnostics.every((d) => d.faults === 0),
      "Native plugin fault",
    );
    ...
    reports.push({
      ...
      renderSeconds,
      realtimeMultiple: expectedSeconds / renderSeconds,
    ...
    console.log(
      `${song.title}: ${file.durationSeconds.toFixed(2)} s, ${file.integratedLufs.toFixed(2)} LUFS, ${file.truePeakDbfs.toFixed(2)} dBTP`,
    );
  } finally {
    dispose(engine);
  }
  ...
  `${JSON.stringify(
    {
      ...
      blockSize: 128,
      instruments:
        "Oxitone synthesis / Circuit electronic drums / Salamander Grand Piano v3 (Alexander Holm, CC BY 3.0)",
      songs: reports,
    },
    null,
    2,
