...
export function inspectImpact(path: string, bpm = 140, startBars: readonly number[] = [24, 72], bars = 16) {
  ...
  while (offset + 8 <= wav.length) {
    ...
    if (kind === "fmt ") {
      assert.equal(wav.readUInt32LE(offset + 12), 48000);
  ...
  const window = (beat: number, from: number, to: number) => {
    ...
    const low = [0, 0],
      high = [0, 0],
      a = 1 - Math.exp((-2 * Math.PI * 150) / 48000),
      b = 1 - Math.exp((-2 * Math.PI * 6000) / 48000);
    for (let i = Math.max(0, start - 2400); i < end; i++)
      for (let ch = 0; ch < 2; ch++) {
        const v = wav.readIntLE(data + i * 6 + ch * 3, 3) / 8388608;
        low[ch] = low[ch]! + a * (v - low[ch]!);
        high[ch] = high[ch]! + b * (v - high[ch]!);
        if (i >= start) {
          energy += v * v;
          middle += (high[ch]! - low[ch]!) ** 2;
          peak = Math.max(peak, Math.abs(v));
        }
      }
    return {
      rmsDbfs: 10 * Math.log10(Math.max(1e-20, energy / ((end - start) * 2))),
  ...
  return startBars.map((startBar) => {
    const hits = Array.from({ length: bars }, (_, i) => {
      const beat = (startBar + i) * 4 + 2,
        before = window(beat, -0.22, -0.035),
        after = window(beat, 0.015, 0.2);
    ...
    return {
      ...
      snareMedianMidLiftDb: (sorted[Math.floor((bars - 1) / 2)]! + sorted[Math.floor(bars / 2)]!) / 2,
      snares: hits,
    };
  });
}
