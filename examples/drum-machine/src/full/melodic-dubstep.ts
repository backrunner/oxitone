...
export function createDubstepSong(piano: (project: Project) => InstrumentRef = uprightPiano): Project {
  ...
  const saw = p.addChannel({ name: "Seven-voice supersaw", mixerChannelId: synthBus.id, level: 0.45,
    instrument: wavetable({ oscA: { wave: "saw", unison: 7, detune: 22, spread: 1 },
      oscB: { wave: "saw", pitch: 12, unison: 3, detune: 14, spread: 0.8 }, mix: 0.14,
      filter: { cutoff: 7500, resonance: 0.08 }, amp: { attack: 0.012, decay: 0.18, sustain: 0.7, release: 0.18 } }),
  ...
  const growl = p.addChannel({ name: "Moving mid bass", mixerChannelId: bassBus.id, level: 0.24,
    instrument: wavetable({ oscA: { wave: "saw", unison: 3, detune: 14, spread: 0.5 },
      oscB: { wave: "square", pitch: 12 }, mix: 0.24, voiceMode: "mono", glide: 0.025,
      filter: { cutoff: 1400, resonance: 0.3 }, amp: { attack: 0.004, decay: 0.16, sustain: 0.62, release: 0.06 } }),
    effectChain: [fx("saturator", { driveDb: 8, outputDb: -5 }, 0.7)] });
  const lead = p.addChannel({ name: "Horizon · saw / triangle", mixerChannelId: leadBus.id, level: 0.34,
    instrument: wavetable({ oscA: { wave: "saw", unison: 5, detune: 11, spread: 0.85 },
      oscB: { wave: "triangle", pitch: 12 }, mix: 0.25, filter: { cutoff: 6500, resonance: 0.06 },
      amp: { attack: 0.008, decay: 0.18, sustain: 0.6, release: 0.2 } }) });
  const air = p.addChannel({ name: "Bloom pad / riser", mixerChannelId: synthBus.id, level: 0.1,
    instrument: wavetable({ oscA: { wave: "saw", unison: 4, detune: 28, spread: 1 },
      filter: { cutoff: 1500, resonance: 0.16 }, amp: { attack: 0.6, decay: 0.2, sustain: 0.65, release: 0.6 } }) });
  ...
  const theme = [[80, 78, 76, 73, 76], [78, 76, 73, 69, 73], [76, 80, 83, 80, 76], [78, 76, 73, 71, 68]];
  for (let b = 0; b < dubstep.bars; b++) {
    ...
    if (!drop && (!build || buildPos < 12) && !final) bar(mt, b,
      theme[index]!.map((pitch, i) => note(pitch - 12, [0, 0.75, 1.5, 2.5, 3.25][i]!,
        i === 4 ? 0.58 : 0.5, end ? 0.46 : 0.64 - i * 0.025)), "Horizon · piano");
    if (drop) {
      const rhythm = b % 2 ? [0, 0.75, 1.5, 2.5, 3.25] : [0, 1.5, 2.5, 3.5];
      bar(ct, b, rhythm.flatMap((t, i) => chord.slice(1).map(pitch => note(pitch + 12, t,
        i === 0 ? 0.62 : 0.38, 0.75))), "Sky chords");
      ...
      bar(lt, b, theme[index]!.map((pitch, i) => note(pitch, [0, 0.75, 1.5, 2.5, 3.25][i]!,
        i === 4 ? 0.62 : 0.52, 0.85 - i * 0.035)), second ? "Horizon · return" : "Horizon · drop");
  ...
  growl.automate("filter.cutoff", automation.sine({ periodBeats: 1, min: 0.37, max: 0.7 }));
