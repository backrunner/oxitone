...
export function inspectSections(path: string, sections: readonly Section[], bpm: number, bars: number) {
  ...
  return sections.map(([name, start], index) => {
    ...
    let energy = 0, peak = 0, leftEnergy = 0, rightEnergy = 0, cross = 0, sum = 0;
    let lowMid = 0, lowSide = 0;
    const filters = [[0, 0, 0], [0, 0, 0]], bands = [0, 0, 0, 0];
    ...
    for (let frame = first; frame < last; frame++) {
      ...
      for (const [ch, value] of lr.entries()) {
        energy += value * value; sum += value; peak = Math.max(peak, Math.abs(value));
      ...
      leftEnergy += lr[0]! ** 2; rightEnergy += lr[1]! ** 2; cross += lr[0]! * lr[1]!;
    ...
    return { name, startBar: start + 1, bars: end - start,
      rmsDbfs, peakDbfs, crestDb: peakDbfs - rmsDbfs, dc: sum / ((last-first)*2),
      ...
      bandRmsDbfs: Object.fromEntries(["below150", "150to1000", "1000to6000", "above6000"]
        .map((label, i) => [label, 10 * Math.log10(Math.max(1e-20, bands[i]! / ((last-first)*2)))])) };
  });
}
