...
export async function preparePianos() {
  ...
  for (let index = 0; index < assets.files.length; index += 4) {
    await Promise.all(assets.files.slice(index, index + 4).map(async file => {
      const path = join(cache, file.file);
      if (existsSync(path) && hash(readFileSync(path)) === file.sha256) return;
      const response = await fetch(`https://raw.githubusercontent.com/sfzinstruments/SalamanderGrandPiano/${assets.commit}/Samples/${encodeURIComponent(file.file)}`,
      ...
      try {
        while (true) {
          ...
          length += value.length; if (length > file.bytes) throw new Error(`Oversized piano recording: ${file.file}`);
          chunks.push(value);
        }
      ...
      try { await writeFile(partial, data); await rename(partial, path); }
...
export function pianoBank(project: Project, range: readonly [number, number] = [48, 84]): PianoBank {
  return { keyRange: range, layers: [2, 5, 9, 13].map(layer => assets.files
    .filter(file => file.layer === layer && file.rootKey >= range[0] - 1 && file.rootKey <= range[1] + 1)
    .map(file => ({ rootKey: file.rootKey, gain: 1.2, sample: project.addSample({
      assetUri: join(cache, file.file), sha256: file.sha256, sampleRate: file.sampleRate,
      channels: file.channels as 1 | 2, frames: BigInt(file.frames), format: "flac",
    }) }))) };
}
