...
export default function createMixerPreview() {
  ...
  const music = project.addMixerChannel({ name: "Music bus", masterSendRatio: 0.82,
    inserts: [{ pluginId: "oxitone.compressor", pluginVersion: "1.0.0", parameters: {} }] });
  const room = project.addMixerChannel({ name: "Room reverb", level: 0.7,
    inserts: [{ pluginId: "oxitone.reverb", pluginVersion: "1.0.0", parameters: {}, mix: 1 }] });
  const echo = project.addMixerChannel({ name: "Tape echo", masterSendRatio: 0.65,
    inserts: [{ pluginId: "oxitone.delay", pluginVersion: "1.0.0",
      parameters: { timeBeats: 0.75, feedback: 0.3 }, mix: 1 }] });
  ...
  music.automate(
    `send.${room.id}.ratio`,
    createAutomationNamespace().sine({ periodBeats: 16, min: 0.18, max: 0.3 }),
  );
  return project;
}
