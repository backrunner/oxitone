...
it("builds a deterministic sampled-piano and electronic arrangement with two developed drops", () => {
  ...
  for (let i = 0; i < dubstep.sections.length; i++) {
    const first = dubstep.sections[i]![1] * 4, last = (dubstep.sections[i + 1]?.[1] ?? dubstep.bars) * 4;
  ...
  const inserts = snapshot.channels.flatMap(c => c.effectChain).concat(snapshot.mixerChannels.flatMap(c => c.inserts));
  for (const id of ["distortion", "multiband-dynamics", "delay", "reverb", "compressor", "limiter",
    "eq", "filter", "convolver", "spreader", "tape", "nonlinear-filter", "compactor"]) {
...
it("preserves every lead note in both drops and develops the surrounding orchestration", () => {
  ...
  const clips = (name: string) => {
    ...
    return s.patternClips.filter(c => c.trackId === track.id).map(c => ({
      bar: beatFromWire(c.startBeat) / 4, pattern: s.patterns.find(p => p.id === c.patternId)!,
    }));
  };
  const leads = clips("Lead · horizon theme");
  expect(leads).toHaveLength(32);
  for (const { bar, pattern } of leads) expect(pattern.notes.map(n => ({
    pitch: n.pitch, start: beatFromWire(n.start), duration: beatFromWire(n.duration), velocity: n.velocity,
  ...
  const bass = clips("Mid bass · syncopation"), vowel = clips("Vowel bass · response");
...
it("applies MIDI channel sharing only to a separate export snapshot", () => {
  const source = createDubstepSong().snapshot(), original = structuredClone(source);
  ...
  expect(midi.tracks.map(({ midiChannel: _midi, ...track }) => track))
    .toEqual(source.tracks);
});
