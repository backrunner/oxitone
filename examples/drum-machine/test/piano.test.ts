...
it("renders soft and grand recorded velocity layers through Rust without a device or downloads", () => {
  const dir = mkdtempSync(join(tmpdir(), "oxitone-piano-")),
    engine = createEngine();
  try {
    const levels = [softPiano, grandPiano].map((instrument, index) => {
      const p = new Project(),
        bank = fixturePiano(dir)(p);
      p.addTrack("Piano")
        .use(p.addChannel({ instrument: instrument(bank) }))
        .add(
          new Pattern({
            lengthBeats: 2,
        ...
        .at({ bar: 1 });
      return renderWav(engine, p.snapshot(), {
        path: join(dir, `${index}.wav`),
        end: { beat: beatToWire(2) },
        tailSeconds: 0,
      }).files[0]!.peakDbfs;
    });
