...
it("layers the half-time backbeat and gives builds a denser roll followed by a pickup gap", () => {
  ...
  const clips = (name: string) => {
    ...
    return s.patternClips.filter(c => c.trackId === track.id).map(c => ({
      bar: beatFromWire(c.startBeat) / 4, notes: s.patterns.find(p => p.id === c.patternId)!.notes,
    }));
  };
  for (const name of ["Half-time snare · crack", "Snare · body layer", "Snare · noise tail layer", "Clap · snare layer"]) {
    const drops = clips(name).filter(c => c.bar >= 24 && c.bar < 40 || c.bar >= 72 && c.bar < 88);
    expect(drops).toHaveLength(32);
    expect(drops.every(c => c.notes.some(n => beatFromWire(n.start) >= 2 && beatFromWire(n.start) < 2.1))).toBe(true);
  }
