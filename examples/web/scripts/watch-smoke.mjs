...
const source = (await readFile(new URL("../src/song.ts", import.meta.url), "utf8")).replace(
  '"@oxitone/web"',
  JSON.stringify(fileURLToPath(new URL("../../../packages/web/src/index.ts", import.meta.url))),
);
...
try {
  const url = await new Promise((resolve, reject) => {
    let log = "";
    const timeout = setTimeout(() => reject(new Error("watch server timed out: " + log)), 30000);
    ...
    server.on("exit", (code) => {
      clearTimeout(timeout);
      reject(new Error("watch server exited " + code + ": " + log));
    });
  });
  browser = await launchSilentBrowser();
  const page = await silentPage(browser);
  await page.goto(url);
  ...
  assert.equal(await gain(), 0.8);
  assert(await active());
  const report = await page.evaluate(() => ({
    validUpdate: true,
    syntaxFailureRetainsGraph: true,
    runtimeFailureRetainsGraph: true,
    recovery: true,
