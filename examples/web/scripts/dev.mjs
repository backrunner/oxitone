...
for (const slug of songs) {
  try {
    const snapshot = JSON.parse(
      await readFile(resolve(root, "target/examples/full-songs", slug + ".snapshot.json"), "utf8"),
    );
    for (const sample of snapshot.samples) assets.set(sample.sha256, resolve(root, sample.assetUri));
  } catch {
    /* Optional full songs are produced by example:songs first. */
  }
}
const types = {
  ".html": "text/html",
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    else if (songs.some((s) => path === `/songs/${s}.snapshot.json`))
      file = resolve(root, "target/examples/full-songs", path.split("/").pop());
    else {
    ...
    res.writeHead(404);
    res.end("Not found; run pnpm build:wasm and pnpm example:songs for optional songs.");
  }
});
