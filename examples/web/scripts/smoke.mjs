...
async function measure() {
  return page.evaluate(async () => {
    ...
    return {
      ...
      diagnostics: window.oxitone.session.diagnostics(),
...
try {
  ...
  assert(initial.peak > 0.01);
  report.songs.push({ song: "glass", ...initial });
  await page.click("#invalid");
  await page.waitForFunction(() => document.querySelector("#status").textContent.includes("rejected"));
  assert((await measure()).peak > 0.01);
  await page.click("#update");
  await page.waitForFunction(() => document.querySelector("#status").textContent.includes("softer"));
  assert((await measure()).peak > 0.01);
  await page.click("#play");
  await page.waitForTimeout(250);
  const paused = await measure();
  assert.equal(paused.peak, 0);
  assert.equal(paused.state.state, "paused");
  await page.evaluate(() => window.oxitone.session.seek(480000));
  const seek = await page.evaluate(() => window.oxitone.session.state());
  assert.equal(seek.cursor, "480000");
  await page.click("#play");
  assert((await measure()).peak > 0.01);
  ...
  if (process.env.OXITONE_WEB_FULL_SONGS === "1") {
    for (const slug of ["after-the-horizon"]) {
      console.log(
        "Loading",
        slug,
        await page.locator("#song option").evaluateAll((options) => options.map((option) => option.value)),
      );
      await page.selectOption("#song", { value: slug });
      await page.waitForFunction(
        () =>
          window.oxitone?.project?.name?.startsWith("After the Horizon") &&
          document.querySelector("#status").textContent === "Ready · Rust engine",
        slug,
        { timeout: 60000 },
      );
      await page.click("#play");
      ...
      report.songs.push({ song: slug, ...result });
    }
  }
  ...
  report.disposed = await page.evaluate(async () => {
    const s = window.oxitone.session;
    await s.dispose();
