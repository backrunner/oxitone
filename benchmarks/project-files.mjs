...
try {
  ...
  source.addTrack().use(source.addChannel()).add(new Pattern({ lengthBeats: 2,
    notes: [{ pitch: 69, start: 0, duration: 2, velocity: 0.5 }],
  })).at({ bar: 1 });
  ...
  for (let i = 0; i < 35; i++) {
    ...
    if (i >= 5) { save.push(saveMs); load.push(performance.now() - start); }
  }
  const stats = (values) => {
    values.sort((a, b) => a - b);
    return { medianMs: (values[14] + values[15]) / 2, p95Ms: values[28], p99Ms: values[29] };
  };
  process.stdout.write(JSON.stringify({ schema: "oxitone.project-file-benchmark.v1",
    date: new Date().toISOString(), machine: { cpu: cpus()[0]?.model, osRelease: release(), arch: process.arch, node: process.version, tempRoot: tmpdir() },
    config: { sampleRate: 48000, blockSize: 128, channels: 2, assets: 1, frames: 48000, bitDepth: "float32", warmupIterations: 5, measurementIterations: 30 },
    save: stats(save), load: stats(load), callbackP95Ns: null, callbackP99Ns: null, xruns: null,
    notes: ["Warm filesystem cache; save revalidates an already-published asset and atomically replaces the manifest, including fsync.",
      ...
      "Control-thread I/O measurement, not realtime callback/worker performance acceptance."] }, null, 2) + "\n");
