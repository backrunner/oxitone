...
const output = resolve(
  process.env.OXITONE_PREVIEW_CAPTURE_OUTPUT ??
    (process.env.OXITONE_PREVIEW_CAPTURE_UI_REVIEW
      ? "target/daw-ui-review.png"
      : configuration
        ? "target/daw-configuration.png"
        : automation
          ? "target/daw-automation.png"
          : patterns
            ? "target/daw-patterns.png"
            : process.env.OXITONE_PREVIEW_CAPTURE_MANAGER
              ? "target/daw-plugins.png"
              : "target/daw-edit.png"),
);
try {
  ...
  if (automation)
    await writeFile(
      ...
      (await readFile(entry, "utf8"))
        ...
        .replace(
          "export default project;",
          "const motion = createAutomationNamespace().sine({ periodBeats: 4 });\nkeys.automate('level', motion); keys.automate('pan', motion);\nexport default project;",
        ),
    );
  ...
  await writeFile(
    process.env.OXITONE_PREVIEW_CAPTURE_OUTPUT
      ? `${output}.log`
      : configuration
        ? "target/daw-configuration-smoke.log"
        : automation
          ? "target/daw-automation-smoke.log"
          : patterns
            ? "target/daw-patterns-smoke.log"
            : process.env.OXITONE_PREVIEW_CAPTURE_MANAGER
              ? "target/daw-manager-smoke.log"
              : "target/daw-smoke.log",
    result.stderr,
  );
  if (!result.stderr.includes("DAW smoke passed:") || !result.stderr.includes("Preview capture saved"))
    throw new Error(result.stderr);
  if (process.env.OXITONE_PREVIEW_CAPTURE_UI_REVIEW && !result.stderr.includes("UI review smoke passed:"))
    throw new Error(result.stderr);
  ...
  if (process.env.OXITONE_PREVIEW_CAPTURE_EDITING && !result.stderr.includes("Editing smoke passed:"))
    throw new Error(result.stderr);
  if (process.env.OXITONE_PREVIEW_CAPTURE_EDITING && !automation && !result.stderr.includes("Piano smoke passed:"))
    throw new Error(result.stderr);
  ...
  if (!saved.includes(".edit(") || saved.includes("__oxitone_project_") || /requestId|sessionId/.test(saved))
    throw new Error("invalid persisted source");
  ...
  try {
    ...
    if (
      ...
      (snapshot?.channels[0]?.level <= 1 ||
        snapshot?.channels[0]?.pan !== 0.25 ||
        !snapshot?.channels[0]?.mute ||
        snapshot?.tempoMap[0]?.bpm !== 132)
    )
      throw new Error("DAW controls did not survive source reopen");
    if (
      process.env.OXITONE_PREVIEW_CAPTURE_CONTROLS &&
      (!snapshot?.tracks[0]?.mute ||
        !snapshot?.tracks[0]?.solo ||
        snapshot?.tracks[1]?.mute ||
        snapshot?.tracks[1]?.solo)
    )
      throw new Error("Track M/S did not survive source reopen independently");
    ...
    if (
      ...
      (selected?.lengthBeats.numerator / selected?.lengthBeats.denominator !== 21 ||
        ...
        !selected?.notes.some((note) => note.velocity > 0.9) ||
        !selected?.notes.some((note) => note.velocity < 0.4))
    ) {
      throw new Error("velocity drawing and phrase extension did not survive source reopen");
    }
    if (
      reopened.view.status !== "ready" ||
      !selected?.notes.some((note) => note.pitch === 61 && note.start.numerator / note.start.denominator === 0.25) ||
    ...
    if (
      automation &&
      (snapshot.automation[0]?.source.kind !== "replaceRange" || snapshot.automation[1]?.source.kind !== "wave")
    )
      throw new Error("automation range did not survive source reopen");
    if (
      configuration &&
      (snapshot.channels[0]?.effectChain[0]?.parameters.gain !== 0.9 ||
        snapshot.channels[1]?.effectChain[1]?.parameters.gain !== 0.4 ||
        snapshot.automation.at(-1)?.target.entityId !== snapshot.channels[0]?.effectChain[1]?.instanceId ||
        /instanceId|ins_[a-f0-9]/.test(saved))
    )
      throw new Error("plugin configuration, order or typed binding did not survive source reopen");
  } finally {
    reopened.close();
  }
