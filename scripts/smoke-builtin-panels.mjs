...
try {
  ...
  for (let i = 0; i < frames; i++)
    wav.writeInt16LE(Math.round(8000 * Math.sin((i / 48000) * Math.PI * 440) * Math.exp(-i / 16000)), 44 + i * 2);
  ...
  for (const name of cases) {
    ...
    const selected =
      name === "sampler"
        ? "sampler(sample, { rootKey: 60, amp: { attack: 0.005, decay: 0.25, sustain: 0.6, release: 0.4 } })"
        : name === "multisampler"
          ? "multisampler([{ sample, rootKey: 48, keyRange: [36, 59], velocityRange: [1, 64] }, { sample, rootKey: 60, keyRange: [36, 59], velocityRange: [65, 127] }, { sample, rootKey: 72, keyRange: [60, 84] }])"
          : name === "slicer"
            ? "slicer(sample, { slices: { grid: 8 }, triggerNote: 36 })"
            : "wavetable()";
    const parameters =
      name === "eq"
        ? { "band1.gainDb": 3, "band2.gainDb": -5, "band3.gainDb": 4, "band4.gainDb": 2 }
        : name === "filter"
          ? { cutoffHz: 1000 }
          : {};
    const source = [
      ...
      "const sample = project.addSample({ assetUri: './tone.wav', sha256: '" +
        hash +
        "', format: 'wav', sampleRate: 48000, channels: 1, frames: 48000n });",
      ...
      "const fx = { pluginId: 'oxitone." +
        (instrument ? "filter" : name) +
        "', pluginVersion: '1.0.0', parameters: " +
    ...
    if (editing) {
      ...
      try {
        const [first, second] = reopened.frame.snapshot.channels;
        if (
          !(first.effectChain[0].parameters.cutoffHz > 1000) ||
          first.effectChain[0].mix !== 0.7 ||
          first.effectChain[0].bypass !== true ||
          second.effectChain[0].parameters.cutoffHz !== 1000
        ) {
