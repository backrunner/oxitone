...
export async function dawPluginFixture(root, entry) {
  ...
  await promisify(execFile)("cc", [
    ...
    "-I",
    join(repo, "include"),
    join(repo, "crates/render/tests/fixtures/gain.c"),
    "-o",
    binary,
  ]);
...
{ pluginId: 'fixture.gain', pluginVersion: '1.0.0', parameters: { gain: .4 }, bypass: true }
  ...
  const source = (await readFile(entry, "utf8"))
    ...
    .replace(
      ...
      "project.addChannel({ name: 'Other rack', effectChain: rack() });\nkeys.effectInstances[0].param('gain').automate(pluginMotion().constant(.35));\nexport default project;",
    );
  await writeFile(entry, source);
  return async () => {
    if (hash(await readFile(binary)) !== sha256 || (await readFile(join(pkg, "index.js"), "utf8")) !== factory)
      throw new Error("DAW mutated npm plugin files");
  };
}
