...
const root = resolve("target/examples/full-songs"),
  out = resolve("target/examples/wasm");
...
for (const song of nativeReport.songs) {
  ...
  try {
    ...
    for (const sample of snapshot.samples) {
      ...
      assert.equal(info.sha256, sample.sha256);
      seen.add(sample.sha256);
    }
    const state = engine.compile(snapshot),
      frames = Math.round(song.durationSeconds * state.sampleRate);
    ...
    const memoryBefore = engine.memoryDiagnostics(),
      times = [];
    let peak = 0;
    ...
    const start = performance.now(),
      wav = engine.renderWav({ frames, bitDepth: 24, dither: true });
    const renderSeconds = (performance.now() - start) / 1000;
    await writeFile(resolve(out, song.slug + ".wav"), wav);
    const native = await readFile(resolve(root, song.slug + ".wav"));
    assert.equal(wav.length, native.length);
    function pcm24(bytes, index) {
      const v = bytes[index] | (bytes[index + 1] << 8) | (bytes[index + 2] << 16);
    ...
    const sectionLevels = song.sections.map(([name, bar], i) => {
      const begin = Math.round(((bar * 4 * 60) / song.bpm) * state.sampleRate),
        end = Math.min(
          frames,
          Math.round((((song.sections[i + 1]?.[1] ?? song.bars) * 4 * 60) / song.bpm) * state.sampleRate),
        );
      let sum = 0;
      for (let frame = begin; frame < end; frame++)
        for (let ch = 0; ch < 2; ch++) {
          const value = pcm24(wav, 44 + (frame * 2 + ch) * 3);
          sum += value * value;
        }
    ...
    for (let i = 44; i < wav.length; i += 3) {
      const value = pcm24(wav, i);
      square += value * value;
      fullPeak = Math.max(fullPeak, Math.abs(value));
    ...
    const item = {
      title: song.title,
      slug: song.slug,
      seconds: frames / state.sampleRate,
      sampleRate: state.sampleRate,
      blockSize: state.blockSize,
      samples: seen.size,
      renderSeconds,
      realtimeMultiple: frames / state.sampleRate / renderSeconds,
      p95Ms: times[Math.floor(times.length * 0.95)],
      p99Ms: times[Math.floor(times.length * 0.99)],
      processBlocks: times.length,
      processStartBar: dropStartBar,
      memoryBefore,
      memoryAfter,
      processAllocations: 0,
      processDeallocations: 0,
      peakDbfs: 20 * Math.log10(fullPeak),
      rmsDbfs: 10 * Math.log10(square / ((wav.length - 44) / 3)),
      maxPcmDifference,
      sectionLevels,
    };
    report.songs.push(item);
