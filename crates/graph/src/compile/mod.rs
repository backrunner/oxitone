...
pub fn compile_plan(
    ...
    // Note schedule: swing is compiled as 0 and applied at dispatch so
    // automated swing can shift events per event (03 §Channel swing).
    ...
    for channel in &channel_specs {
        for track in tracks.iter().filter(|t| {
            t.enabled != Some(false) && t.channel_ids.iter().any(|id| id == &channel.id)
        }) {
    ...
    let bindings =
        bindings::compile_bindings(snapshot, registry, &channel_index, &clip_index, seed)?;
